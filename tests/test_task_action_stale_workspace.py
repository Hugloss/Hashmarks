...
def test_deleted_edit_owner_cannot_remain_safe_action_without_watcher(tmp_path: Path) -> None:
    _write(tmp_path, "src/foo.py", "def calculate_total(): return 1\n")
    _write(tmp_path, "tests/test_foo.py", "from src.foo import calculate_total\ndef test_total(): assert calculate_total()==1\n")
...
def test_rewritten_edit_owner_cannot_reuse_stale_symbol_evidence(tmp_path: Path) -> None:
    _write(tmp_path, "src/foo.py", "def calculate_total(): return 1\n")
    _write(tmp_path, "tests/test_foo.py", "from src.foo import calculate_total\ndef test_total(): assert calculate_total()==1\n")
    with CodeMap(tmp_path) as codemap:
        codemap.sync()
        assert codemap.task_action_map("fix calculate_total")["edit"]["path"] == "src/foo.py"
...
def test_cold_map_rejects_module_only_owner_when_imported_symbol_is_gone(tmp_path: Path) -> None:
    _write(tmp_path, "src/foo.py", "def unrelated(): return 2\n")
    _write(tmp_path, "tests/test_foo.py", "from src.foo import calculate_total\ndef test_total(): assert calculate_total()==1\n")
    with CodeMap(tmp_path) as codemap:
        codemap.sync()
...
def test_changed_projected_owner_reconciles_before_cached_authority_is_reused(tmp_path: Path) -> None:
    _write(tmp_path, "src/foo.py", "def calculate_total(): return 1\n")
    _write(tmp_path, "tests/test_foo.py", "from src.foo import calculate_total\ndef test_total(): assert calculate_total()==1\n")
    with CodeMap(tmp_path) as codemap:
        codemap.sync()
...
def test_unsignaled_owner_rename_converges_to_cold_truth(tmp_path: Path) -> None:
    ...
    with CodeMap(tmp_path) as codemap:
        ...
        _write(tmp_path, "tests/test_foo.py", "from src.bar import calculate_total\ndef test_total(): assert calculate_total()==1\n")
...
def test_unsignaled_module_package_transition_converges(tmp_path: Path, before_path: str, after_path: str) -> None:
    _write(tmp_path, before_path, "def calculate_total(): return 1\n")
    _write(tmp_path, "tests/test_foo.py", "from src.foo import calculate_total\ndef test_total(): assert calculate_total()==1\n")
    with CodeMap(tmp_path) as codemap:
        codemap.sync()
        assert codemap.task_action_map("fix calculate_total")["edit"]["path"] == before_path
...
def test_unsignaled_aba_owner_restoration_reconciles_from_recent_authority(tmp_path: Path) -> None:
    _write(tmp_path, "src/foo.py", "def calculate_total(): return 1\n")
    _write(tmp_path, "tests/test_foo.py", "from src.foo import calculate_total\ndef test_total(): assert calculate_total()==1\n")
    with CodeMap(tmp_path) as codemap:
        codemap.sync()
...
def test_unsignaled_rename_reconciliation_retires_obsolete_history(tmp_path: Path, monkeypatch) -> None:
    _write(tmp_path, "src/foo.py", "def calculate_total(): return 1\n")
    _write(tmp_path, "tests/test_foo.py", "from src.foo import calculate_total\ndef test_total(): assert calculate_total()==1\n")
    with CodeMap(tmp_path) as codemap:
        codemap.sync()
        ...
        (tmp_path / "src/foo.py").rename(tmp_path / "src/bar.py")
        _write(tmp_path, "tests/test_foo.py", "from src.bar import calculate_total\ndef test_total(): assert calculate_total()==1\n")
        assert codemap.task_action_map("fix calculate_total")["edit"]["path"] == "src/bar.py"
        for _ in range(3):
            assert codemap.task_action_map("fix calculate_total")["edit"]["path"] == "src/bar.py"
...
def test_unsignaled_delete_recreate_recovers_first_task_decision_after_disappearance(tmp_path: Path) -> None:
    ...
    with CodeMap(tmp_path, state_dir=tmp_path / ".cold-state", artifact_db=tmp_path / "cold-artifacts.sqlite3") as cold:
        cold.sync()
        expected = cold.task_action_map("fix calculate_total")
...
def test_unsignaled_file_symlink_file_transition_recovers_without_following_symlink(tmp_path: Path) -> None:
    _write(tmp_path, "src/foo.py", "def calculate_total(): return 1\n")
    _write(tmp_path, "tests/test_foo.py", "from src.foo import calculate_total\ndef test_total(): assert calculate_total()==1\n")
...
def test_delete_recreate_resurrection_survives_recent_authority_cache_pressure(tmp_path: Path) -> None:
    _write(tmp_path, "src/foo.py", "def calculate_total(): return 1\n")
    _write(tmp_path, "tests/test_foo.py", "from src.foo import calculate_total\ndef test_total(): assert calculate_total()==1\n")
    source = tmp_path / "src/foo.py"
    original = source.read_bytes()
