...
def test_scale_generator_separates_public_and_secret(tmp_path: Path):
    base=tmp_path/'repos'; public=tmp_path/'public.json'; secret=tmp_path/'secret.json'
    manifest=generate(base,public,secret,sizes=[10],shapes=['diamond'],contracts=2)
    pub=json.loads(public.read_text()); sec=json.loads(secret.read_text())
...
def test_scale_measurement_exposes_provenance_limit_without_hiding_project_recall(tmp_path: Path):
    base=tmp_path/'repos'; public=tmp_path/'public.json'; secret=tmp_path/'secret.json'; output=tmp_path/'out.json'
