...
def test_authority_graph_v3_composes_unguarded_concurrency_risk_as_nomination(tmp_path):
    ...
    with CodeMap(tmp_path) as codemap:
        codemap.sync()
        graph = codemap.repository_ownership_graph(["store.py"])
    assert graph["schema"] == "hashmarks.authority-ownership-graph.v3"
    ...
    edge = next(
        e for e in graph["edges"] if e["relation"] == "contains-concurrency-risk"
    )
...
def test_repository_ownership_graph_materializes_repository_rows_once(
...
def test_structural_owner_prefers_nearest_corroborated_candidate(tmp_path):
    (tmp_path / "src").mkdir()
    (tmp_path / "tests").mkdir()
    (tmp_path / "src/__init__.py").write_text("", encoding="utf-8")
    (tmp_path / "src/plan.py").write_text(
        "class StartupPlan:\n    pass\n", encoding="utf-8"
    )
    (tmp_path / "src/supervisor.py").write_text(
        "from src.plan import StartupPlan\n\n"
        "def startup_output():\n"
        "    return StartupPlan\n",
        encoding="utf-8",
    )
    (tmp_path / "tests/test_supervisor.py").write_text(
        "from src.supervisor import startup_output\n\n"
    ...
    with CodeMap(tmp_path) as codemap:
        ...
        graph = codemap.ownership_relation_graph(
            "startup output redaction",
            "tests/test_supervisor.py",
            max_depth=2,
        )

    assert graph["selected"] == "src/supervisor.py"
    assert graph["selection_reason"] == "bounded-two-hop-corroboration"
    assert graph["owner_path"] == [
        {
            "from": "tests/test_supervisor.py",
            "to": "src/supervisor.py",
            "relation": "imports",
        }
    ]
