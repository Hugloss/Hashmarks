...
def test_action_ambiguity_explains_plausibility_and_discriminator_without_secret(tmp_path: Path) -> None:
    (tmp_path / "src").mkdir(); (tmp_path / "tests").mkdir()
    (tmp_path / "src" / "widget.py").write_text("def widget():\n    return 1\n")
    (tmp_path / "tests" / "test_widget.py").write_text("from src.widget import widget\ndef test_widget(): assert widget() == 1\n")
...
def test_decision_packet_carries_ambiguity_only_when_scout_needs_it(tmp_path: Path) -> None:
    (tmp_path / "src").mkdir(); (tmp_path / "tests").mkdir()
    (tmp_path / "src" / "widget.py").write_text("def widget():\n    return 1\n")
    ...
    if packet["discrimination"]["needed"] and packet["discrimination"]["reason"] == "competing-action-roles":
        assert packet["discrimination"]["ambiguity"]["schema"] == "hashmarks.action-ambiguity.v2"
    else:
