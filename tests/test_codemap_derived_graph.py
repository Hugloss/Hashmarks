...
def test_sync_persists_dependency_tracked_derived_surfaces(tmp_path: Path, monkeypatch) -> None:
    ...
    assert set(nodes) == {"content", "symbol_surface", "relationship_surface", "outline_surface", "lexical_surface"}
...
def test_body_only_change_can_preserve_symbol_surface_identity(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("XDG_CACHE_HOME", str(tmp_path / "cache"))
    ws = tmp_path / "repo"; ws.mkdir(); source = ws / "service.py"
    source.write_text("def public(x: int) -> int:\n    return x + 1\n")
    with CodeMap(ws) as codemap:
        codemap.sync(); before = {x["kind"]: x for x in codemap.derived_graph("service.py")["nodes"]}
        source.write_text("def public(x: int) -> int:\n    y = x + 1\n    return y\n")
        codemap.sync(["service.py"]); after = {x["kind"]: x for x in codemap.derived_graph("service.py")["nodes"]}
    assert before["content"]["identity"] != after["content"]["identity"]
    assert before["symbol_surface"]["identity"] == after["symbol_surface"]["identity"]
    assert before["symbol_surface"]["input_identities"] != after["symbol_surface"]["input_identities"]


def test_signature_change_changes_symbol_and_outline_surfaces(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("XDG_CACHE_HOME", str(tmp_path / "cache"))
    ws = tmp_path / "repo"; ws.mkdir(); source = ws / "service.py"
    source.write_text("def public(x: int) -> int:\n    return x\n")
    with CodeMap(ws) as codemap:
...
def test_deleted_file_removes_derived_nodes(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("XDG_CACHE_HOME", str(tmp_path / "cache"))
    ws = tmp_path / "repo"; ws.mkdir(); source = ws / "service.py"; source.write_text("x = 1\n")
    with CodeMap(ws) as codemap:
        codemap.sync(); assert codemap.derived_graph("service.py")["nodes"]
        source.unlink(); codemap.sync(["service.py"])
        assert codemap.derived_graph("service.py")["nodes"] == []


def test_body_only_change_reports_semantic_invalidation_shields(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("XDG_CACHE_HOME", str(tmp_path / "cache"))
    ws = tmp_path / "repo"; ws.mkdir(); source = ws / "service.py"
    source.write_text("def public(x: int) -> int:\n    return x + 1\n")
    with CodeMap(ws) as codemap:
...
def test_signature_change_does_not_shield_symbol_surface(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("XDG_CACHE_HOME", str(tmp_path / "cache"))
    ws = tmp_path / "repo"; ws.mkdir(); source = ws / "service.py"
    source.write_text("def public(x: int) -> int:\n    return x\n")
    with CodeMap(ws) as codemap:
...
def test_shielding_never_skips_line_level_storage_refresh(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("XDG_CACHE_HOME", str(tmp_path / "cache"))
    ws = tmp_path / "repo"; ws.mkdir(); source = ws / "service.py"
    ...
    with CodeMap(ws) as codemap:
        ...
        source.write_text("# inserted line\ndef first():\n    return 1\n\ndef second():\n    return 2\n")
        result = codemap.sync(["service.py"])
...
def test_change_impact_projects_reachable_files_into_repository_surfaces(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("XDG_CACHE_HOME", str(tmp_path / "cache"))
    ws = tmp_path / "repo"; ws.mkdir()
    (ws / "src").mkdir(); (ws / "tests").mkdir(); (ws / "docs").mkdir()
    (ws / "core.py").write_text("def value():\n    return 1\n")
    (ws / "api.py").write_text("from core import value\ndef public():\n    return value()\n")
    (ws / "tests" / "test_api.py").write_text("from api import public\ndef test_public():\n    assert public() == 1\n")
    with CodeMap(ws) as codemap:
        codemap.sync()
    ...
    assert all(row["provenance"] == "reverse-file-graph" for row in impact["surfaces"]["implementation"])


def test_change_impact_is_bounded_and_does_not_change_task_retrieval(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("XDG_CACHE_HOME", str(tmp_path / "cache"))
    ws = tmp_path / "repo"; ws.mkdir()
    (ws / "a.py").write_text("def root():\n    return 1\n")
    for index in range(5):
        (ws / f"b{index}.py").write_text(f"from a import root\ndef use_{index}():\n    return root()\n")
    with CodeMap(ws) as codemap:
        codemap.sync()
...
def test_derived_graph_revalidates_exact_path_after_unsignaled_rewrite(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("XDG_CACHE_HOME", str(tmp_path / "cache"))
    ws = tmp_path / "repo"; ws.mkdir(); source = ws / "service.py"
    source.write_text("def public(x: int) -> int:\n    return x\n")
    with CodeMap(ws) as codemap:
        codemap.sync()
        before = {row["kind"]: row for row in codemap.derived_graph("service.py")["nodes"]}
        source.write_text("def renamed(x: str) -> str:\n    return x\n")
        after = {row["kind"]: row for row in codemap.derived_graph("service.py")["nodes"]}
    assert before["content"]["identity"] != after["content"]["identity"]
    assert before["symbol_surface"]["identity"] != after["symbol_surface"]["identity"]


def test_derived_graph_obeys_live_context_policy_before_exposing_nodes(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("XDG_CACHE_HOME", str(tmp_path / "cache"))
    ws = tmp_path / "repo"; ws.mkdir(); private = ws / "private"; private.mkdir()
    source = private / "hidden.py"; source.write_text("def hidden():\n    return 1\n")
    with CodeMap(ws) as codemap:
        codemap.sync()
