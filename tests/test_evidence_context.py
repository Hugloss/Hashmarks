...
from hashmarks.evidence_context import (
...
def test_native_context_validation_rejects_tampered_revision_or_freshness() -> None:
    ...
    for key, value in (("revision", "r2"), ("freshness", "current")):
        ...
        result = validate_evidence_context(
            receipt, tampered, producer_implementation_identity=PRODUCER
        )
...
def test_native_context_validation_rejects_tampered_authority() -> None:
    ...
    changed["evidence_identity"] = "other"
    assert (
        validate_evidence_context(
            changed, provenance, producer_implementation_identity=PRODUCER
        )["valid"]
        is False
    )
...
def test_context_identity_rejects_nonportable_generation_and_scalar_coercion() -> None:
    ...
    for generation in (True, 1.0, -1, math.nan, 2**53):
        receipt = _receipt()
        receipt["codemap_generation"] = generation
        with pytest.raises(ValueError):
            evidence_context_identity(
                receipt, _provenance(), producer_implementation_identity=PRODUCER
            )
    provenance = _provenance()
    provenance["freshness"] = False
    with pytest.raises(ValueError):
        evidence_context_identity(
            _receipt(), provenance, producer_implementation_identity=PRODUCER
        )
