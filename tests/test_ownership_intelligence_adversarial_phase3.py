...
def test_authority_graph_does_not_attach_reader_to_same_named_cache_in_other_module(tmp_path):
    (tmp_path/"a.py").write_text("CACHE={}\n")
    (tmp_path/"b.py").write_text("CACHE={}\n")
    (tmp_path/"reader.py").write_text("from a import CACHE\nVALUE=CACHE.get('x')\n")
    with _sync(tmp_path) as c: graph=c.repository_ownership_graph()
    edges=[e for e in graph["edges"] if e["relation"]=="references-cache-owner" and e["source"]=="file:reader.py"]
...
def test_repository_wide_rmw_scan_excludes_test_fixture_mutation_but_explicit_path_can_inspect(tmp_path):
