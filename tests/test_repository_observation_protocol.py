...
def test_repository_observation_returns_typed_atomic_snapshot(tmp_path: Path, monkeypatch):
...
@pytest.mark.parametrize(
    "payload",
    [
        {"observation": "dirty", "generation": 2, "dirty_paths": 2, "paths": ["a.py"], "paths_complete": True},
        {"observation": "dirty", "generation": 2, "dirty_paths": 1, "paths": [1], "paths_complete": True},
        {"observation": "dirty", "generation": 2, "dirty_paths": 2, "paths": ["a.py"], "paths_complete": False},
        {"observation": "clean", "generation": 2, "dirty_paths": 1, "paths": ["a.py"], "paths_complete": True},
        {"observation": "unknown", "generation": 2, "dirty_paths": 1, "paths": ["a.py"], "paths_complete": True},
    ],
)
def test_repository_observation_rejects_incoherent_payloads(tmp_path: Path, monkeypatch, payload):
    client = _client(tmp_path)
    monkeypatch.setattr(client, "request", lambda op: {"ok": True, "reason": None, **payload})
