...
def test_v2_inventory_refuses_graph_only_source_authority(tmp_path: Path) -> None:
    changed = _snapshot_v2()
    changed["inventory"][0]["evidence_sources"] = ["tree:compile"]

    with CodeMap(tmp_path) as codemap:
        codemap.sync()
        with pytest.raises(
            ValueError,
            match="inventory requires resolved-inventory evidence source",
        ):
            codemap.dependency_resolution_evidence(changed)
