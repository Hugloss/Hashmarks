...
from hashmarks.codemap.repository_index_store import WorkspaceMapStore, WorkspaceStateMismatchError
...
def _repo(path: Path, *, symbol: str) -> None:
    ...
    (path / "source.py").write_text(f"def {symbol}():\n    return 1\n", encoding="utf-8")
...
def test_external_state_dir_is_bound_to_exactly_one_workspace(tmp_path: Path) -> None:
    ...
    with CodeMap(repo_a, state_dir=state, artifact_db=tmp_path / "a-artifacts.sqlite3") as codemap:
        codemap.sync()
        assert codemap.find("alpha_only")

    with pytest.raises(WorkspaceStateMismatchError, match="already bound to a different workspace"):
...
def test_concurrent_external_state_first_claim_has_one_workspace_owner(tmp_path: Path) -> None:
...
def test_in_workspace_unbound_existing_state_can_adopt_its_owner(tmp_path: Path) -> None:
    workspace = tmp_path / "repo"
    workspace.mkdir()
    ...
    with CodeMap(workspace, state_dir=state, artifact_db=tmp_path / "artifacts.sqlite3") as codemap:
