...
def test_ordinary_tests_do_not_dynamically_reexecute_modules_per_test() -> None:
    forbidden = {"importlib.util.spec_from_file_location", "importlib.util.module_from_spec"}
    violations: list[str] = []
    for path in sorted((ROOT / "tests").glob("test_*.py")):
        ...
        for node in ast.walk(tree):
            ...
            for child in ast.walk(node):
                ...
                if isinstance(child, ast.Call) and isinstance(child.func, ast.Attribute) and child.func.attr == "exec_module":
                    violations.append(f"{path.name}:{node.lineno}:exec_module")
    assert violations == [], "per-test dynamic module re-execution defeats normal module/cache ownership: " + ", ".join(violations)
    ...
    for path in sorted((ROOT / "hashmarks").rglob("*.py")):
        ...
        for node in ast.walk(tree):
            ...
            if isinstance(parent, (ast.FunctionDef, ast.AsyncFunctionDef)):
                violations.append(f"{path.relative_to(ROOT)}:{node.lineno}:nested cached function")
            elif isinstance(parent, ast.ClassDef):
                decorators = {d.id for d in node.decorator_list if isinstance(d, ast.Name)}
                if "staticmethod" not in decorators:
                    violations.append(f"{path.relative_to(ROOT)}:{node.lineno}:instance/class-owned cache")
    assert violations == [], "cache lifetime must not be owned by mutable objects or recreated inside calls: " + ", ".join(violations)


def test_hot_codemap_loops_do_not_issue_known_row_at_a_time_store_reads() -> None:
    path = ROOT / "hashmarks" / "codemap" / "engine.py"
    tree = ast.parse(path.read_text(encoding="utf-8"), filename=str(path))
    guarded = {"_find_impl", "ownership_relation_graph", "task_graph_adjacency", "deps", "refs", "_context_impl"}
    row_reads = {"file_row", "edges_from", "refs", "symbols_named"}
    violations: list[str] = []
    for fn in [n for n in ast.walk(tree) if isinstance(n, ast.FunctionDef) and n.name in guarded]:
        for loop in [n for n in ast.walk(fn) if isinstance(n, (ast.For, ast.While, ast.ListComp, ast.SetComp, ast.DictComp, ast.GeneratorExp))]:
            for child in ast.walk(loop):
                if not isinstance(child, ast.Call) or not isinstance(child.func, ast.Attribute):
                    continue
                if child.func.attr in row_reads and isinstance(child.func.value, ast.Attribute) and child.func.value.attr == "store":
                    violations.append(f"{fn.name}:{getattr(child, 'lineno', '?')}:{child.func.attr}")
    ...
    unexpected = [v for v in violations if not any(v.startswith(prefix.split(':')[0] + ':') and v.endswith(':' + prefix.split(':')[1]) for prefix in allowed_prefixes)]
    assert unexpected == [], "hot path added row-at-a-time store reads instead of bulk/stable-key reuse: " + ", ".join(unexpected)


def test_bounded_rerank_topn_matches_reference_full_sort(tmp_path: Path) -> None:
    from hashmarks.codemap.engine import CodeMap, _query_terms
    ...
    with CodeMap(tmp_path, state_dir=tmp_path / ".state") as codemap:
        ...
        for row in ordered:
            ...
            if len(reference) >= rerank_limit:
                break
        actual = codemap._bounded_rerank_rows(query, tuple(rows), limit=limit, tokens=tokens)
...
def test_symbols_for_paths_many_matches_per_path_prefixes(tmp_path: Path) -> None:
    ...
    with CodeMap(tmp_path, state_dir=tmp_path / ".state") as codemap:
        codemap.sync()
        expected = {
            path: codemap.store.symbols_for_path(path)[:2]
            for path in ("a.py", "b.py")
        }
        codemap.store.reset_read_counters()
        actual = codemap.store.symbols_for_paths_many(["a.py", "b.py"], limit_per_path=2)
        counters = codemap.store.read_counters()
    assert actual == expected
    assert counters == {"symbols_for_paths_many": 1}


def test_decision_session_exact_symbol_cache_reuses_larger_prefix(tmp_path: Path) -> None:
    from hashmarks.codemap.engine import CodeMap
...
def test_decision_session_reuses_graph_prefixes_and_file_rows(tmp_path: Path) -> None:
    ...
    with CodeMap(tmp_path, state_dir=tmp_path / ".state") as codemap:
        ...
        with codemap.decision_session():
            ...
            assert edges_narrow[("owner.py", "Owner.work")] == edges_wide[("owner.py", "Owner.work")][:1]

            by_path_wide = codemap._session_edges_for_paths_many(["owner.py"], limit_per_path=20)
            by_path_narrow = codemap._session_edges_for_paths_many(["owner.py"], limit_per_path=1)
...
def test_reverse_reference_identity_is_canonicalized_by_target_short(tmp_path: Path) -> None:
    from hashmarks.codemap.engine import CodeMap
...
def test_decision_symbol_preload_uses_complete_multi_path_evidence(tmp_path: Path) -> None:
    from hashmarks.codemap.engine import CodeMap
    ...
    with CodeMap(tmp_path, state_dir=tmp_path / ".state") as codemap:
        ...
        with codemap.decision_session():
            ...
            stats = codemap.decision_session_stats()
        assert [row["qualname"] for row in dense] == [row["qualname"] for row in expected_dense]
        assert [row["qualname"] for row in later] == [row["qualname"] for row in expected_later]
        assert stats.get("store_symbols_for_paths_complete") == 1
        assert stats.get("store_symbols_for_path", 0) == 0


def test_module_paths_many_matches_individual_and_session_preload(tmp_path: Path) -> None:
    from hashmarks.codemap.engine import CodeMap

    (tmp_path / "pkg").mkdir()
    (tmp_path / "pkg" / "__init__.py").write_text("from .owner import Owner\n", encoding="utf-8")
    (tmp_path / "pkg" / "owner.py").write_text("class Owner:\n    pass\n", encoding="utf-8")
    (tmp_path / "pkg" / "helper.py").write_text("def helper():\n    return 1\n", encoding="utf-8")
    with CodeMap(tmp_path, state_dir=tmp_path / ".state") as codemap:
        codemap.sync()
        modules = ["pkg.owner.Owner", "pkg.owner", "pkg.helper.helper", "pkg.helper", "pkg.missing"]
        ...
        with codemap.decision_session():
            ...
            resolved = {module: codemap._session_module_paths(module) for module in modules}
            stats = codemap.decision_session_stats()
...
def test_tooling_does_not_parse_direct_path_read_text_without_ast_cache() -> None:
    ...
    for base in roots:
        for path in sorted(base.rglob("*.py")):
            tree = ast.parse(path.read_text(encoding="utf-8"), filename=str(path))
            for node in ast.walk(tree):
                if not isinstance(node, ast.Call) or _call_name(node) != "ast.parse" or not node.args:
                    continue
                first = node.args[0]
    ...
    assert violations == [], "route file-backed Python AST parsing through hashmarks.python_ast_cache: " + ", ".join(violations)
