...
def _repo(root: Path) -> str:
    ...
    (root / "src" / "case" / "engine.py").write_text("def ember(v): return v + '-old'\n")
...
def _row_key(row: dict[str, object]) -> tuple[str, str | None]:
    kind = str(row["kind"])
    member = str(row["member"]) if kind == "negative-verification-evidence" and row.get("member") is not None else None
    return kind, member


def _by_key(freshness: dict[str, object]) -> dict[tuple[str, str | None], dict[str, object]]:
...
def test_freshness_map_is_derived_bounded_repository_truth(tmp_path: Path) -> None:
    ...
    with CodeMap(tmp_path) as codemap:
        ...
        freshness = codemap.evidence_freshness_map(
            task, ["src/case/engine.py"], negative_members=["tests/test_other.py"],
        )
...
def test_prior_map_invalidates_only_changed_evidence_identity(tmp_path: Path) -> None:
    ...
    with CodeMap(tmp_path) as codemap:
        ...
        (tmp_path / "src" / "case" / "engine.py").write_text("def ember(v): return v + '-new'\n")
        codemap.sync()
    ...
    assert prior[("negative-verification-evidence", "tests/test_other.py")]["state"] == "current"
...
def test_selected_member_invalidates_prior_negative_claim(tmp_path: Path) -> None:
    ...
    with CodeMap(tmp_path) as codemap:
        codemap.sync()
        freshness = codemap.evidence_freshness_map(
            task, ["src/case/engine.py"], negative_members=["tests/test_ember.py"],
        )
...
def test_cross_repository_freshness_is_dependency_bound(tmp_path: Path) -> None:
    ...
    with CodeMap(tmp_path) as codemap:
        ...
        freshness = codemap.evidence_freshness_map("backend value change", ["backend/value.ts"])

    rows = _by_key(freshness)
    cross = rows[("cross-repository", None)]
    assert cross["state"] == "dependent"
    assert cross["dependency_state"] == "current"
    assert any(row["producer"] == "declared-project-links" for row in cross["dependencies"])
...
def test_service_exposes_freshness_map(tmp_path: Path) -> None:
    ...
    try:
        freshness = client.repository_intelligence_query(
            "freshness", task, ["src/case/engine.py"], negative_members=["tests/test_other.py"],
        )["result"]
        assert freshness["schema"] == "hashmarks.evidence-freshness-map.v1"
    finally:
        client.stop()
...
def test_negative_evidence_invalidates_when_reference_evidence_changes(tmp_path: Path) -> None:
    task = _repo(tmp_path)
    with CodeMap(tmp_path) as codemap:
        codemap.sync()
        before = codemap.evidence_freshness_map(
    ...
    prior = {_row_key(row): row for row in after["prior"]}
    assert prior[("negative-verification-evidence", "tests/test_other.py")]["state"] == "invalidated"


def test_cross_repository_prior_invalidates_on_declared_dependency_change(tmp_path: Path) -> None:
    ...
    with CodeMap(tmp_path) as codemap:
        codemap.sync()
        codemap.enrich_projects(("npm-package-graph", "declared-project-links"))
        before = codemap.evidence_freshness_map("backend value change", ["backend/value.ts"])
        links.write_text(
            "[[link]]\nsource='npm:frontend'\ntarget='npm:backend'\nkind='consumer'\n"
            "[[link]]\nsource='npm:mobile'\ntarget='npm:backend'\nkind='consumer'\n"
        )
        codemap.sync()
        after = codemap.evidence_freshness_map(
            "backend value change", ["backend/value.ts"], previous_map=before,
        )
