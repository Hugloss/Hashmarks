...
def test_producer_identity_provenance_explains_same_version_byte_drift(tmp_path: Path) -> None:
    ...
    assert _rows(before)["engine.py"]["content_sha256"] != _rows(after)["engine.py"]["content_sha256"]


def test_producer_identity_provenance_explains_same_content_rename(tmp_path: Path) -> None:
    a = _package(tmp_path / "a")
    ...
    assert _rows(before)["engine.py"]["content_sha256"] == _rows(after)["core.py"]["content_sha256"]


def test_producer_identity_provenance_ignores_inode_and_mtime_churn(tmp_path: Path) -> None:
...
def test_producer_identity_provenance_is_relative_bounded_metadata() -> None:
    ...
    assert receipt["implementation_identity"] == native_producer_implementation_identity()
