...
def test_brief_preserves_action_and_verification_without_verbose_candidates(tmp_path: Path) -> None:
    ...
    with CodeMap(tmp_path) as c:
        c.sync(); full=c.task_decision_packet('widget implementation test'); brief=c.task_decision_brief('widget implementation test')
    ...
    assert len(json.dumps(brief,sort_keys=True)) < len(json.dumps(full,sort_keys=True)) * 0.55
...
def test_brief_deduplicates_contract_when_edit_is_contract(tmp_path: Path) -> None:
    ...
    (tmp_path/'tests'/'test_policy.py').write_text("def test_policy(): assert True\n")
    with CodeMap(tmp_path) as c:
        c.sync(); brief=c.task_decision_brief('change policy config test')
