...
def test_task_action_brief_is_smaller_than_authority_brief_and_action_complete(tmp_path: Path) -> None:
    _repo(tmp_path)
    ...
    assert len(json.dumps(agent, sort_keys=True, separators=(',', ':')).encode()) < len(json.dumps(full, sort_keys=True, separators=(',', ':')).encode())


def test_task_action_brief_fails_closed_without_hiding_missing_roles(tmp_path: Path) -> None:
    _repo(tmp_path)
    with CodeMap(tmp_path) as c:
        c.sync()
...
def test_task_action_brief_auto_budget_qualifies_instead_of_assuming_fixed_minimum(tmp_path: Path) -> None:
    _repo(tmp_path)
    with CodeMap(tmp_path) as c:
        c.sync()
...
def test_task_action_brief_fast_budget_matches_diagnostic_sweep(tmp_path: Path) -> None:
    ...
    with CodeMap(tmp_path) as c:
        c.sync()
        sweep = c.task_decision_brief_budget_sweep('widget implementation test', budgets=[1, 32, 64, 96, 128])
        auto = c.task_action_brief('widget implementation test', candidate_budgets=[1, 32, 64, 96, 128])
        expected = c.task_action_brief('widget implementation test', token_budget=sweep['smallest_safe_budget'])
    assert sweep['smallest_safe_budget'] is not None
    assert auto == expected


def test_task_action_brief_auto_budget_does_not_call_diagnostic_sweep(tmp_path: Path) -> None:
    _repo(tmp_path)
    with CodeMap(tmp_path) as c:
        c.sync()
        def forbidden(*args, **kwargs):
            raise AssertionError('diagnostic sweep must not run on production brief path')
...
def test_rare_ticket_action_brief_uses_exact_index_anchor_without_general_find(tmp_path: Path) -> None:
    (tmp_path/'src'/'abc123').mkdir(parents=True)
    (tmp_path/'tests').mkdir()
    (tmp_path/'archive').mkdir()
    (tmp_path/'src'/'abc123'/'engine_a.py').write_text('def ABC123(): return "old"\n')
    (tmp_path/'src'/'abc123'/'engine_b.py').write_text('def ABC123(): return "old"\n')
    ...
    (tmp_path/'archive'/'abc123_old.py').write_text('def ABC123(): return "legacy"\n')
    with CodeMap(tmp_path) as c:
        c.sync()
...
def test_rare_ticket_fast_path_is_not_used_for_configuration_authority(tmp_path: Path) -> None:
    (tmp_path/'src'/'abc123').mkdir(parents=True)
    (tmp_path/'tests').mkdir()
    ...
    with CodeMap(tmp_path) as c:
        c.sync()
        def forbidden(*args, **kwargs):
            raise AssertionError('configuration authority must use the normal evidence path')
...
def test_multi_term_task_local_island_beats_large_generic_vocabulary(tmp_path: Path) -> None:
...
def test_multi_term_island_does_not_activate_when_multiple_tests_share_terms(tmp_path: Path) -> None:
    (tmp_path / "src/a").mkdir(parents=True)
    (tmp_path / "src/b").mkdir(parents=True)
    (tmp_path / "tests").mkdir()
    for name in ("a", "b"):
        (tmp_path / f"src/{name}/engine.py").write_text("def resolve(): return 'old'\n# cobalt grove\n")
        (tmp_path / f"tests/test_{name}.py").write_text(f"from src.{name}.engine import resolve\ndef test_{name}(): assert resolve() == 'old'\n# cobalt grove\n")
    codemap = CodeMap(tmp_path)
    codemap.sync()
    try:
        hits = codemap._task_local_lexical_island_hits("cobalt grove behavior", limit=20)
        assert hits == ()
    finally:
        codemap.close()


def test_task_action_brief_never_claims_safe_without_edit_target(tmp_path: Path) -> None:
    (tmp_path / "tests").mkdir()
    (tmp_path / "tests/test_only.py").write_text("def test_only(): assert True\n# scarlet field behavior\n")
    codemap = CodeMap(tmp_path)
    codemap.sync()
    try:
        brief = codemap.task_action_brief("Change the scarlet field behavior and verify it")
...
def test_multi_term_lexical_island_does_not_intercept_plain_compound_symbol_lookup(tmp_path: Path) -> None:
    ...
    codemap = CodeMap(tmp_path); codemap.sync()
    try:
