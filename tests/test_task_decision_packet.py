...
def test_decision_packet_does_not_require_discrimination_when_action_is_resolved(tmp_path: Path) -> None:
...
def test_decision_packet_requires_discrimination_when_no_safe_edit_exists(tmp_path: Path) -> None:
...
def test_structural_owner_resolution_does_not_admit_redundant_discrimination(tmp_path: Path) -> None:
    (tmp_path / "src").mkdir(); (tmp_path / "tests").mkdir()
    (tmp_path / "src" / "engine.py").write_text("def apply_widget(value):\n    return value + '-old'\n")
    (tmp_path / "src" / "route.py").write_text("from .engine import apply_widget\ndef handle_widget(value): return apply_widget(value)\n")
    (tmp_path / "src" / "legacy.py").write_text("def handle_widget(value): return value + '-old'\n")
    (tmp_path / "tests" / "test_widget.py").write_text("from src.route import handle_widget\ndef test_widget_contract(): assert handle_widget('x') == 'x-new'\n")
    with CodeMap(tmp_path) as codemap:
        codemap.sync()
        packet = codemap.task_decision_packet("Fix widget accepted response to '-new'", limit=20)
    assert packet["edit"]["path"] == "src/engine.py"
    assert packet["discrimination"]["needed"] is False
    assert packet["discrimination"]["reason"] == "resolved"


def test_decision_packet_exposes_provenance_bound_verification_selection(tmp_path: Path) -> None:
    (tmp_path / "src").mkdir(); (tmp_path / "tests").mkdir()
    (tmp_path / "src" / "adapter.py").write_text("class Adapter:\n    pass\n")
    ...
    assert envelope["repository"]["repository_identity"] == packet["identity"]["repository_identity"]
    assert envelope["repository"]["source_identity"] == packet["identity"]["source_identity"]
    assert packet["identity"]["verification_membership_identity"] == membership["membership_identity"]
    assert (
