...
def test_query_facade_delegates_without_changing_producer_semantics(tmp_path: Path) -> None:
    _source, task = _repo(tmp_path)
    with CodeMap(tmp_path) as codemap:
        codemap.sync()
        expected = {
            "change-intelligence": codemap.change_intelligence_brief(task, ["src/owner.py"]),
            ...
            "profile": codemap.repository_intelligence_profile(task, ["src/owner.py"], profile="compact"),
            "cross-repository": codemap.cross_repository_evidence_packet(task, ["src/owner.py"]),
            "verification-explanation": codemap.explain_verification_selection(task, "tests/test_owner.py"),
        }
        actual = {
            surface: codemap.repository_intelligence_query(
                ...
                member_path="tests/test_owner.py" if surface == "verification-explanation" else None,
            )
...
def test_query_facade_is_deterministic_and_fails_closed(tmp_path: Path) -> None:
    ...
    with CodeMap(tmp_path) as codemap:
        ...
        second = codemap.repository_intelligence_query("profile", task, ["src/owner.py"])
        assert first == second
...
def test_query_facade_service_roundtrip(tmp_path: Path) -> None:
    ...
    try:
        ...
        result = client.repository_intelligence_query(
            "profile", task, ["src/owner.py"], profile="compact",
        )
...
def test_service_v2_rejects_removed_direct_repository_intelligence_ops(tmp_path: Path) -> None:
    _source, task = _repo(tmp_path)
    ...
    for op in removed:
        with pytest.raises(ValueError, match="unsupported operation"):
            service.dispatch({"protocol": "hashmarks.codemap-service.v2", "op": op, "task": task})


def test_service_client_exposes_single_repository_intelligence_protocol_facade(tmp_path: Path) -> None:
