...
def test_typescript_verification_plan_uses_existing_tsconfig(tmp_path: Path) -> None:
    ...
    (tmp_path / "tests").mkdir()
    (tmp_path / "tests" / "ember.test.ts").write_text(
        "export const expected: 'ember' = 'ember';\n", encoding="utf-8"
    )
    with CodeMap(tmp_path) as codemap:
...
def test_go_test_file_is_classified_as_verification_surface(tmp_path: Path) -> None:
    ...
    (tmp_path / "route" / "route_test.go").write_text(
...
def test_javascript_test_file_is_classified_as_verification_surface(
    tmp_path: Path,
) -> None:
    (tmp_path / "package.json").write_text('{"type":"module"}\n', encoding="utf-8")
    (tmp_path / "src").mkdir()
    (tmp_path / "tests").mkdir()
    (tmp_path / "src" / "ember.js").write_text(
