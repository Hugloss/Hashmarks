...
def test_agent_suite_aggregates_multiple_repositories(tmp_path: Path) -> None:
    corpus_script = _load_script("metrics_agent_corpus", Path("scripts/agent_evaluation/metrics_agent_corpus.py"))
    suite = _load_script("metrics_agent_suite", Path("scripts/agent_evaluation/metrics_agent_suite.py"))
    repos = []
    for index in range(2):
        workspace = tmp_path / f"repo{index}"
        workspace.mkdir()
        (workspace / "service.py").write_text("def locate_me():\n    return 1\n", encoding="utf-8")
...
def test_agent_suite_rejects_nested_repository_roots(tmp_path: Path) -> None:
    _load_script("metrics_agent_corpus", Path("scripts/agent_evaluation/metrics_agent_corpus.py"))
    suite = _load_script("metrics_agent_suite", Path("scripts/agent_evaluation/metrics_agent_suite.py"))
...
def test_suite_protocol_identity_binds_corpus_workspace_and_parameters(tmp_path: Path, monkeypatch) -> None:
    ...
    report = {
        ...
        "parameters": {"tasks": 1},
        "summary": {
            "file_recall": 1.0, "symbol_recall": 1.0, "first_query_hit_rate": 1.0,
            "fallback_search_rate": 0.0, "average_context_tokens": 1.0,
            "average_candidate_file_reduction": 1.0, "average_find_ms": 1.0,
            "average_context_ms": 1.0, "p95_find_ms": 1.0, "p95_context_ms": 1.0,
            "selected_file_tokens_avoided": 1, "seconds": 1.0,
        },
    ...
    assert first["benchmark_protocol"]["repositories"][0]["corpus_sha256"].startswith("sha256:")
    changed = module.collect_suite([("repo", workspace, corpus)], budget=1300, limit=20)
    assert changed["benchmark_protocol_identity"] != first["benchmark_protocol_identity"]


def test_fresh_multi_repo_fixture_is_reproducible_and_disjoint(tmp_path: Path) -> None:
    _load_script("metrics_agent_corpus", Path("scripts/agent_evaluation/metrics_agent_corpus.py"))
    _load_script("metrics_agent_suite", Path("scripts/agent_evaluation/metrics_agent_suite.py"))
    fresh = _load_script("metrics_fresh_multi_repo", Path("scripts/agent_evaluation/metrics_fresh_multi_repo.py"))
    ...
    assert len({row["workspace_tree_identity"] for row in first["fixture"]["repositories"]}) == 3


def test_fresh_multi_repo_fixture_has_perfect_localization_floor(tmp_path: Path) -> None:
    _load_script("metrics_agent_corpus", Path("scripts/agent_evaluation/metrics_agent_corpus.py"))
    _load_script("metrics_agent_suite", Path("scripts/agent_evaluation/metrics_agent_suite.py"))
    fresh = _load_script("metrics_fresh_multi_repo_floor", Path("scripts/agent_evaluation/metrics_fresh_multi_repo.py"))
...
def test_blind_worker_input_never_contains_expected_answers(tmp_path: Path) -> None:
    blind = _load_script("metrics_blind_worker_ab", Path("scripts/agent_evaluation/metrics_blind_worker_ab.py"))
...
@pytest.mark.scale
def test_blind_worker_ab_is_reproducible_and_scores_both_workers(tmp_path: Path) -> None:
    blind = _load_script("metrics_blind_worker_ab_run", Path("scripts/agent_evaluation/metrics_blind_worker_ab.py"))
    ...
    assert first["summary"]["entry_points_top1_rate"] >= first["summary"]["hashmarks_top1_rate"]
...
def test_worker_behavior_ab_is_answer_blind_and_reproducible(tmp_path: Path) -> None:
    first = _run_benchmark_script("scripts/agent_evaluation/metrics_worker_behavior_ab.py", tmp_path / "first")
    ...
    assert first["protocol"]["worker_input_fields"] == ["id", "query"]
    assert first["protocol"]["hidden_fields"] == ["expected_files", "expected_symbols"]
    assert first["summary"]["direct_unsafe_wrong_first_edits"] >= first["summary"]["gated_unsafe_wrong_first_edits"]
...
def test_worker_behavior_worker_rejects_hidden_fields(tmp_path: Path) -> None:
    ...
    tasks.write_text(
        json.dumps({
            ...
            "tasks": [{"id": "x", "query": "find config", "expected_files": ["secret"]}],
        }),
        encoding="utf-8",
    )
...
@pytest.mark.scale
def test_worker_inspection_ab_recovers_ambiguity_without_hidden_answers(tmp_path: Path) -> None:
    first = _run_benchmark_script("scripts/agent_evaluation/metrics_worker_inspection_ab.py", tmp_path / "first")
...
def test_worker_inspection_worker_rejects_hidden_fields(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="only id and query"):
        run_worker(policy="inspect-then-resolve", workspace=tmp_path, tasks_path=tasks, output=tmp_path / "out.json", limit=20)


@pytest.mark.scale
def test_worker_multistep_ab_improves_edit_safety_and_preserves_verification(tmp_path: Path) -> None:
    first = _run_benchmark_script("scripts/agent_evaluation/metrics_worker_multistep_ab.py", tmp_path / "first")
    ...
    assert first["summary"]["guided_correct_edits"] >= first["summary"]["direct_correct_edits"]
    ...
    assert first["summary"]["guided_evidence_approx_tokens"] >= first["summary"]["direct_evidence_approx_tokens"]
...
def test_worker_multistep_worker_rejects_hidden_fields(tmp_path: Path) -> None:
    ...
    tasks.write_text(json.dumps({
        "schema": "hashmarks.blind-worker-tasks.v1",
        "tasks": [{"id": "x", "query": "find config", "expected_verification": "tests/secret.py"}],
    }), encoding="utf-8")
    with pytest.raises(ValueError, match="only id and query"):
        run_worker(policy="uncertainty-aware-sequence", workspace=tmp_path, tasks_path=tasks, output=tmp_path / "out.json", limit=20)


@pytest.mark.scale
def test_worker_failed_verification_ab_recovers_without_hidden_answers(tmp_path: Path) -> None:
    ...
    assert first["summary"]["fresh_correct_recoveries"] > first["summary"]["repeat_correct_recoveries"]
...
def test_worker_failed_verification_worker_rejects_hidden_fields(tmp_path: Path) -> None:
    from scripts.agent_evaluation.metrics_worker_failed_verification_ab import run_worker

    tasks = tmp_path / "tasks.json"
    tasks.write_text(json.dumps({
        "schema": "hashmarks.worker-failed-verification-input.v1",
        "tasks": [{
            "id": "x", "query": "fix config", "failed_edit_target": "wrong.py",
            "verification_target": "tests/test_x.py", "verification_outcome": "failed",
            "expected_files": ["secret.py"],
        }],
    }), encoding="utf-8")
    with pytest.raises(ValueError, match="hidden or unsupported fields"):
        run_worker(policy="evidence-guided-recovery", workspace=tmp_path, tasks_path=tasks, output=tmp_path / "out.json", limit=20)
...
def test_fielded_bm25_economics_is_additive_and_answer_blind(tmp_path: Path) -> None:
    ...
    assert value['summary']['bm25_correct_first'] < value['summary']['current_hashmarks_correct_first']
    assert value['summary']['candidate_fused_correct_first'] < value['summary']['current_hashmarks_correct_first']
...
def test_retrieval_residuals_reject_new_recall_indexes_for_ambiguity_only_misses(tmp_path: Path) -> None:
...
def test_codex_agent_economics_exec_contract_with_fake_codex(tmp_path: Path) -> None:
    ...
    fake.chmod(0o755)
    ws=tmp_path/'repo'; ws.mkdir(); (ws/'src').mkdir(); (ws/'src/example.py').write_text('x=1\n'); (ws/'tests').mkdir(); (ws/'tests/test_example.py').write_text('def test_x(): pass\n')
    task={'id':'t1','query':'fix example'}
    value=_run_one(str(fake),'native',task,ws,tmp_path/'run',model='fake-model',effort='low',sandbox='read-only',bridge=Path('unused'),timeout_s=30)
...
def test_codex_selective_scout_rejects_path_outside_worker_visible_candidates(tmp_path: Path) -> None:
    ...
    fake.chmod(0o755); ws=tmp_path/'repo'; ws.mkdir(); (ws/'a.py').write_text('x=1\n')
    pkt={'ambiguity':{'alternatives':[{'role':'implementation','path':'a.py','canonical_rank':1}]}}
    result=_run_scout(str(fake),{'id':'x','query':'fix x'},ws,pkt,tmp_path/'run',model=None,effort=None,sandbox='read-only',timeout_s=30)
...
def test_codex_selective_scout_prompt_is_answer_blind() -> None:
    ...
    pkt={'ambiguity':{'alternatives':[{'role':'authority','path':'AGENTS.md','canonical_rank':4},{'role':'verification','path':'tests/test_x.py','canonical_rank':1}]}}
    prompt=_scout_prompt({'id':'x','query':'find authority'},Path('/tmp/repo'),pkt)
...
def test_codex_rollout_usage_recovers_thread_and_token_counts(tmp_path: Path) -> None:
    ...
    events=tmp_path/'events.jsonl'; events.write_text(json.dumps({'type':'thread.started','thread_id':'thread-123'})+'\n')
    sessions=tmp_path/'sessions'; sessions.mkdir(); rollout=sessions/'rollout-thread-123.jsonl'
    rollout.write_text(json.dumps({'event_msg':{'payload':{'type':'token_count','input_tokens':120,'cached_input_tokens':80,'output_tokens':30,'reasoning_output_tokens':10,'total_tokens':150}}})+'\n')
...
def test_codex_economics_matrix_plan_is_fail_closed_without_codex(tmp_path: Path) -> None:
    from scripts.agent_evaluation.codex_economics_matrix import parse_variant,plan
    variants=[parse_variant('cheap-native:cheap-model:low:native'),parse_variant('cheap-hm:cheap-model:low:hashmarks'),parse_variant('cheap-scout:cheap-model:low:selective-real'),parse_variant('strong-native:strong-model:medium:native')]
