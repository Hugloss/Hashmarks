...
def test_publish_workflow_uses_trusted_publishing_and_exact_artifact_handoff() -> None:
    text = (_root() / ".github" / "workflows" / "publish.yml").read_text(encoding="utf-8")
...
def test_every_ci_and_publish_job_has_a_bounded_timeout() -> None:
    for name in ("ci.yml", "publish.yml"):
        ...
        assert text.count("    runs-on: ubuntu-latest") == text.count("    timeout-minutes:")


def test_ci_checkout_does_not_persist_git_credentials() -> None:
    lines = (_root() / ".github" / "workflows" / "ci.yml").read_text(encoding="utf-8").splitlines()
    checkout_indexes = [
    ...
    for index in checkout_indexes:
        assert any("persist-credentials: false" in line for line in lines[index + 1:index + 5])
