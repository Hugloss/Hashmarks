...
def test_lane_summary_does_not_fake_missing_token_metrics() -> None:
    value = summarize_lane(
        "unknown", [{"verified_solution": True, "model_tokens": None, "wall_ms": 10.0}]
    )
...
def test_pareto_requires_real_token_metrics() -> None:
    ...
    assert pareto_dominance(lanes) == [
        {"dominant": "cheap-hm", "dominated": "strong-native"}
    ]
