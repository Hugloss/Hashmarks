...
def test_benchmark_membership_variant_is_copy_only_and_preserves_source_owner(
    tmp_path: Path,
) -> None:
    ...
    (workspace / "docs" / "guide.md").write_text(
        "\n".join(
            f"WorkspaceLeaseAuthority operational guide line {i}" for i in range(50)
        )
        + "\n",
        encoding="utf-8",
    )
    ...
    changed = apply_variant(
        variant_state / "codemap.sqlite3", "membership", surfaces={"docs"}
    )
...
def test_benchmark_scores_expected_discrimination_for_underspecified_tasks(
    tmp_path: Path,
) -> None:
    workspace = tmp_path / "repo"
    workspace.mkdir()
    ...
    with CodeMap(
        ...
        result = evaluate(
            ...
            [
                {
                    "id": "ambiguous",
                    "task": "Change observer behavior.",
                    "expected_discrimination": True,
                }
            ],
        )
...
def test_decision_packet_contract_rejects_wrong_shape(tmp_path: Path) -> None:
    ...
    try:
        DecisionPacketContract.parse(
            {
                "schema": "hashmarks.task-decision-packet.v2",
                "action": {"edit": {"path": "a.py"}},
            }
        )
...
def test_calibration_manifest_is_deterministic_and_groups_tasks() -> None:
    ...
    first = build_manifest(
        corpus, repository_identity="repo-1", generation=7, variant="full", group_size=8
    )
...
def test_decision_session_reuses_evidence_primitives_without_caching_final_packet(
    tmp_path: Path,
) -> None:
    ...
    (workspace / "test_owner.py").write_text(
        "from owner import LeaseOwner\ndef test_issue(): assert LeaseOwner().issue() == 1\n",
        encoding="utf-8",
    )
    with CodeMap(
        workspace,
        state_dir=tmp_path / "state-session",
        artifact_db=tmp_path / "artifacts-session.sqlite3",
    ) as codemap:
        codemap.sync()
        with codemap.decision_session():
            first = codemap.task_decision_packet("Change LeaseOwner issue behavior.")
            second = codemap.task_decision_packet(
                "Add regression coverage for LeaseOwner issue behavior."
            )
            stats = codemap.decision_session_stats()
        assert first["task"] != second["task"]
        assert (
            first["identity"]["codemap_generation"]
            == second["identity"]["codemap_generation"]
        )
...
def test_calibration_aggregate_requires_every_matching_group() -> None:
    ...
    manifest = build_manifest(
        corpus, repository_identity="repo", generation=3, variant="full", group_size=4
    )
    groups = [
        {
...
def test_decision_scale_ladder_reports_per_task_cost(tmp_path: Path) -> None:
    ...
    workspace.mkdir()
    for i in range(12):
        (workspace / f"owner_{i}.py").write_text(
            f"def unique_owner_{i}():\n    return {i}\n", encoding="utf-8"
        )
    corpus = [
        {
            "id": f"t{i}",
            "task": f"Change unique_owner_{i} behavior.",
            "expected_edit": [f"owner_{i}.py"],
        }
        for i in range(12)
    ]
    with CodeMap(
        workspace,
        state_dir=tmp_path / "state-scale",
        artifact_db=tmp_path / "artifacts-scale.sqlite3",
    ) as codemap:
        codemap.sync()
...
def test_scale_point_aggregates_completed_group_evidence() -> None:
    from benchmarks.codemap_surface_calibration import (
        build_manifest,
        scale_point_from_group_results,
    )

    corpus = [{"id": f"t{i}", "task": f"Task {i}."} for i in range(16)]
    manifest = build_manifest(
        corpus, repository_identity="repo", generation=4, variant="full", group_size=8
    )
    groups = [
        {
            "manifest_sha256": manifest["manifest_sha256"],
            ...
            "decision": {
                "tasks": 8,
                "false_safe": 0,
                "decision_seconds": {"total": 5.0, "p95": 1.0},
            },
        },
        {
            "manifest_sha256": manifest["manifest_sha256"],
            ...
            "decision": {
                "tasks": 8,
                "false_safe": 1,
                "decision_seconds": {"total": 7.0, "p95": 2.0},
            },
        },
    ]
...
def test_weak_generic_contract_anchor_fails_closed_but_identifier_anchor_resolves(
    tmp_path: Path,
) -> None:
    ...
    with CodeMap(
        workspace,
        state_dir=tmp_path / "state-weak-anchor",
        artifact_db=tmp_path / "artifacts-weak-anchor.sqlite3",
    ) as codemap:
        codemap.sync()
        generic = codemap.task_action_map(
            "Change the observer behavior while preserving its existing public contract."
        )
        anchored = codemap.task_action_map(
            "Change the project_observability_receipt behavior while preserving its existing public contract."
        )
...
def test_resumable_calibration_reuses_exact_task_receipts_after_interruption(
    tmp_path: Path,
) -> None:
    from benchmarks.codemap_surface_calibration import (
        build_manifest,
    ...
    corpus = [
        {
            "id": f"t{index}",
            "task": f"Change unique_resume_owner_{index} behavior.",
            "expected_edit": [f"owner_{index}.py"],
        }
        for index in range(3)
    ]
    ...
    with CodeMap(workspace, state_dir=state, artifact_db=artifacts) as codemap:
        ...
        partial = evaluate_resumable(
            codemap,
            corpus[:2],
            receipt_dir=receipts,
            manifest_sha256=manifest["manifest_sha256"],
            group_index=0,
            expected_task_ids=manifest["groups"][0],
        )
...
def test_resumable_calibration_rejects_receipt_identity_drift(tmp_path: Path) -> None:
    ...
    corpus = [
        {
            "id": "t0",
            "task": "Change receipt_owner behavior.",
