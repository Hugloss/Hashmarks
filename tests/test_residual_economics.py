...
def test_residual_report_composes_reuse_candidates_scale_and_impact(
    tmp_path: Path,
) -> None:
    _repo(tmp_path)
...
def test_residual_report_can_include_qualification_classification(
    tmp_path: Path,
) -> None:
    _repo(tmp_path)
    ...
    assert qualification["ordinary_correctness_members_avoided"] == (
        qualification["process_sensitive_members"]
        + qualification["empirical_benchmark_members"]
    )
