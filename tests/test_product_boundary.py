...
def test_agents_file_declares_repository_intelligence_boundary() -> None:
    ...
    assert "Hashmarks is not an agentic orchestrator and must not become another ChatGPT/Codex/OpenCode clone." in text
    assert "Hashmarks owns **repository intelligence**" in text
    assert "External coding agents and their harnesses own the **solution loop**" in text
...
def test_product_profile_explicitly_rejects_agent_and_execution_motor_drift() -> None:
    ...
    assert "Hashmarks must never be turned into either the coding agent's solution loop or Oh-Goon's execution/certification motor." in agents
    ...
    assert "PB6. Hashmarks must never become the agent or the execution motor." in invariants
...
def test_external_library_findings_do_not_expand_repository_analysis_scope() -> None:
    ...
    invariants = _text("docs/reference/INVARIANTS.md")
    history = _text("docs/development/reviews/HM297_EXTERNAL_LIBRARY_CHARACTERIZATION_LESSON.md")

    for text in (boundary, agents, contributing, history):
        assert "Do not chase remaining findings in external libraries." in text

    assert "An import is an evidence edge, not permission to recursively investigate the imported library." in boundary
    assert "not active roadmap items" in agents
    assert "external dependency edge" in architecture
    assert "PB8. Repository ownership does not transit through dependencies." in invariants
    assert "PB9. External-library characterization is qualification evidence, not backlog." in invariants
    assert "Package-name suppression tables" in invariants
    assert "characterization evidence, not active roadmap work" in _text("docs/README.md")
...
def test_evidence_authority_precedence_is_explicit_and_non_strengthening() -> None:
    ...
    assert "PB10. Derived evidence may never silently strengthen its authority source." in invariants
    assert "Authority precedence is non-strengthening, not a global score" in architecture
    assert "consumer/model interpretation stays outside repository authority" in architecture
...
def test_architecture_invariant_and_readme_repeat_boundary() -> None:
    ...
    assert "G25. Hashmarks is repository intelligence, not the coding-agent solution loop." in invariants
    assert "Agent-facing features that would transfer those authorities into Hashmarks are architecture regressions." in invariants
    assert "Non-negotiable agent boundary" in readme
    assert "`AGENTS.md` is the contributor/agent-facing guardrail for this boundary." in readme
    assert "G29. Post-change evidence delta is incremental invalidation, not autonomous recovery." in invariants
    assert "compact post-change evidence deltas over caller-reported changed paths" in _text("AGENTS.md")
    assert "G33. Declared cross-repository impact is provenance-bearing evidence, never coordination." in invariants
    assert "G34. Dynamic Python module loading is repository ownership evidence, not runtime authority." in invariants
...
def test_repository_carries_agent_boundary_instructions_without_shipping_them_in_sdist() -> None:
...
def test_agents_file_freezes_hashmarks_oh_goon_execution_boundary() -> None:
    text = _text("AGENTS.md")
    assert "Hashmarks defines immutable repository evidence and selection authority; Oh-Goon owns execution and certification authority." in text
...
def test_prepublic_boundary_debt_record_supersedes_historical_review_labels() -> None:
    ...
    assert "ActionCache` / `ExecutionCache` / `ActionResult` | REVIEW LATER" not in review
...
def test_repository_root_has_no_historical_phase_evidence() -> None:
    ...
    historical = sorted(
        ...
        if path.is_file() and path.name.startswith("HM") and path.suffix in {".json", ".txt", ".md"}
    )
    assert historical == []
