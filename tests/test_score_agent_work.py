...
def test_work_score_distinguishes_verified_solution_from_selected_test(tmp_path: Path) -> None:
    secret = tmp_path / "secret.json"
    secret.write_text(json.dumps({"tasks": [{"id": "t", "expected_files": ["src/a.py"]}]}))
    selected = tmp_path / "selected.json"
    selected.write_text(json.dumps({
        "schema": M.TRACE_SCHEMA, "task_id": "t", "worker_kind": "process-worker",
        "events": [
            {"sequence": 1, "kind": "edit_attempt", "path": "src/a.py", "started_at_ns": 10, "finished_at_ns": 11},
            {"sequence": 2, "kind": "verification", "path": "tests/test_a.py", "edit_target": "src/a.py", "outcome": "not-run", "started_at_ns": 12, "finished_at_ns": 13},
        ],
    }))
    verified = tmp_path / "verified.json"
    verified.write_text(json.dumps({
        "schema": M.TRACE_SCHEMA, "task_id": "t", "worker_kind": "codex",
        "events": [
            {"sequence": 1, "kind": "edit_attempt", "path": "src/a.py", "started_at_ns": 10, "finished_at_ns": 11},
            {"sequence": 2, "kind": "verification", "path": "tests/test_a.py", "edit_target": "src/a.py", "outcome": "passed", "started_at_ns": 12, "finished_at_ns": 20},
        ],
    }))
...
def test_work_score_penalizes_thrashing_and_duplicate_reads(tmp_path: Path) -> None:
    trace = {
        ...
        "events": [
            ...
            {"sequence": 4, "kind": "verification", "edit_target": "src/wrong.py", "outcome": "failed"},
            {"sequence": 5, "kind": "edit_attempt", "path": "src/wrong.py"},
            {"sequence": 6, "kind": "edit_attempt", "path": "src/right.py"},
            {"sequence": 7, "kind": "verification", "edit_target": "src/right.py", "outcome": "passed"},
        ],
    }
