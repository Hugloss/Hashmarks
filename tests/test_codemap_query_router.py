...
def test_query_router_classifies_strong_intents() -> None:
    ...
    assert (
        route_query("callers references AuthService").intent is QueryIntent.RELATIONSHIP
    )
    ...
    assert (
        route_query("architecture ownership lifecycle").intent is QueryIntent.CONCEPTUAL
    )
...
def test_query_router_exposes_intent_scoped_repository_domains() -> None:
    route = route_query(
        "who owns release packaging and which make command enforces the contract"
    )
...
def test_repository_control_surface_domains_are_not_global_boosts(
    tmp_path: Path,
) -> None:
    (tmp_path / "AGENTS.md").write_text(
        "Ownership: release belongs to ReleaseAuthority.\n", encoding="utf-8"
    )
    (tmp_path / "Makefile").write_text(
        "release:\n\tpython scripts/release.sh\n", encoding="utf-8"
    )
    (tmp_path / "ARCHITECTURE.md").write_text(
        "ReleaseAuthority owns release lifecycle boundaries.\n", encoding="utf-8"
    )
    ...
    with CodeMap(tmp_path) as codemap:
        codemap.sync()
        ownership = codemap.find(
            "who owns release authority and build contract", limit=8
        )
...
def test_task_query_formulation_is_candidate_visible_and_governance_aware(
    tmp_path: Path,
) -> None:
    (tmp_path / "AGENTS.md").write_text(
        "Ownership authority architecture contract.\n", encoding="utf-8"
    )
    (tmp_path / "backend").mkdir()
    (tmp_path / "backend" / "README.md").write_text(
        "Backend API ownership boundary.\n", encoding="utf-8"
    )
    (tmp_path / "backend" / "service.py").write_text(
        "class WidgetRoute:\n    pass\n", encoding="utf-8"
    )
...
def test_task_query_views_add_generic_evidence_families_without_project_filenames(
    tmp_path: Path,
) -> None:
    (tmp_path / "PRIVACY.md").write_text(
        "Security privacy redaction exposure rules.\n", encoding="utf-8"
    )
    (tmp_path / "CI_CD_CONTRACT.md").write_text(
        "CI CD release delivery automation contract.\n", encoding="utf-8"
    )
    (tmp_path / "service.py").write_text(
...
def test_task_query_evidence_family_does_not_change_exact_identifier_leader(
    tmp_path: Path,
) -> None:
    (tmp_path / "README.md").write_text(
        "Release privacy security architecture.\n", encoding="utf-8"
    )
    (tmp_path / "service.py").write_text(
        "class ReleaseAuthority:\n    pass\n", encoding="utf-8"
    )
...
def test_find_task_deduplicates_identical_query_views(
...
def test_find_task_preserves_scoped_agents_for_ambiguous_localized_task(
    tmp_path: Path, monkeypatch
) -> None:
    ...
    with CodeMap(tmp_path, state_dir=tmp_path / ".state") as codemap:
        ...
        base = tuple(
            SearchHit(
                path=f"backend/src/pkg/item_{index}.py",
                score=100.0 - index,
                kind="file",
            )
...
def test_find_task_does_not_override_high_confidence_relationship_route(
    tmp_path: Path, monkeypatch
) -> None:
    from hashmarks.codemap.model import SearchHit
...
def test_find_task_preserves_deep_scoped_readme_for_conceptual_localization(
    tmp_path: Path, monkeypatch
) -> None:
    from hashmarks.codemap.model import SearchHit
    ...
    with CodeMap(tmp_path, state_dir=tmp_path / ".state") as codemap:
        ...
        base = tuple(
            [
                SearchHit(
                    path="backend/src/pkg/runtime/a.py", score=100.0, kind="file"
                ),
                SearchHit(path="backend/src/pkg/runtime/b.py", score=99.0, kind="file"),
            ]
            + [
                SearchHit(
                    path=f"tests/item_{index}.py", score=98.0 - index, kind="file"
                )
        ...
        governance = (
            ...
            + (
                SearchHit(
                    path="backend/src/pkg/runtime/README.md", score=82.0, kind="file"
                ),
            )
            + tuple(base[17:])
        )
...
def test_find_task_does_not_preserve_scoped_readme_without_two_localized_results(
    tmp_path: Path, monkeypatch
) -> None:
    from hashmarks.codemap.model import SearchHit
    ...
    with CodeMap(tmp_path, state_dir=tmp_path / ".state") as codemap:
        ...
        base = tuple(
            ...
            + [
                SearchHit(
                    path=f"tests/item_{index}.py", score=99.0 - index, kind="file"
                )
...
def test_find_task_does_not_preserve_scoped_readme_for_relationship_route(
    tmp_path: Path, monkeypatch
) -> None:
    from hashmarks.codemap.model import SearchHit
...
def test_task_graph_adjacency_is_additive_bounded_and_provenanced(
    tmp_path: Path,
) -> None:
    pkg = tmp_path / "pkg"
    pkg.mkdir()
    ...
    assert [row["path"] for row in adjacency["primary"]] == [
...
def test_task_graph_adjacency_never_rewrites_canonical_find_task(
    tmp_path: Path,
) -> None:
...
def test_scoped_authority_resolves_root_nested_and_override_without_ranking_effect(
    tmp_path: Path,
):
    ...
    assert [row["path"] for row in value["authority"]] == [
        ...
        "backend/pkg/AGENTS.override.md",
    ]
...
def test_scoped_authority_does_not_treat_readme_as_authority(tmp_path: Path):
    ...
    (area / "README.md").write_text(
        "architecture ownership words but not authority\n", encoding="utf-8"
    )
...
def test_explain_task_retrieval_reports_rrf_provenance_without_changing_results(
    tmp_path,
):
    root = tmp_path / "repo"
    root.mkdir()
    ...
    (root / "test_alpha.py").write_text(
        "from alpha import alpha_service\n", encoding="utf-8"
    )
    ...
    assert [row["path"] for row in explained["selected"]] == [
        hit.path for hit in before
    ]
    assert explained["selected"][0]["discovered_by"]
    assert {
        item["lane"] for row in explained["selected"] for item in row["discovered_by"]
    } <= {"base", "governance", "evidence"}
...
def test_find_task_reuses_generation_bound_result_without_replaying_views(
    tmp_path: Path, monkeypatch
) -> None:
...
def test_find_task_cache_is_invalidated_by_codemap_generation(
    tmp_path: Path, monkeypatch
) -> None:
...
def test_task_context_plan_is_bounded_and_route_sensitive(tmp_path: Path) -> None:
    ...
    with CodeMap(tmp_path, state_dir=tmp_path / ".state") as codemap:
        ...
        conceptual = codemap.task_context_plan(
            "review ownership authority architecture contract"
        )
...
def test_task_context_explicit_budget_overrides_adaptive_plan(tmp_path: Path) -> None:
    ...
    with CodeMap(tmp_path, state_dir=tmp_path / ".state") as codemap:
        codemap.sync()
        pack = codemap.task_context(
            "AuthService", token_budget=600, disclosure="outline"
        )
        assert pack.budget == 600
        assert pack.disclosure.value == "outline"


def test_task_relationships_composes_typed_layers_without_changing_primary(
    tmp_path: Path,
) -> None:
    (tmp_path / "AGENTS.md").write_text("root authority\n", encoding="utf-8")
    src = tmp_path / "src"
    ...
    assert any(
        row["layer"] == "authority" and row["relation"] == "governs"
...
def test_retrieval_stability_replays_without_changing_canonical_result(
    tmp_path: Path,
) -> None:
...
def test_task_entry_points_projects_roles_without_changing_find_task(
    tmp_path: Path,
) -> None:
    ...
    (docs / "notes.md").write_text(
        "package configuration historical notes\n", encoding="utf-8"
    )
    with CodeMap(tmp_path, state_dir=tmp_path / ".state") as codemap:
        codemap.sync()
        before = [
            hit.path for hit in codemap.find_task("package configuration", limit=20)
        ]
...
def test_task_entry_points_prefers_scoped_authority_for_authority_task(
    tmp_path: Path,
) -> None:
...
def test_task_entry_points_reports_test_authority_ambiguity_without_ranking_change(
    tmp_path: Path,
) -> None:
...
def test_task_entry_points_never_projects_test_source_as_implementation(
    tmp_path: Path,
) -> None:
    src = tmp_path / "src"
    src.mkdir()
    ...
    assert all(
        not str(row["path"]).startswith("tests/")
        for row in value["roles"]["implementation"]
    )
    assert value["recommended"][0]["role"] == "implementation"
    assert value["recommended"][0]["path"] == "src/service.py"


def test_task_entry_points_prefers_stronger_same_role_symbol_evidence(
    tmp_path: Path,
) -> None:
    src = tmp_path / "src"
    src.mkdir()
    ...
    with CodeMap(tmp_path, state_dir=tmp_path / ".state") as codemap:
        ...
        canonical = [
            hit.path
            for hit in codemap.find_task("OrderStore save_order persistence", limit=20)
        ]
    ...
    assert (
        value["roles"]["implementation"][0]["role_symbol_specificity"]
        > value["roles"]["implementation"][1]["role_symbol_specificity"]
    )
...
def test_task_entry_points_reports_vite_test_config_ambiguity(tmp_path: Path) -> None:
    ...
    tests = tmp_path / "tests"
    tests.mkdir()
    (tests / "panel.test.ts").write_text(
        "export function panelTest() { return true }\n", encoding="utf-8"
    )
    with CodeMap(tmp_path, state_dir=tmp_path / ".state") as codemap:
        codemap.sync()
...
def test_task_entry_points_distinctive_test_identifier_resolves_role_ambiguity(
    tmp_path: Path,
) -> None:
    (tmp_path / "contracts").mkdir()
    (tmp_path / "tests").mkdir()
    (tmp_path / "contracts" / "release.schema.json").write_text(
        '{"title":"release checksum contract"}\n', encoding="utf-8"
    )
    ...
    assert (
        "test_release_checksum_contract"
        in ambiguity["resolution"]["task_anchor_tokens"]
    )
    assert ambiguity["secret_knowledge_used"] is False


def test_task_entry_points_generic_implementation_test_stays_ambiguous(
    tmp_path: Path,
) -> None:
    (tmp_path / "tests").mkdir()
    (tmp_path / "src").mkdir()
