...
def test_overlay_indexes_only_changed_paths_and_reuses_base_artifacts(tmp_path: Path) -> None:
    base_dir = tmp_path / "base"; worktree = tmp_path / "worker-a"
    _repo(base_dir); shutil.copytree(base_dir, worktree)
...
def test_overlays_are_worker_isolated(tmp_path: Path) -> None:
    base_dir = tmp_path / "base"; a = tmp_path / "a"; b = tmp_path / "b"
    _repo(base_dir); shutil.copytree(base_dir, a); shutil.copytree(base_dir, b)
    (a / "src" / "engine.py").write_text("def alpha_only():\n    return 1\n")
    (b / "src" / "engine.py").write_text("def beta_only():\n    return 1\n")
    with CodeMap(base_dir) as base:
        base.sync()
        with WorktreeOverlay(base, a, worker_id="agent-a") as oa, WorktreeOverlay(base, b, worker_id="agent-b") as ob:
            oa.sync(["src/engine.py"]); ob.sync(["src/engine.py"])
