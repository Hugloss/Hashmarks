...
def _manifest() -> dict[str, object]:
    return json.loads((ROOT / 'scripts/repository_evaluation/retained_measurement_authority.json').read_text())
...
def test_like_fallback_keeps_recall_and_now_has_explicit_stable_order() -> None:
    ...
    broad=text[text.index('def search_candidates'):text.index('def exact_symbol_candidates')]
    assert 'WHERE lower(s.name) LIKE ? OR lower(s.qualname) LIKE ? OR lower(s.signature) LIKE ? OR lower(s.path) LIKE ?' in broad
...
def test_controller_cutoff_has_no_product_failure_authority() -> None:
    evaluation='\n'.join(p.read_text(encoding='utf-8') for p in (ROOT/'scripts/repository_evaluation').glob('*.py'))
...
def test_resume_receipts_suppress_timing_authority() -> None:
    text=(ROOT/'scripts/repository_evaluation/run_cases.py').read_text(encoding='utf-8')
    assert '"timing_comparable": reused == 0' in text
    merge=(ROOT/'scripts/repository_evaluation/merge_runs.py').read_text(encoding='utf-8')
    assert '"timing_comparable": all(bool(run.get("timing_comparable")) for run in runs)' in merge
