...
def test_decision_brief_budget_sweep_finds_smallest_safe_agent_budget(
    tmp_path: Path,
) -> None:
    _repo(tmp_path)
...
def test_unsafe_budget_exposes_missing_roles_instead_of_false_qualification(
    tmp_path: Path,
) -> None:
    _repo(tmp_path)
    with CodeMap(tmp_path) as codemap:
        codemap.sync()
        sweep = codemap.task_decision_brief_budget_sweep(
            "widget implementation test", budgets=(1, 512)
        )
