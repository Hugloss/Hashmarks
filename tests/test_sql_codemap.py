...
def test_sql_is_indexed_as_editable_source_evidence(tmp_path: Path) -> None:
    ...
    (tmp_path / 'queries' / 'ember_731.sql').write_text("SELECT 'old-ember-731' AS label;\n", encoding='utf-8')
    (tmp_path / 'tests').mkdir()
    (tmp_path / 'tests' / 'test_ember_731.py').write_text("def test_ember_731():\n    pass\n", encoding='utf-8')
...
def test_lexical_file_candidates_count_distinct_terms_not_occurrences(tmp_path: Path) -> None:
