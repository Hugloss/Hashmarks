...
def _repo(root: Path) -> None:
    ...
    (root / "tests").mkdir()
    (root / "src" / "owner.py").write_text("def widget(value: str) -> str:\n    return value + '-old'\n")
    (root / "src" / "route.py").write_text("from .owner import widget\n\ndef run(value: str) -> str:\n    return widget(value)\n")
...
def test_profiles_share_one_snapshot_identity_and_are_deterministic(tmp_path: Path) -> None:
    _repo(tmp_path)
    with CodeMap(tmp_path) as codemap:
        codemap.sync()
        profiles = {
            name: codemap.repository_intelligence_profile(
                "Fix widget accepted response behavior", ["src/owner.py"], profile=name,
            )
            for name in ("compact", "standard", "audit")
        }
        repeated = codemap.repository_intelligence_profile(
            "Fix widget accepted response behavior", ["src/owner.py"], profile="compact",
        )

    identities = {row["source_snapshot_identity"] for row in profiles.values()}
    assert len(identities) == 1
    assert profiles["compact"] == repeated
    assert profiles["compact"]["profile_identity"] != profiles["standard"]["profile_identity"]
    assert profiles["standard"]["profile_identity"] != profiles["audit"]["profile_identity"]
...
def test_profile_density_contract_preserves_authoritative_facts(tmp_path: Path) -> None:
    ...
    with CodeMap(tmp_path) as codemap:
        ...
        standard = codemap.repository_intelligence_profile(
            "Fix widget accepted response behavior", ["src/owner.py"], profile="standard",
        )
        audit = codemap.repository_intelligence_profile(
            "Fix widget accepted response behavior", ["src/owner.py"], profile="audit",
        )

    compact_evidence = compact["evidence"]
    standard_evidence = standard["evidence"]
    snapshot = audit["evidence"]["snapshot"]
    assert compact_evidence["verification"]["member"] == snapshot["verification"]["member"]
...
def test_audit_profile_contains_exact_snapshot(tmp_path: Path) -> None:
    ...
    with CodeMap(tmp_path) as codemap:
        ...
        snapshot = codemap.repository_intelligence_snapshot(
            "Fix widget accepted response behavior", ["src/owner.py"],
        )
    ...
    with CodeMap(tmp_path) as codemap:
        ...
        with pytest.raises(ValueError, match="profile must be one of"):
            codemap.repository_intelligence_profile(
                "Fix widget accepted response behavior", ["src/owner.py"], profile="tiny",
            )
...
def test_profile_service_roundtrip(tmp_path: Path) -> None:
    ...
    try:
        ...
        profile = client.repository_intelligence_query(
            "profile", "Fix widget accepted response behavior", ["src/owner.py"], profile="compact",
        )["result"]
