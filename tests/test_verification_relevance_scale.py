...
def _build_repetitive_verification_repo(root: Path, distractors: int) -> tuple[str, str]:
    ...
    for index in range(distractors):
        path = root / "tests" / "regression" / f"test_apply_policy_feature0042_regression_{index:04d}.py"
        path.write_text(
            "from packages.feature0042.service import apply_policy\n\n"
...
def test_verification_relevance_recovers_local_test_outside_canonical_top20(tmp_path: Path) -> None:
    ...
    assert relevance["selected"]["selection_reason"] == "unique-exact-reference-plus-namespace-locality"
...
def test_verification_relevance_preserves_canonical_choice_when_reference_evidence_ties(tmp_path: Path) -> None:
    (tmp_path / "src").mkdir()
...
def test_unique_indirect_active_verifier_overrides_stale_direct_legacy_decoys(tmp_path: Path) -> None:
    ...
    (tmp_path / "src/case/engine.py").write_text("def ember_snow(v): return v + '-active'\n")
    ...
    (tmp_path / "legacy/ember_snow.py").write_text("def ember_snow(v): return v + '-legacy'\n")
    ...
    assert action["verification_relevance"]["selection_reason"] == "unique-bounded-indirect-reference-plus-namespace-locality"
...
def test_reexported_active_symbol_keeps_exact_owner_identity(tmp_path: Path) -> None:
    ...
    for index in range(8):
        path = tmp_path / "tests/regression" / f"test_ember_snow_{index:02d}.py"
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text("from legacy.ember_snow import ember_snow\ndef test_old(): assert ember_snow('x') == 'x-legacy'\n")
    with CodeMap(tmp_path) as codemap:
        codemap.sync()
        action = codemap.task_action_map("Fix ember_snow accepted response implementation and verify behavior", limit=20)
    assert action["edit"]["path"] == "src/case/engine.py"
    assert action["verify"]["path"] == "tests/contract/test_public_contract.py"
...
def test_aliased_import_preserves_underlying_owner_identity(tmp_path: Path) -> None:
    ...
    with CodeMap(tmp_path) as codemap:
        codemap.sync()
        action = codemap.task_action_map("Fix ember_snow implementation and verify behavior", limit=20)
    assert action["edit"]["path"] == "src/case/engine.py"
...
def test_ambiguous_reexport_does_not_invent_qualified_owner(tmp_path: Path) -> None:
    ...
    (tmp_path / "src/case/engine_a.py").write_text("def ember_snow(v): return v + '-a'\n")
    (tmp_path / "src/case/engine_b.py").write_text("def ember_snow(v): return v + '-b'\n")
    (tmp_path / "tests/test_contract.py").write_text("from src.case import ember_snow\n")
    with CodeMap(tmp_path) as codemap:
        codemap.sync()
        resolved = codemap._resolve_import_owner_paths("tests/test_contract.py", "src.case.ember_snow")
    assert resolved == ["src/case/__init__.py"]


def test_two_hop_reexport_keeps_unique_underlying_owner_identity(tmp_path: Path) -> None:
    (tmp_path / "src/case").mkdir(parents=True)
    (tmp_path / "tests/contract").mkdir(parents=True)
    (tmp_path / "legacy").mkdir()
    (tmp_path / "src/__init__.py").write_text("")
...
def test_two_hop_alias_chain_keeps_unique_underlying_owner_identity(tmp_path: Path) -> None:
    (tmp_path / "src/case").mkdir(parents=True)
    (tmp_path / "tests/contract").mkdir(parents=True)
    (tmp_path / "legacy").mkdir()
    (tmp_path / "src/__init__.py").write_text("")
    (tmp_path / "src/case/__init__.py").write_text("from .public import active_ember as ember_snow\n")
    (tmp_path / "src/case/public.py").write_text("from .engine import ember_snow as active_ember\n")
    (tmp_path / "src/case/engine.py").write_text("def ember_snow(v): return v + '-active'\n")
    (tmp_path / "tests/contract/test_public_contract.py").write_text(
        "from src.case import ember_snow\ndef test_public_contract(): assert ember_snow('x') == 'x-active'\n"
    )
...
def test_same_symbol_across_multiple_live_packages_fails_closed_instead_of_archive_fallback(tmp_path: Path) -> None:
    (tmp_path / "src/case").mkdir(parents=True)
    ...
    for package, suffix in (("case", "active"), ("other", "other")):
        (tmp_path / f"src/{package}/__init__.py").write_text("from .engine import ember_snow\n")
        (tmp_path / f"src/{package}/engine.py").write_text(f"def ember_snow(v): return v + '-{suffix}'\n")
    ...
    with CodeMap(tmp_path) as codemap:
        codemap.sync()
        action = codemap.task_action_map("Fix ember_snow accepted response implementation and verify behavior", limit=20)
        packet = codemap.task_decision_packet("Fix ember_snow accepted response implementation and verify behavior", limit=20)
    assert action["ambiguity"]["ambiguous"] is True
    assert action["ambiguity"]["reason"] in {"multiple-live-owners-behind-archive-hit", "multiple-task-local-structural-owners"}
    assert packet["discrimination"]["needed"] is True


def test_reexport_chain_beyond_bound_fails_closed_instead_of_stale_verifier(tmp_path: Path) -> None:
    (tmp_path / "src/case").mkdir(parents=True)
    (tmp_path / "tests/contract").mkdir(parents=True)
    (tmp_path / "legacy").mkdir()
    (tmp_path / "src/__init__.py").write_text("")
    layers = [f"layer{i}" for i in range(8)]
    (tmp_path / "src/case/__init__.py").write_text(f"from .{layers[0]} import ember_snow\n")
    for index, layer in enumerate(layers):
        nxt = layers[index + 1] if index + 1 < len(layers) else "engine"
        (tmp_path / f"src/case/{layer}.py").write_text(f"from .{nxt} import ember_snow\n")
    (tmp_path / "src/case/engine.py").write_text("def ember_snow(v): return v + '-active'\n")
    (tmp_path / "tests/contract/test_public_contract.py").write_text(
        "from src.case import ember_snow\ndef test_public_contract(): assert ember_snow('x') == 'x-active'\n"
    )
...
def test_multiple_star_reexports_do_not_invent_unique_owner(tmp_path: Path) -> None:
    ...
    (tmp_path / "tests").mkdir()
    (tmp_path / "src/__init__.py").write_text("")
    (tmp_path / "src/case/__init__.py").write_text("from .engine_a import *\nfrom .engine_b import *\n")
    (tmp_path / "src/case/engine_a.py").write_text("def ember_snow(v): return v + '-a'\n")
    (tmp_path / "src/case/engine_b.py").write_text("def ember_snow(v): return v + '-b'\n")
    (tmp_path / "tests/test_contract.py").write_text("from src.case import ember_snow\n")
    with CodeMap(tmp_path) as codemap:
        codemap.sync()
        resolved, unresolved = codemap._resolve_import_owner_evidence("tests/test_contract.py", "src.case.ember_snow")
    assert resolved == ["src/case/__init__.py"]
    assert unresolved is True


def _verification_reference_case(tmp_path: Path, verifier_source: str) -> dict[str, object]:
    (tmp_path / "src").mkdir()
    (tmp_path / "tests").mkdir()
    ...
    with CodeMap(tmp_path) as codemap:
        codemap.sync()
        relevance = codemap.verification_relevance("Fix ember_snow implementation and verify behavior", limit=20)
    return next(row for row in relevance["candidates"] if row["path"] == "tests/test_owner.py")


def test_type_checking_only_import_is_not_authoritative_direct_reference(tmp_path: Path) -> None:
...
def test_statically_dead_import_is_not_authoritative_direct_reference(tmp_path: Path) -> None:
    row = _verification_reference_case(
        tmp_path,
        "if False:\n    from src.owner import ember_snow\n\ndef test_owner():\n    assert True\n",
    )
...
def test_reachable_aliased_symbol_use_is_authoritative_direct_reference(tmp_path: Path) -> None:
    row = _verification_reference_case(
        tmp_path,
...
def test_verification_reference_index_invalidates_when_source_changes(tmp_path: Path) -> None:
    (tmp_path / "src").mkdir()
    (tmp_path / "tests").mkdir()
    (tmp_path / "src" / "owner.py").write_text("def ember_snow(value):\n    return value\n")
...
def test_verification_candidate_projection_samples_pytest_config_once(tmp_path: Path, monkeypatch) -> None:
...
def test_verification_reference_projection_reads_each_candidate_snapshot_once(tmp_path: Path, monkeypatch) -> None:
    task, _ = _build_repetitive_verification_repo(tmp_path, distractors=80)
