...
def test_economics_receipt_measures_hashmarks_owned_evidence_only(tmp_path: Path) -> None:
    ...
    assert rows["compact"]["serialized_bytes"] < rows["standard"]["serialized_bytes"] < rows["audit"]["serialized_bytes"]
    ...
    assert {"model-tokens", "tool-calls", "execution-attempts", "billing", "certification"} <= excluded
...
def test_economics_receipt_delta_economics_use_existing_delta_authority(tmp_path: Path) -> None:
    ...
    with CodeMap(tmp_path) as codemap:
        ...
        receipt = codemap.intelligence_economics_receipt(task, ["src/owner.py"], previous_snapshot=previous)
...
def test_query_facade_exposes_economics_without_changing_receipt(tmp_path: Path) -> None:
    _source, task = _repo(tmp_path)
    with CodeMap(tmp_path) as codemap:
        codemap.sync()
        direct = codemap.intelligence_economics_receipt(task, ["src/owner.py"])
        query = codemap.repository_intelligence_query("economics", task, ["src/owner.py"])
...
def test_economics_service_roundtrip(tmp_path: Path) -> None:
    ...
    try:
        client.sync()
        receipt = client.repository_intelligence_query("economics", task, ["src/owner.py"])["result"]
        assert receipt["schema"] == "hashmarks.intelligence-economics-receipt.v1"
    finally:
