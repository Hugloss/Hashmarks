...
_MODULE = __import__(
    "scripts.agent_evaluation.metrics_agent_regret_suite", fromlist=["*"]
)
...
def _write_run(
    root: Path, task: str, mode: str, events: list[dict]
) -> tuple[Path, Path]:
            ...
            json.dumps(
                {
                    ...
                    "task_revision": "v1",
                    "repository_identity": "sha256:repo",
                    "evidence_authority_identity": "grader:evidence:v1",
...
def test_suite_ranks_observed_opportunities_and_pairs_modes(tmp_path: Path) -> None:
    ...
    manifest.write_text(
        json.dumps(
            {
                "schema": "hashmarks.agent-retrieval-regret-suite.v1",
                "suite_id": "suite-1",
                "entries": [
                    _entry(tmp_path, base, evidence),
                    _entry(tmp_path, hm, evidence),
                ],
            }
        )
    )
    ...
    assert (
        report["summary"]["observed_opportunities_ranked"][0]["kind"]
        == "late_all_required_evidence"
    )
...
def test_suite_binds_exact_trace_and_evidence_bytes(tmp_path: Path) -> None:
    module = _load()
    trace, evidence = _write_run(
        tmp_path, "t1", "baseline", [{"kind": "file_read", "path": "src/target.py"}]
    )
    manifest = tmp_path / "suite.json"
    entry = _entry(tmp_path, trace, evidence)
    entry["trace_sha256"] = "sha256:" + "0" * 64
    manifest.write_text(
        json.dumps(
            {
                "schema": "hashmarks.agent-retrieval-regret-suite.v1",
                ...
                "entries": [
                    {
                        ...
                        "trace_sha256": "sha256:x",
                        "evidence_sha256": "sha256:y",
                    }
                ],
            }
        )
