...
def test_change_intelligence_brief_is_compact_identity_bound_repository_truth(tmp_path: Path) -> None:
    task = _repo(tmp_path)
    ...
    assert brief["repository"]["repository_identity"] == packet["identity"]["repository_identity"]
    ...
    assert len(json.dumps(brief, sort_keys=True)) < len(json.dumps(packet, sort_keys=True))
    encoded = json.dumps(brief, sort_keys=True)
    assert '"argv"' not in encoded
    assert '"content"' not in encoded


def test_verification_explanation_selected_and_why_not_are_deterministic(tmp_path: Path) -> None:
    task = _repo(tmp_path)
...
def test_service_exposes_change_brief_and_verification_explanation(tmp_path: Path) -> None:
    task = _repo(tmp_path)
    ...
    _wait(client)
    try:
        brief = client.repository_intelligence_query("change-intelligence", task, ["src/case/engine.py"])["result"]
