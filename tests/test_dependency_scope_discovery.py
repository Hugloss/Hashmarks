...
def _write_scope_fixture(root: Path) -> dict[str, Path]:
    ...
    plain_venv_dep = (
        root / "venv" / "lib" / "python3.13" / "site-packages" / "other_dep.py"
    )
...
def test_full_and_incremental_sync_share_dependency_scope_boundary(
    tmp_path: Path,
) -> None:
