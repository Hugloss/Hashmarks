...
def test_checks_python_spec_is_verification_domain() -> None:
    ...
    assert RepositoryDomain.TEST not in set(
        classify_repository_path("checks/widget/helper.py")
    )


def test_explicit_config_task_uses_verification_backed_active_locality(
    tmp_path: Path,
) -> None:
    _write(
    ...
    _write(
        ...
        "from src.widget.api import handle_quartz_config\ndef test_quartz_config():\n    assert handle_quartz_config('x') == 'x-active'\n",
    )
    _write(
        tmp_path / "legacy/compat.py",
        "# Change the quartz_config accepted-response policy; behavior is configuration-controlled\ndef handle_quartz_config(value: str) -> str:\n    return value + '-legacy'\n",
    )
...
def test_checks_spec_can_be_selected_as_verification(tmp_path: Path) -> None:
    ...
    _write(
        tmp_path / "checks/widget/contract_spec.py",
        "from src.widget.api import handle_quartz_checks\ndef test_quartz_checks_contract():\n    assert handle_quartz_checks('x') == 'x-active'\n",
    )
...
def test_test_prefixed_python_module_under_src_is_not_test_domain() -> None:
    domains = set(
        classify_repository_path("src/oh_goon/repository_tooling/test_batches.py")
    )
    assert RepositoryDomain.SOURCE in domains
    assert RepositoryDomain.TEST not in domains
    assert RepositoryDomain.TEST in set(
        classify_repository_path("tests/unit/test_batches.py")
    )
