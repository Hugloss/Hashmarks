...
_MODULE = __import__(
    "scripts.agent_evaluation.metrics_agent_experiment_set", fromlist=["*"]
)
...
def _experiment(
    ...
    *,
    repo: str | None = None,
    tasks: int = 1,
    task_policy: str = "sha256:policy-v1",
    model_config: str = "sha256:model-config",
) -> Path:
        ...
        json.dumps(
            {
                ...
                "experiment_id": f"experiment-{number}",
                "model_identity": "model-x",
                "model_config_identity": model_config,
                "task_policy_identity": task_policy,
                "benchmark_protocol_identity": "sha256:protocol-v1",
                "runner_identity": "runner:test",
                "grader_identity": "grader:test:v1",
                "provider_identity": "provider:test:v1",
                "runs": runs,
...
def _set(tmp_path: Path, *, experiments: int = 2, tasks_each: int = 2) -> Path:
    ...
    for i in range(experiments):
        ...
        entries.append(
            {
                "manifest": str(exp.relative_to(tmp_path)),
                "manifest_sha256": _digest(exp),
            }
        )
    ...
    path.write_text(
        json.dumps(
            {
                "schema": "hashmarks.agent-experiment-set.v2",
                "experiment_set_id": "set-1",
                "model_identity": "model-x",
                "model_config_identity": "sha256:model-config",
                "task_policy_identity": "sha256:policy-v1",
                "benchmark_protocol_identity": "sha256:protocol-v1",
                "runner_identity": "runner:test",
                "grader_identity": "grader:test:v1",
                "provider_identity": "provider:test:v1",
                "replication_policy": {
                    "min_repeats_per_group": 2,
                    "confidence": 0.95,
                    "bootstrap_samples": 1000,
                },
                "experiments": [
                    dict(entry, replication_group=f"group-{i}")
...
def test_set_aggregates_exact_tokens_but_does_not_overclaim_breadth(
    tmp_path: Path,
) -> None:
    module = _load()
...
def test_set_rejects_model_config_drift(tmp_path: Path) -> None:
    ...
    payload["experiments"].append(
        {
            "manifest": str(drift.relative_to(tmp_path)),
            "manifest_sha256": _digest(drift),
        }
    )
...
def test_set_rejects_duplicate_experiment_id(tmp_path: Path) -> None:
    ...
    payload["experiments"].append(
        {
            "manifest": str(duplicate.relative_to(tmp_path)),
            "manifest_sha256": _digest(duplicate),
        }
    )
    set_path.write_text(json.dumps(payload))
...
def test_set_rejects_reused_task_identity_for_padding(tmp_path: Path) -> None:
    ...
    set_path.write_text(
        json.dumps(
            {
                "schema": "hashmarks.agent-experiment-set.v2",
                "experiment_set_id": "set-dup-task",
                "model_identity": "model-x",
                "model_config_identity": "sha256:model-config",
                "task_policy_identity": "sha256:policy-v1",
                "benchmark_protocol_identity": "sha256:protocol-v1",
                ...
                "experiments": [
                    {
                        ...
                        "manifest_sha256": _digest(first),
                        "replication_group": "group-a",
                    },
                    {
                        "manifest": str(second.relative_to(tmp_path)),
                        "manifest_sha256": _digest(second),
                        "replication_group": "group-b",
                    },
                ],
            }
        )
...
def _replicated_set(tmp_path: Path, *, repeats: int = 3) -> Path:
    ...
    for i in range(repeats):
        exp = _experiment(
            tmp_path / f"r{i}", i, repo=f"sha256:repo-repeat-{i}", tasks=1
        )
        entries.append(
            {
                "manifest": str(exp.relative_to(tmp_path)),
                "manifest_sha256": _digest(exp),
                "replication_group": "same-controlled-condition",
            }
        )
    ...
    path.write_text(
        json.dumps(
            {
                "schema": "hashmarks.agent-experiment-set.v2",
                "experiment_set_id": "replicated-set",
                "model_identity": "model-x",
                "model_config_identity": "sha256:model-config",
                "task_policy_identity": "sha256:policy-v1",
                "benchmark_protocol_identity": "sha256:protocol-v1",
...
def test_v2_replication_reports_repeat_variance_and_descriptive_ci(
    tmp_path: Path,
) -> None:
    module = _load()
    ...
    assert (
        rep["descriptive_confidence_interval"]["scope"]
        == "exact-retained-experiments-only"
    )
