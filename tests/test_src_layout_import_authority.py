...
def test_setuptools_src_layout_uses_import_identity_not_repository_prefix(tmp_path: Path) -> None:
    ...
    _write(tmp_path, "tests/test_api.py", "from pkg.api import run\ndef test_run(): assert run() == 1\n")
    with CodeMap(tmp_path) as codemap:
        codemap.sync()
        row = codemap.store.file_row("src/pkg/api.py")
        affected = codemap.affected("src/pkg/api.py", max_depth=2)
        owners, ambiguous = codemap._resolve_import_owner_evidence("tests/test_api.py", "pkg.api.run")
...
def test_relative_import_in_setuptools_src_layout_uses_configured_module_root(tmp_path: Path) -> None:
    _write(tmp_path, "pyproject.toml", '[project]\nname="demo"\nversion="0"\n[tool.setuptools.package-dir]\n""="src"\n')
    _write(tmp_path, "src/pkg/__init__.py", "")
    _write(tmp_path, "src/pkg/api.py", "def run(): return 1\n")
    _write(tmp_path, "src/pkg/use.py", "from .api import run\ndef use(): return run()\n")
    with CodeMap(tmp_path) as codemap:
        codemap.sync()
        owners, ambiguous = codemap._resolve_import_owner_evidence("src/pkg/use.py", ".api.run")
    assert owners == ["src/pkg/api.py"]
    assert ambiguous is False


def test_unconfigured_src_namespace_keeps_src_as_module_identity(tmp_path: Path) -> None:
    _write(tmp_path, "pyproject.toml", '[project]\nname="demo"\nversion="0"\n')
...
def test_source_root_configuration_change_reprojects_reused_module_identity(tmp_path: Path) -> None:
    _write(tmp_path, "pyproject.toml", '[project]\nname="demo"\nversion="0"\n')
    _write(tmp_path, "src/pkg/api.py", "def run(): return 1\n")
    _write(tmp_path, "src/pkg/use.py", "from pkg.api import run\n")
...
def test_duplicate_configured_import_identity_is_ambiguous_and_not_reverse_owned(tmp_path: Path) -> None:
    ...
    for service in ("a", "b"):
        ...
        _write(tmp_path, f"services/{service}/src/shared/api.py", "def run(): return 1\n")
    _write(tmp_path, "consumer.py", "from shared.api import run\ndef use(): return run()\n")
    _write(tmp_path, "tests/test_consumer.py", "from shared.api import run\ndef test_run(): assert run() == 1\n")
    with CodeMap(tmp_path) as codemap:
        codemap.sync()
        paths = codemap.store.module_paths("shared.api")
        owners, ambiguous = codemap._resolve_import_owner_evidence("tests/test_consumer.py", "shared.api.run")
...
def test_duplicate_import_identity_becomes_unique_after_one_owner_removed(tmp_path: Path) -> None:
    _write(tmp_path, "pyproject.toml", '''[tool.poetry]\nname="demo"\nversion="0"\npackages=[\n  {include="shared", from="services/a/src"},\n  {include="shared", from="services/b/src"},\n]\n''')
    for service in ("a", "b"):
        _write(tmp_path, f"services/{service}/src/shared/api.py", "def run(): return 1\n")
    _write(tmp_path, "consumer.py", "from shared.api import run\n")
    ...
    with CodeMap(tmp_path, state_dir=state) as codemap:
        codemap.sync()
        owners, ambiguous = codemap._resolve_import_owner_evidence("consumer.py", "shared.api.run")
...
def test_nested_project_pyproject_scopes_src_root_to_project_directory(tmp_path: Path) -> None:
    _write(tmp_path, "services/a/pyproject.toml", '[project]\nname="service-a"\nversion="0"\n[tool.setuptools.package-dir]\n""="src"\n')
...
def test_nested_projects_exposing_same_import_identity_remain_ambiguous(tmp_path: Path) -> None:
    for service in ("a", "b"):
        _write(tmp_path, f"services/{service}/pyproject.toml", f'[project]\nname="service-{service}"\nversion="0"\n[tool.setuptools.package-dir]\n""="src"\n')
        _write(tmp_path, f"services/{service}/src/shared/api.py", "def run(): return 1\n")
    _write(tmp_path, "consumer.py", "from shared.api import run\n")
    with CodeMap(tmp_path) as codemap:
        codemap.sync()
...
def test_same_service_targeted_pyproject_change_refreshes_import_roots(tmp_path: Path) -> None:
    _write(tmp_path, "services/a/pyproject.toml", '[project]\nname="service-a"\nversion="0"\n')
    _write(tmp_path, "services/a/src/pkg/api.py", "def run(): return 1\n")
    _write(tmp_path, "services/a/src/pkg/use.py", "from pkg.api import run\n")
    with CodeMap(tmp_path) as codemap:
...
def test_uv_workspace_exclude_does_not_create_false_import_ambiguity(tmp_path: Path) -> None:
    _write(tmp_path, "pyproject.toml", '''[project]\nname="root"\nversion="0"\n[tool.uv.workspace]\nmembers=["packages/*"]\nexclude=["packages/legacy"]\n''')
    for member in ("app", "legacy"):
        _write(tmp_path, f"packages/{member}/pyproject.toml", f'''[project]\nname="{member}"\nversion="0"\n[tool.setuptools.package-dir]\n""="src"\n''')
        _write(tmp_path, f"packages/{member}/src/shared/api.py", "def run(): return 1\n")
    _write(tmp_path, "consumer.py", "from shared.api import run\n")
    with CodeMap(tmp_path) as codemap:
        codemap.sync()
    ...
    assert legacy is not None and legacy["module_name"] == "packages.legacy.src.shared.api"
    assert owners == ["packages/app/src/shared/api.py"]
    assert ambiguous is False


def test_targeted_pyproject_change_reprojects_python_without_explicit_source_paths(tmp_path: Path) -> None:
    _write(tmp_path, "services/a/pyproject.toml", '[project]\nname="service-a"\nversion="0"\n')
    _write(tmp_path, "services/a/src/pkg/api.py", "def run(): return 1\n")
    with CodeMap(tmp_path) as codemap:
        codemap.sync()
...
def test_uv_workspace_membership_change_reprojects_existing_member_files(tmp_path: Path) -> None:
    _write(tmp_path, "pyproject.toml", '''[project]\nname="root"\nversion="0"\n[tool.uv.workspace]\nmembers=["packages/a"]\n''')
    for member in ("a", "b"):
        _write(tmp_path, f"packages/{member}/pyproject.toml", f'''[project]\nname="{member}"\nversion="0"\n[tool.setuptools.package-dir]\n""="src"\n''')
...
def test_uv_workspace_member_move_root_only_sync_discovers_new_and_drops_ghost_owner(tmp_path: Path) -> None:
    import shutil
    _write(tmp_path, "pyproject.toml", '''[project]\nname="root"\nversion="0"\n[tool.uv.workspace]\nmembers=["packages/a"]\n''')
...
def test_uv_workspace_member_removal_reprojects_files_without_deleting_repository_evidence(tmp_path: Path) -> None:
    _write(tmp_path, "pyproject.toml", '''[project]\nname="root"\nversion="0"\n[tool.uv.workspace]\nmembers=["packages/a", "packages/b"]\n''')
    for member in ("a", "b"):
        _write(tmp_path, f"packages/{member}/pyproject.toml", f'''[project]\nname="{member}"\nversion="0"\n[tool.setuptools.package-dir]\n""="src"\n''')
        _write(tmp_path, f"packages/{member}/src/pkg/api.py", "def run(): return 1\n")
    with CodeMap(tmp_path) as codemap:
        codemap.sync()
        assert codemap.store.module_paths("pkg.api") == ["packages/a/src/pkg/api.py", "packages/b/src/pkg/api.py"]
        _write(tmp_path, "pyproject.toml", '''[project]\nname="root"\nversion="0"\n[tool.uv.workspace]\nmembers=["packages/a"]\n''')
        result = codemap.sync(paths=["pyproject.toml"])
        removed_member = codemap.store.file_row("packages/b/src/pkg/api.py")
        paths = codemap.store.module_paths("pkg.api")
    assert result.discovered == 2
    assert removed_member is not None and removed_member["module_name"] == "packages.b.src.pkg.api"
    assert paths == ["packages/a/src/pkg/api.py"]


def test_uv_member_source_root_move_pyproject_only_sync_drops_ghost_and_discovers_new_root(tmp_path: Path) -> None:
    import shutil
    _write(tmp_path, "pyproject.toml", '''[project]\nname="root"\nversion="0"\n[tool.uv.workspace]\nmembers=["packages/a"]\n''')
    _write(tmp_path, "packages/a/pyproject.toml", '''[project]\nname="a"\nversion="0"\n[tool.setuptools.package-dir]\n""="src"\n''')
    _write(tmp_path, "packages/a/src/pkg/api.py", "def run(): return 1\n")
    with CodeMap(tmp_path) as codemap:
        codemap.sync()
        shutil.move(str(tmp_path / "packages/a/src"), str(tmp_path / "packages/a/python"))
        _write(tmp_path, "packages/a/pyproject.toml", '''[project]\nname="a"\nversion="0"\n[tool.setuptools.package-dir]\n""="python"\n''')
...
def test_uv_member_manifest_deletion_revokes_packaging_import_authority(tmp_path: Path) -> None:
    _write(tmp_path, "pyproject.toml", '''[project]\nname="root"\nversion="0"\n[tool.uv.workspace]\nmembers=["packages/a"]\n''')
    _write(tmp_path, "packages/a/pyproject.toml", '''[project]\nname="a"\nversion="0"\n[tool.setuptools.package-dir]\n""="src"\n''')
    _write(tmp_path, "packages/a/src/pkg/api.py", "def run(): return 1\n")
    with CodeMap(tmp_path) as codemap:
...
def test_uv_workspace_namespace_portions_are_distinct_modules_not_duplicate_owners(tmp_path: Path) -> None:
    _write(tmp_path, "pyproject.toml", '''[project]\nname="root"\nversion="0"\n[tool.uv.workspace]\nmembers=["packages/a", "packages/b"]\n''')
    _write(tmp_path, "packages/a/pyproject.toml", '''[project]\nname="a"\nversion="0"\n[tool.setuptools.package-dir]\n""="src"\n''')
    ...
    _write(tmp_path, "consumer.py", "from company.alpha.api import alpha\nfrom company.beta.api import beta\n")
    with CodeMap(tmp_path) as codemap:
        codemap.sync()
        ...
        assert codemap.store.module_paths("company.alpha.api") == ["packages/a/src/company/alpha/api.py"]
        assert codemap.store.module_paths("company.beta.api") == ["packages/b/src/company/beta/api.py"]
    assert a == ["packages/a/src/company/alpha/api.py"]
    assert b == ["packages/b/src/company/beta/api.py"]


def test_uv_workspace_same_namespace_concrete_module_duplicate_fails_closed(tmp_path: Path) -> None:
    _write(tmp_path, "pyproject.toml", '''[project]\nname="root"\nversion="0"\n[tool.uv.workspace]\nmembers=["packages/a", "packages/b"]\n''')
    for member in ("a", "b"):
        _write(tmp_path, f"packages/{member}/pyproject.toml", f'''[project]\nname="{member}"\nversion="0"\n[tool.setuptools.package-dir]\n""="src"\n''')
        _write(tmp_path, f"packages/{member}/src/company/shared/api.py", "def run(): return 1\n")
    _write(tmp_path, "consumer.py", "from company.shared.api import run\n")
    with CodeMap(tmp_path) as codemap:
        codemap.sync()
        paths = codemap.store.module_paths("company.shared.api")
        resolved = codemap._resolve_import_paths("consumer.py", "company.shared.api.run")
        ambiguous = codemap._python_import_identity_ambiguous("consumer.py", "company.shared.api.run")
    assert paths == ["packages/a/src/company/shared/api.py", "packages/b/src/company/shared/api.py"]
    assert resolved == []
    assert ambiguous is True


def test_uv_workspace_transition_reprojection_does_not_scan_all_persisted_rows(tmp_path: Path, monkeypatch) -> None:
    _write(tmp_path, "pyproject.toml", '''[project]\nname="root"\nversion="0"\n[tool.uv.workspace]\nmembers=["packages/a"]\n''')
    for member in ("a", "b"):
        _write(tmp_path, f"packages/{member}/pyproject.toml", f'''[project]\nname="{member}"\nversion="0"\n[tool.setuptools.package-dir]\n""="src"\n''')
        for index in range(20):
            _write(tmp_path, f"packages/{member}/src/pkg/m{index}.py", f"VALUE = {index}\n")
    with CodeMap(tmp_path) as codemap:
        codemap.sync()
        def forbidden():
            raise AssertionError("uv member reprojection must not materialize all repository rows")
