...
from scripts.agent_evaluation.experiment import (
    ExperimentLane,
    experiment_manifest,
    import_native_run,
    load_jsonl,
...
def test_manifest_is_public_metadata_not_secret_answers() -> None:
    lanes = [
        ExperimentLane("cheap-native", "codex-jsonl", "cheap", "native", "low"),
        ExperimentLane("cheap-hm", "codex-jsonl", "cheap", "hashmarks", "low"),
    ]
    value = experiment_manifest(
        corpus_identity="sha256:corpus", lanes=lanes, secret_identity="sha256:secret"
    )
...
def test_coverage_ledger_fails_closed_on_missing_lane_task() -> None:
    ...
    manifest = experiment_manifest(
        corpus_identity="c",
        lanes=[
            ExperimentLane("native", "generic-jsonl", "m", "native"),
    ...
    ledger = coverage_ledger(
        manifest=manifest,
        task_ids=["t1", "t2"],
        bundles=[_bundle("native", "t1"), _bundle("native", "t2"), _bundle("hm", "t1")],
    )
...
def test_coverage_ledger_rejects_duplicates_and_unexpected_runs() -> None:
    ...
    manifest = experiment_manifest(
        corpus_identity="c",
        lanes=[ExperimentLane("native", "generic-jsonl", "m", "native")],
    )
    ledger = coverage_ledger(
        manifest=manifest,
        task_ids=["t1"],
...
def test_coverage_ledger_can_certify_exact_matrix() -> None:
    ...
    bundles = [
        _bundle(lane, task) for lane in ("native", "hm") for task in ("t1", "t2")
    ]
...
def _trace_bundle(lane: str, task: str, tokens: int, wall: float):
    return {
        ...
        "trace": {
            ...
            "task_id": task,
            "events": [
                {"event_type": "final_result", "input_tokens": tokens, "wall_ms": wall}
            ],
        },
    }
...
def test_report_requires_complete_coverage_before_secret_join() -> None:
    ...
    with pytest.raises(ValueError, match="coverage is incomplete"):
        assemble_experiment_report(
            manifest=m,
            task_ids=["t1"],
            bundles=[_trace_bundle("native", "t1", 10, 20)],
            grades={"t1": True},
        )
...
def test_report_emits_verified_cost_and_pareto_without_answer_key() -> None:
    ...
    m = experiment_manifest(
        corpus_identity="c",
        lanes=[
            ExperimentLane("native", "generic-jsonl", "strong", "native"),
            ExperimentLane("hm", "generic-jsonl", "cheap", "hashmarks"),
        ],
    )
    bundles = [
        _trace_bundle("native", "t1", 100, 100),
        _trace_bundle("hm", "t1", 40, 80),
    ]
    r = assemble_experiment_report(
        manifest=m, task_ids=["t1"], bundles=bundles, grades={"t1": True}
    )
...
def _cert_bundle(lane: str, task: str, trace_identity: str, native_identity: str):
    return {
        ...
        "native_event_sha256": native_identity,
    }
...
def test_experiment_certificate_binds_manifest_traces_secret_and_report() -> None:
    ...
    report = {
        ...
        "complete": True,
        "manifest_identity": m["manifest_identity"],
        "coverage": {"complete": True},
        "lanes": [],
    }
    cert = experiment_certificate(
        manifest=m, bundles=bundles, report=report, secret_identity="sha256:secret"
    )
