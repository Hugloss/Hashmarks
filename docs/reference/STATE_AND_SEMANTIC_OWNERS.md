...
`repository_declaration_provider.py` owns only the explicit provider SPI and bounded provider-envelope validation. `repository_declaration_discovery.py` deterministically composes caller-selected providers into a discovery wrapper around the canonical declaration packet. Provider detection/provenance/warnings remain separate from declaration semantics; there is no ambient provider registry, plugin entry-point loading, or MCP provider execution authority.
