...
`repository_declaration_provider.py` owns the explicit provider SPI, the Hashmarks-owned revision-binding provider read context, provider input/evidence envelope validation, and provider input-count bounds. `repository_declaration_discovery.py` deterministically composes caller-selected providers into a discovery wrapper around the canonical declaration packet and revalidates provider inputs around nested declaration qualification. Provider detection/provenance/warnings/input observations remain separate from declaration semantics; there is no ambient provider registry, plugin entry-point loading, MCP provider execution authority, provider sandbox, retry/timeout owner, or provider-result certification authority. Provider code is caller-selected trusted in-process extension code under a read-only contract.
