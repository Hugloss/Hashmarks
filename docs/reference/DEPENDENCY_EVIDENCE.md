...
Dependency query requests are fail-closed for unknown fields while preserving the documented shared request shape used across operations. Graph traversal never crosses declared context boundaries: an observation with multiple contexts requires an explicit context. Within one context, relationship markers are preserved as facts but are not evaluated for PEP 508/environment satisfiability by topology queries, so a returned path is structural possible-topology evidence rather than a claim that one runtime environment satisfies every marker. Relationship multiplicity by marker/effective scope remains in the observation and delta, while node-topology traversal collapses duplicate source-target edges.
...
The current adapter admits only uv lock schema version 1 shapes it can model faithfully. Top-level `resolution-markers` are preserved as producer provenance when package selections themselves do not carry fork membership; topology remains the structural union of marker-conditioned relationships. Package-level `resolution-markers` are rejected because selection fork membership is not yet represented by the general model. Declared conflict sets are also rejected rather than flattened because they encode mutually exclusive extras/groups that Hashmarks cannot yet preserve.
