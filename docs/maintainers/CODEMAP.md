...
- `dependency_resolution_evidence.py` — owns typed producer-neutral external dependency-resolution observations, resolution definition/observation identity, comparability, and explicit repository-input correspondence. It does not run package managers, inspect dependency implementations, infer import/distribution ownership, or duplicate `project_graph.py`, `import_resolution.py`, repository freshness/delta, or `evidence_correlation.py`. Module/distribution ownership remains a later admitted extension of this dependency evidence domain.
