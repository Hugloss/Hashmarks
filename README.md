...
For in-process integrations, `RepositoryDeclarationProvider` plus `CodeMap.discover_repository_declarations(...)` provides an explicit provider-discovery seam. Providers are caller-selected and producer-specific; Hashmarks does not auto-load plugins or run arbitrary providers inside MCP. The existing MCP `repository_declarations` tool accepts already normalized groups.
