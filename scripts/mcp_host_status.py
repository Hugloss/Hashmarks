...
def _opencode_discovery_ok(result: subprocess.CompletedProcess[str]) -> bool:
    ...
    return any("connected" in line and not any(marker in line for marker in bad) for line in matching)
...
def _opencode(workspace: Path, candidate_identity: str) -> HostStatus:
    ...
    candidates = [workspace / "opencode.json", workspace / ".opencode" / "opencode.json"]
    ...
    source = Check("PASS", str(path.relative_to(workspace))) if path.exists() else Check("NOT REGISTERED")
    ...
    if installed.status == "PASS" and registration.status == "PASS":
        ...
        discovery = Check("PASS" if _opencode_discovery_ok(result) else "FAIL", _summarize_failure(result) if result.returncode else "")
    return HostStatus(
        installed,
        registration,
        source,
...
def _claude(workspace: Path, candidate_identity: str) -> HostStatus:
    ...
    if installed.status == "PASS" and registration.status == "PASS":
        ...
        discovery = Check("PASS" if _claude_discovery_ok(result) else "FAIL", _summarize_failure(result) if result.returncode else "")
    return HostStatus(
        installed,
        registration,
        source,
...
def _codex(workspace: Path, candidate_identity: str) -> HostStatus:
    ...
    try:
        ...
        data = {}
    entry = (data.get("mcp_servers") or {}).get("hashmarks") if isinstance(data, dict) else None
    if not isinstance(entry, dict):
        registration = Check("NOT REGISTERED")
        ownership = Check("NOT CHECKED")
    elif (
        entry.get("enabled", True) is True
        and _is_uv_registration(entry.get("command"), entry.get("args"))
    ):
        registration = Check("PASS")
        ownership = Check("HASHMARKS")
    else:
        registration = Check("FOREIGN CONFIG")
        ownership = Check("USER")
    source = Check("PASS", ".codex/config.toml") if path.exists() else Check("NOT REGISTERED")
    discovery = Check("NOT CHECKED")
    if installed.status == "PASS" and registration.status == "PASS":
...
def _pi(workspace: Path, candidate_identity: str) -> dict[str, Check]:
    ...
    if installed.status == "PASS":
        ...
        adapter = Check("PASS" if result.returncode == 0 and "pi-mcp-adapter" in text else "ADAPTER REQUIRED")
    path = workspace / ".mcp.json"
    data = _read_json(path)
    entry = None if data is None else (data.get("mcpServers") or {}).get("hashmarks")
    if isinstance(entry, dict) and _is_uv_registration(entry.get("command"), entry.get("args")):
        registration = Check("PASS")
        ownership = Check("HASHMARKS")
    ...
    if adapter.status == "PASS" and registration.status == "PASS":
        discovery = Check("READY", "pi-mcp-adapter loads project .mcp.json; live tool call not executed by status")
    return {
        "installed": installed,
        "native_mcp": Check("NO", "Pi core intentionally has no native MCP client"),
        "mcp_adapter": adapter,
        "project_registration": registration,
        "registration_source": Check("PASS", ".mcp.json") if path.exists() else Check("NOT REGISTERED"),
        "registration_ownership": ownership,
        "global_hashmarks_registration": Check("NOT REQUIRED", "Hashmarks registration is project-local"),
        "discovery": discovery,
...
def collect(workspace: Path) -> dict[str, Any]:
    ...
    catalog_ok = all(callable(getattr(HashmarksMcpSurface, name, None)) for name in EXPECTED_TOOLS)
    core = {
        "mcp_server_executable": Check("PASS" if (workspace / ".venv" / "bin" / "hashmarks").exists() or shutil.which("hashmarks") else "NOT INSTALLED"),
        "mcp_sdk": Check("PASS" if importlib.util.find_spec("mcp") is not None else "NOT INSTALLED"),
        "mcp_tool_catalog": Check("PASS" if catalog_ok else "FAIL", f"{len(EXPECTED_TOOLS)} tools"),
    }
    def host_dict(value: HostStatus) -> dict[str, dict[str, str]]:
        return {
            ...
            "global_hashmarks_registration": asdict(value.global_hashmarks_registration),
    ...
    return {
        ...
        "pi": {key: asdict(value) for key, value in _pi(workspace, candidate_identity).items()},
    }
...
def render(report: dict[str, Any]) -> str:
    ...
    rows.append(_line("MCP server executable ", report["core"]["mcp_server_executable"]))
    rows.append(_line("MCP SDK ", report["core"]["mcp_sdk"]))
    rows.append(_line("MCP tool catalog ", report["core"]["mcp_tool_catalog"]))
    for title, key in (("OpenCode", "opencode"), ("Claude Code", "claude"), ("Codex", "codex")):
        host = report[key]
        rows.extend(["", title])
        ...
        rows.append(_line("Global Hashmarks registration ", host["global_hashmarks_registration"]))
        rows.append(_line("MCP discovery ", host["discovery"]))
        rows.append(_line("Last successful MCP call ", host["last_successful_mcp_call"]))
    pi = report["pi"]
    rows.extend(["", "Pi"])
    ...
    rows.append(_line("Global Hashmarks registration ", pi["global_hashmarks_registration"]))
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Inspect project-local Hashmarks MCP host integrations")
