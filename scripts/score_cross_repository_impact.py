...
_TOKENS = ("amber", "bravo", "cobalt", "delta", "ember", "fjord", "glint", "harbor", "indigo", "juno")


def _write_repo(root: Path, index: int, *, decoys: int) -> tuple[list[dict], list[dict]]:
    ...
    (web_root / "package.json").write_text(json.dumps({"name": f"@demo/web-{token}"}), encoding="utf-8")
    (mobile_root / "package.json").write_text(json.dumps({"name": f"@demo/mobile-{token}"}), encoding="utf-8")
    source_path = "backend/src/Api.java"
    shared_path = f"contracts/{token}.yaml"
    (backend_root / "src/Api.java").write_text(f"class Api {{ String marker = \"{token}\"; }}\n", encoding="utf-8")
    (web_root / "src/api.ts").write_text(f"export const marker = '{token}';\n", encoding="utf-8")
    (mobile_root / "src/api.ts").write_text(f"export const marker = '{token}';\n", encoding="utf-8")
    (root / "contracts").mkdir()
    (root / shared_path).write_text(f"openapi: 3.1.0\ninfo:\n  title: {token}\n", encoding="utf-8")
    ...
    for decoy in range(decoys):
        ...
        (droot / "package.json").write_text(json.dumps({"name": name}), encoding="utf-8")
        (droot / "value.ts").write_text(f"export const value = '{name}';\n", encoding="utf-8")
...
def generate(base: Path, public_path: Path, secret_path: Path, *, scenarios: int = 6, decoys: int = 24) -> None:
    if base.exists():
        shutil.rmtree(base)
    ...
    public_path.write_text(json.dumps({"schema": "hashmarks.cross-repo-public.v1", "tasks": public_rows}, indent=2, sort_keys=True), encoding="utf-8")
    secret_path.write_text(json.dumps({"schema": "hashmarks.cross-repo-secret.v1", "tasks": secret_rows}, indent=2, sort_keys=True), encoding="utf-8")
...
def run(base: Path, public_path: Path, secret_path: Path, output: Path) -> dict:
    ...
    for task in public:
        ...
        with CodeMap(repo, artifact_db=repo / "artifacts.sqlite3") as codemap:
            ...
            if task["kind"] == "shared-input":
                text = path.read_text(encoding="utf-8").replace("openapi: 3.1.0", "openapi: 3.1.1")
        ...
        frozen.append({
            ...
            "kind": task["kind"],
            "packet": packet,
            "packet_bytes": len(json.dumps(packet, sort_keys=True, separators=(",", ":")).encode("utf-8")),
            "impact_ms": impact_ms,
        })

    # SECRET is opened only after every packet is frozen.
    secret = {row["id"]: row for row in json.loads(secret_path.read_text(encoding="utf-8"))["tasks"]}
    scored: list[dict] = []
    for row in frozen:
        ...
        project_fragment = packet.get("project_impact") if isinstance(packet.get("project_impact"), dict) else {}
        provenance_rows = {str(value.get("project")): value for value in project_fragment.get("affected", []) if isinstance(value, dict)}
        edges = project_fragment.get("edges", []) if isinstance(project_fragment, dict) else []
        provenance_correct = bool(edges) and all(
            isinstance(edge, dict) and edge.get("producer") == "declared-project-links" for edge in edges
        )
        ...
        if row["kind"] == "shared-input":
            ...
            refresh_correct = isinstance(refresh, dict) and refresh.get("producer") == "declared-project-links"
        scored.append({
    ...
    summary = {
        ...
        "shared_refresh_correct": sum(int(row["refresh_correct"]) for row in scored if row["kind"] == "shared-input"),
...
def main() -> int:
    ...
    if args.generate:
        generate(args.base, args.public, args.secret, scenarios=args.scenarios, decoys=args.decoys)
