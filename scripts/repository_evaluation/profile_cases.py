...
def _selected_cases(
    doc: Mapping[str, object], *, shard_count: int, shard_index: int
...
def profile_cases(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="hashmarks-profile-") as state_root:
        ...
        with CodeMap(
            workspace, state_dir=state, artifact_db=state / "artifacts.sqlite3"
        ) as codemap:
            codemap.sync()
            repo_identity = repository_content_identity(
                workspace, excluded_paths=(codemap.state_dir,)
            )
            for case in selected:
                for _ in range(warmups):
                    _run_action(codemap, case)
                samples_ns = []
                fingerprints = []
                for _ in range(samples):
                    elapsed, fingerprint = _run_action(codemap, case)
                    samples_ns.append(elapsed)
                    fingerprints.append(fingerprint)
                median = int(statistics.median(samples_ns))
                ...
                rows.append(
                    {
                        ...
                        "semantic_stable": len(set(fingerprints)) == 1,
                    }
                )
    return {
        "schema": PROFILE_SCHEMA,
        "suite": doc.get("suite"),
        "cases_sha256": "sha256:" + canonical_sha256(doc),
...
def paired_profile_cases(
    *,
    ...
    rows = []
    with (
        tempfile.TemporaryDirectory(prefix="hashmarks-paired-a-") as ar,
        tempfile.TemporaryDirectory(prefix="hashmarks-paired-b-") as br,
    ):
        sa = Path(ar)
        sb = Path(br)
        with (
            CodeMap(
                workspace_a, state_dir=sa, artifact_db=sa / "artifacts.sqlite3"
            ) as a,
            CodeMap(
                workspace_b, state_dir=sb, artifact_db=sb / "artifacts.sqlite3"
            ) as b,
        ):
            a.sync()
            b.sync()
            aid = repository_content_identity(
                workspace_a, excluded_paths=(a.state_dir,)
            )
            bid = repository_content_identity(
...
def main() -> int:
    ...
    p.add_argument("--workspace-a", type=Path)
    p.add_argument("--workspace-b", type=Path)
    p.add_argument("--cases", type=Path, required=True)
    p.add_argument("--output", type=Path, required=True)
    p.add_argument("--warmups", type=int, default=2)
    p.add_argument("--samples", type=int, default=9)
    p.add_argument("--pairs", type=int)
    p.add_argument("--max-control-mad-pct", type=float, default=10.0)
    p.add_argument("--shard-count", type=int, default=1)
    p.add_argument("--shard-index", type=int, default=0)
    a = p.parse_args()
    if a.workspace_a or a.workspace_b or a.pairs is not None:
        if not a.workspace_a or not a.workspace_b:
            p.error("paired profiling requires --workspace-a and --workspace-b")
        result = paired_profile_cases(
        ...
        if not a.workspace:
            p.error("profile requires --workspace")
