...
def run(
    ...
    *,
    ...
    for task in public:
        ...
        with CodeMap(repo, artifact_db=repo / "artifacts.sqlite3") as codemap:
            ...
            economics = meter.finish(
                evidence=packet, elapsed_ns=elapsed, refresh_ns=None
            )
    ...
    for item in frozen:
        ...
        affected = {
            str(r.get("project")): int(r.get("depth", -1))
            for r in fragment.get("affected", [])
            if isinstance(r, dict)
        }
        expected_depths = {str(k): int(v) for k, v in expected["depths"].items()}
        provenance_recall = sum(
            1 for k, v in expected_depths.items() if affected.get(k) == v
        ) / max(1, len(expected_depths))
        project_recall = len(projects & expected_projects) / max(
            1, len(expected_projects)
        )
        project_precision = (
            len(projects & expected_projects) / max(1, len(projects))
            if projects
            else (1.0 if not expected_projects else 0.0)
        )
        edges = fragment.get("edges", []) if isinstance(fragment, dict) else []
        producer_complete = all(
            isinstance(e, dict) and e.get("producer") == "declared-project-links"
            for e in edges
        )
        refresh_ok = True
        if task["kind"] == "shared-input":
