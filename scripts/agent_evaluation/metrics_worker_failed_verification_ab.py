...
from .metrics_worker_multistep_ab import _verification_state
    ...
    *,
    ...
    with CodeMap(workspace) as codemap:
        ...
        for task in hidden_tasks:
            ...
            injected = next(((rank, hit.path) for rank, hit in enumerate(hits, 1) if hit.path not in expected), None)
            if injected is None:
                raise RuntimeError(f"no plausible wrong candidate available for {task_id}")
...
def _guided_recovery(codemap: CodeMap, query: str, failed_target: str, *, limit: int) -> tuple[str | None, str, int]:
    ...
    if resolution.get("resolved") and resolution.get("target") != failed_target:
        return str(resolution.get("target")), "inspection-" + str(resolution.get("reason")), max(1, examined)
...
def run_worker(*, policy: str, workspace: Path, tasks_path: Path, output: Path, limit: int) -> None:
    ...
    allowed = {"id", "query", "failed_edit_target", "verification_target", "verification_outcome"}
    rows: list[dict[str, object]] = []
    with CodeMap(workspace) as codemap:
        codemap.sync()
        for row in tasks:
            if not isinstance(row, dict) or set(row) - allowed:
                raise ValueError("worker recovery input contains hidden or unsupported fields")
            if row.get("verification_outcome") != "failed":
                raise ValueError("recovery worker requires a failed verification outcome")
            ...
            if policy == "repeat-failed":
                ...
                target, reason, examined = _guided_recovery(codemap, query, failed_target, limit=limit)
            else:
    ...
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(json.dumps({"schema": WORKER_OUTPUT_SCHEMA, "policy": policy, "tasks": rows}, indent=2, sort_keys=True) + "\n", encoding="utf-8")


def _score(hidden_tasks: list[dict[str, Any]], output: dict[str, Any]) -> dict[str, object]:
    ...
    for task in hidden_tasks:
        ...
        rows.append({
            ...
            "recovery_candidates_examined": observed.get("recovery_candidates_examined"),
        })
    n = len(rows)
    summary = {
        "tasks": n,
        ...
        "different_wrong_edits": sum(bool(row["changed_to_different_wrong_edit"]) for row in rows),
        "recovery_candidates_examined": sum(int(row["recovery_candidates_examined"] or 0) for row in rows),
    }
    summary["correct_recovery_rate"] = summary["correct_recoveries"] / n if n else 0.0
    summary["repeated_failed_edit_rate"] = summary["repeated_failed_edits"] / n if n else 0.0
    return {"summary": summary, "tasks": rows}


def collect(root: Path, *, limit: int = 20) -> dict[str, object]:
    reports: list[dict[str, object]] = []
    for name, workspace, corpus, _public_path in materialize_challenge(root / "challenge"):
        hidden = _load_corpus(corpus)
        packet_path = root / "failure-packets" / f"{name}.json"
        injection_provenance = _inject_failures(workspace, hidden, packet_path, limit=limit)
        ...
        for policy in POLICIES:
            ...
            subprocess.run([
                sys.executable, "-m", "scripts.agent_evaluation.metrics_worker_failed_verification_ab", "--worker", "--policy", policy,
                "--workspace", str(workspace), "--tasks", str(packet_path), "--output", str(output),
                "--limit", str(limit),
            ], check=True, env=env)
        ...
        for policy in POLICIES:
            report[policy.replace("-", "_")] = _score(hidden, json.loads(outputs[policy].read_text(encoding="utf-8")))
    ...
    protocol = {
        ...
        "failure_packet_fields": ["id", "query", "failed_edit_target", "verification_target", "verification_outcome"],
        "worker_hidden_fields": ["expected_files", "expected_symbols", "injected_canonical_rank", "selection_rule"],
        ...
        "limit": limit,
        "repositories": [
            {"name": repo["name"], "failure_packet_sha256": repo["failure_packet_sha256"], "hidden_corpus_sha256": repo["hidden_corpus_sha256"]}
            for repo in reports
        ],
    }
    ...
    return {
        ...
        "summary": {
            ...
            "repeat_repeated_failed_edits": total("repeat_failed", "repeated_failed_edits"),
            "fresh_correct_recoveries": total("fresh_research", "correct_recoveries"),
            "fresh_repeated_failed_edits": total("fresh_research", "repeated_failed_edits"),
            "fresh_different_wrong_edits": total("fresh_research", "different_wrong_edits"),
            "guided_correct_recoveries": total("evidence_guided_recovery", "correct_recoveries"),
            "guided_repeated_failed_edits": total("evidence_guided_recovery", "repeated_failed_edits"),
            "guided_different_wrong_edits": total("evidence_guided_recovery", "different_wrong_edits"),
            "guided_recovery_candidates_examined": total("evidence_guided_recovery", "recovery_candidates_examined"),
        },
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Measure worker recovery after controlled failed verification.")
    parser.add_argument("--root", type=Path, default=Path(".hashmarks/benchmarks/worker-failed-verification-ab"))
