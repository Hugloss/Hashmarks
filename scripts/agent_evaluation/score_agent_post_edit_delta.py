...
def _append_probe(path: Path, task_id: str) -> None:
    ...
    elif suffix in {".ts", ".tsx", ".js", ".jsx", ".go", ".rs", ".java", ".c", ".cc", ".cpp", ".h"}:
        marker = f"// hashmarks post-edit delta probe {task_id}"
    elif suffix == ".sql":
        marker = f"-- hashmarks post-edit delta probe {task_id}"
    else:
...
def run(
    ...
    *,
    ...
    with CodeMap(repo) as codemap:
        ...
        for task in tasks:
            ...
            delta = codemap.task_post_change_delta(
                query, [edit_path], previous_evidence=previous, token_budget=token_budget
            )
            ...
            full_refreshed_start = codemap.task_evidence(query, token_budget=token_budget)
            frozen.append({
                "id": task_id,
                "previous": previous,
                "delta": delta,
    ...
    for row in frozen:
        ...
        verify_surface = verification_surface(verify if isinstance(verify, list) else ())
        path_changes = delta.get("path_changes") if isinstance(delta.get("path_changes"), list) else []
        path_change = path_changes[0] if len(path_changes) == 1 and isinstance(path_changes[0], dict) else {}
        invalidated = set(str(value) for value in (delta.get("invalidated") or []))
        reused = set(str(value) for value in (delta.get("reused") or []))
        ...
        graded = {
            ...
            "previous_edit_correct": str(previous.get("edit") or "") == str(truth["expected_edit_path"]),
            "refreshed_edit_correct": str(refreshed.get("edit") or "") == str(truth["expected_edit_path"]),
            "refreshed_verify_correct": str(verify_surface.get("surface") or "") == str(truth["expected_verify_path"]),
            "path_change_detected": state == "changed" and str(path_change.get("path") or "") == str(truth["expected_edit_path"]),
            ...
            "freshness": freshness,
            "semantic_shields": int((delta.get("semantic_invalidation") or {}).get("shields", 0)) if isinstance(delta.get("semantic_invalidation"), dict) else 0,
        }
        graded["fully_correct"] = all(bool(graded[key]) for key in (
            "previous_edit_correct", "refreshed_edit_correct", "refreshed_verify_correct",
            "path_change_detected", "revision_invalidated", "generation_invalidated",
            "edit_authority_reused", "verification_surface_reused",
            "selection_provenance_reused", "replacement_absent",
        ))
    ...
    summary = {
        ...
        "path_change_detected": sum(bool(row["path_change_detected"]) for row in results),
        "revision_invalidated": sum(bool(row["revision_invalidated"]) for row in results),
        "generation_invalidated": sum(bool(row["generation_invalidated"]) for row in results),
        "edit_authority_reused": sum(bool(row["edit_authority_reused"]) for row in results),
        "verification_surface_reused": sum(bool(row["verification_surface_reused"]) for row in results),
        "selection_provenance_reused": sum(bool(row["selection_provenance_reused"]) for row in results),
    ...
    category_summary = {
        name: {
            ...
            "mean_delta_bytes": sum(int(row["delta_bytes"]) for row in rows) / len(rows),
        }
