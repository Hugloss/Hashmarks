...
def collect(workspace: Path, corpus: Path, *, budget: int = 1200, limit: int = 20) -> dict[str, object]:
    ...
    with CodeMap(workspace) as codemap:
        sync = codemap.sync()
        full_tokens = {str(row["path"]): int(row["full_tokens"]) for row in (codemap.store.outline(path) for path in codemap.store.paths()) if row is not None}
        ...
        for task in tasks:
            ...
            expected_symbols = {str(value) for value in task.get("expected_symbols") or ()}
            ...
            hit_symbols = {hit.qualname for hit in hits if hit.qualname} | {hit.name for hit in hits if hit.name}
            ...
            context_full_file_tokens = sum(full_tokens.get(path, 0) for path in context_files)
            ratio = None if pack.estimated_tokens <= 0 else baseline_tokens / pack.estimated_tokens
            context_ratio = None if pack.estimated_tokens <= 0 else context_full_file_tokens / pack.estimated_tokens
            source_range_tokens = sum(item.estimated_tokens for item in pack.items if item.representation == "source-range")
            ...
            rows.append({
                ...
                "file_recall": 1.0 if not expected_files else len(file_hits) / len(expected_files),
                "symbol_recall": 1.0 if not expected_symbols else len(symbol_hits) / len(expected_symbols),
                "first_hit_file": None if not hits else hits[0].path,
                "first_query_hit": bool(hits and hits[0].path in expected_files),
                "fallback_search_required": not expected_files.issubset(relevant_files),
    ...
    return {
        ...
        "repository": {"files": repository_file_count, "source_tokens": repository_source_tokens},
        "sync": sync.as_dict(),
        "index": {key: status[key] for key in ("files", "symbols", "edges", "projects", "project_edges")},
        "summary": {
            "file_recall": sum(row["file_recall"] for row in rows) / task_count,
            "symbol_recall": sum(row["symbol_recall"] for row in rows) / task_count,
            "first_query_hit_rate": sum(bool(row["first_query_hit"]) for row in rows) / task_count,
            "fallback_search_rate": sum(bool(row["fallback_search_required"]) for row in rows) / task_count,
            "average_context_tokens": sum(int(row["context_tokens"]) for row in rows) / task_count,
            "average_candidate_file_reduction": sum(float(row["candidate_file_reduction"]) for row in rows) / task_count,
            "average_source_range_fraction": sum(float(row["source_range_fraction"]) for row in rows) / task_count,
            "selected_file_tokens_avoided": sum(int(row["selected_file_tokens_avoided"]) for row in rows),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Replay agent localization tasks against Hashmarks CodeMap")
    parser.add_argument("--workspace", type=Path, default=Path("."))
    parser.add_argument("--corpus", type=Path, default=Path("benchmarks/agent_tasks.json"))
