...
def main():
    ...
    tasks = [t for t in pub["tasks"] if sm[t["id"]]["category"] == a.category]
    with tempfile.TemporaryDirectory() as td:
        td = Path(td)
        base = td / "base"
        build_combined(a.corpus_root, pub, base)
        old = run_lane(a.old_source, base, tasks, sm, td / "old")
        new = run_lane(a.new_source, base, tasks, sm, td / "new")
    out = {
        "schema": "hashmarks.full-edit-verify-category.v1",
        "category": a.category,
        "old": old,
        "new": new,
    }
    ...
    log_command_output(
        ...
        json.dumps(
            {
                "category": a.category,
                "old": {k: v for k, v in old.items() if k != "rows"},
