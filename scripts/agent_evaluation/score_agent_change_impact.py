...
def _surface_paths(packet: dict[str, Any], role: str) -> set[str]:
    ...
    if not isinstance(rows, list):
        return set()
    return {str(row.get("path") or "") for row in rows if isinstance(row, dict) and row.get("path")}


def run(repo: Path, public_path: Path, secret_path: Path, output: Path) -> dict[str, Any]:
    ...
    frozen_identity = _identity([
        {"id": row["id"], "start": _identity(row["start"]), "impact": _identity(row["impact"])}
        for row in frozen
    ])
    ...
    for row in frozen:
        ...
        impact = row["impact"]
        start = row["start"]
        verification_rows = ((impact.get("surfaces") or {}).get("verification") or []) if isinstance(impact.get("surfaces"), dict) else []
        implementation_rows = ((impact.get("surfaces") or {}).get("implementation") or []) if isinstance(impact.get("surfaces"), dict) else []
        ...
        changed = impact.get("changed") if isinstance(impact.get("changed"), list) else []
        changed_row = changed[0] if len(changed) == 1 and isinstance(changed[0], dict) else {}
        changed_roles = set(str(value) for value in (changed_row.get("roles") or []))
        ...
        serialized = json.dumps(impact, sort_keys=True, separators=(",", ":"), ensure_ascii=False)
        graded = {
            "id": row["id"],
            ...
            "structural_dependency_relevant": (not structural) or bool(implementation_paths),
            "contract_root_typed": ("contract" in changed_roles) if str(truth.get("category")) == "configuration-ownership" else True,
            "external_execution_owner": impact.get("owner") == "external",
            "advisory_authority": impact.get("authority") == "advisory",
            "no_source_or_argv_replay": '"content"' not in serialized and '"argv"' not in serialized,
            "impact_bytes": int(row["impact_bytes"]),
            "impact_ms": float(row["impact_ms"]),
        }
        graded["fully_correct"] = all(bool(graded[key]) for key in (
            "edit_correct", "verify_relevant", "structural_dependency_relevant",
            "contract_root_typed", "external_execution_owner", "advisory_authority",
    ...
    summary = {
        ...
        "structural_dependency_relevant": sum(bool(row["structural_dependency_relevant"]) for row in results),
        "contract_root_typed": sum(bool(row["contract_root_typed"]) for row in results),
        "no_source_or_argv_replay": sum(bool(row["no_source_or_argv_replay"]) for row in results),
        "visible_bytes": sum(int(row["impact_bytes"]) for row in results),
        "visible_bytes_per_task": sum(int(row["impact_bytes"]) for row in results) / count,
