...
class ExperimentCapabilities:
    ...
    def as_record(self) -> dict[str, Any]:
        values = asdict(self)
        return {
            "schema": EXPERIMENT_CAPABILITIES_SCHEMA,
            "values": values,
...
def normalize_capabilities(
    ...
    if capabilities is None:
        return ExperimentCapabilities().as_record()
    if isinstance(capabilities, ExperimentCapabilities):
        return capabilities.as_record()
    unknown = set(capabilities) - _ALLOWED_CAPABILITIES
    if unknown:
        raise ValueError(f"unsupported experiment capabilities: {sorted(unknown)}")
    values = {}
...
def experiment_environment(
    *,
    ...
    if cache_state not in {"cold", "warm", "incremental", "repeat", "unknown"}:
        raise ValueError("unsupported experiment cache_state")
    hashmarks_version = require_nonblank_string(
    ...
    for key, value in (provider_revisions or {}).items():
        ...
        clean_value = require_nonblank_string(
            value, field=f"provider revision {clean_key}"
        )
        revisions[clean_key] = clean_value
    payload = {
        "schema": EXPERIMENT_ENVIRONMENT_SCHEMA,
        "hashmarks_version": hashmarks_version,
        "repository_identity": repository_identity,
        "codemap_generation": codemap_generation,
        "identity_generation": identity_generation,
        "provider_revisions": dict(sorted(revisions.items())),
        "capabilities": normalize_capabilities(capabilities),
        "seed": seed,
        "cache_state": cache_state,
    }
    payload["environment_identity"] = digest(payload)
    return payload
...
class EvidenceEconomics:
    ...
    def add(self, name: str, amount: int = 1) -> None:
        if name not in self.counters:
            raise ValueError(f"unsupported evidence economics counter: {name}")
        if amount < 0:
    ...
    def finish(
        ...
        *,
        ...
        duration = max(
            ...
            else time.perf_counter_ns() - self.started_ns,
        )
        return {
            **self.counters,
            "decision_latency_ns": duration,
            "decision_latency_ms": duration / 1_000_000,
            "refresh_latency_ns": refresh_ns,
            "refresh_latency_ms": None
            if refresh_ns is None
            else refresh_ns / 1_000_000,
...
def classify_decision(packet: Mapping[str, Any]) -> dict[str, Any]:
    ...
    ownership = packet.get("ownership_resolution")
    reasons = []
    if isinstance(identity, Mapping) and identity.get("stale") is True:
        reasons.append("stale_repository_evidence")
    if not isinstance(edit, Mapping) or not edit.get("path"):
        reasons.append("no_exact_owner")
    if isinstance(discrimination, Mapping) and discrimination.get("needed"):
        reason = str(discrimination.get("reason") or "")
        reasons.append(
            "ambiguous_owner" if "ambig" in reason else reason.replace(" ", "_")
        )
    if isinstance(ownership, Mapping):
        status = str(ownership.get("status") or ownership.get("reason") or "")
        if status and status != "resolved" and status not in reasons:
            reasons.append(status.replace(" ", "_"))
    return {
        "status": "resolved"
        if isinstance(edit, Mapping) and edit.get("path") and not reasons
        else "abstained",
        "reasons": reasons,
    }
...
def decision_trace(packet: Mapping[str, Any]) -> dict[str, Any]:
    identity = (
        packet.get("identity") if isinstance(packet.get("identity"), Mapping) else {}
    )
    discrimination = (
    ...
    candidates = (
        discrimination.get("candidates")
        if isinstance(discrimination.get("candidates"), list)
        else []
    )
    emitted = []
    for key in ("edit", "verify", "contract"):
        ...
        if isinstance(row, Mapping) and row.get("path"):
            emitted.append({"role": key, "path": str(row["path"])})
    return {
        "schema": "hashmarks.experiment-decision-trace.v1",
        "decision_generation": identity.get("decision_generation"),
        "repository_identity": identity.get("repository_identity"),
        "codemap_generation": identity.get("codemap_generation"),
        "identity_generation": identity.get("identity_generation"),
        "classification": classify_decision(packet),
        "candidate_count": len(candidates),
        "emitted": emitted,
        "authority_sources": {
            "ownership_resolution": bool(packet.get("ownership_resolution")),
...
def experiment_record(
    *,
    ...
    if environment.get("schema") != EXPERIMENT_ENVIRONMENT_SCHEMA:
        raise ValueError("unsupported experiment environment schema")
    payload = {
        "schema": EXPERIMENT_RECORD_SCHEMA,
        "task_id": task_id,
        "task_identity": task_identity,
        "environment_identity": environment.get("environment_identity"),
        "environment": dict(environment),
        "decision": decision_trace(decision_packet),
        "economics": dict(economics),
        "decision_packet_identity": digest(decision_packet),
    }
    payload["record_identity"] = digest(payload)
    return payload


def verify_experiment_record(record: Mapping[str, Any]) -> bool:
    if record.get("schema") != EXPERIMENT_RECORD_SCHEMA:
        return False
    claimed = record.get("record_identity")
    payload = dict(record)
