...
def emit(repo: Path, public_path: Path, secret_path: Path, trace_dir: Path) -> list[Path]:
    ...
    with CodeMap(repo) as codemap:
        ...
        for task in public:
            ...
            if bool((packet.get("discrimination") or {}).get("needed")):
                ...
                sequence += 1
            verify = packet.get("verify") if isinstance(packet.get("verify"), dict) else None
...
def main() -> None:
    ...
    print(json.dumps({"traces": len(paths), "trace_dir": str(args.trace_dir)}, sort_keys=True))
