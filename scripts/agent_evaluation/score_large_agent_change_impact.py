...
from scripts.generate_large_impact_corpus import KINDS, generate
...
def _append_probe(path: Path) -> None:
    marker = "# external edit probe\n" if path.suffix == ".py" else "// external edit probe\n"
    path.write_text(path.read_text(encoding="utf-8").rstrip("\n") + "\n" + marker, encoding="utf-8")


def _surface_paths(packet: dict[str, Any], role: str) -> set[str]:
    surfaces = packet.get("surfaces") if isinstance(packet.get("surfaces"), dict) else {}
    rows = surfaces.get(role) if isinstance(surfaces, dict) else []
    return {str(row.get("path")) for row in rows if isinstance(row, dict) and row.get("path")} if isinstance(rows, list) else set()


def run_scenario(root: Path, *, kind: str, noise_files: int, tasks: int) -> dict[str, Any]:
    ...
    manifest = generate(repo, public_path, secret_path, kind=kind, noise_files=noise_files, tasks=tasks)
    public = json.loads(public_path.read_text(encoding="utf-8"))

    frozen: list[dict[str, Any]] = []
    with CodeMap(repo) as codemap:
        started = time.perf_counter(); codemap.sync(); sync_ms = (time.perf_counter() - started) * 1000.0
        for task in public["tasks"]:
            query = str(task["query"]); task_id = str(task["id"])
            start_started = time.perf_counter(); start = codemap.task_evidence(query); start_ms = (time.perf_counter() - start_started) * 1000.0
            edit_path = str(start.get("edit") or "")
            if edit_path:
                _append_probe(repo / edit_path)
                impact_started = time.perf_counter(); impact = codemap.task_change_impact(query, [edit_path]); impact_ms = (time.perf_counter() - impact_started) * 1000.0
            else:
                impact = {}; impact_ms = 0.0
            frozen.append({
                "id": task_id, "query": query, "start": start, "edit": edit_path,
                "impact": impact, "impact_bytes": _bytes(impact), "start_ms": start_ms, "impact_ms": impact_ms,
            })
    ...
    for row in frozen:
        truth = expected[row["id"]]; impact = row["impact"]
        verify = _surface_paths(impact, "verification"); implementation = _surface_paths(impact, "implementation")
        encoded = json.dumps(impact, sort_keys=True, separators=(",", ":"), ensure_ascii=False)
        result = {
            "id": row["id"],
            ...
            "dependency_relevant": str(truth["expected_dependency_path"]) in implementation,
            "no_source_or_argv_replay": '"content"' not in encoded and '"argv"' not in encoded,
            "impact_bytes": int(row["impact_bytes"]), "start_ms": float(row["start_ms"]), "impact_ms": float(row["impact_ms"]),
        }
        result["fully_correct"] = all(bool(result[key]) for key in (
            "edit_correct", "safe_start", "verify_relevant", "dependency_relevant", "no_source_or_argv_replay",
        ))
        graded.append(result)

    return {
        "scenario": scenario, "kind": kind, "noise_files": noise_files,
        "repository_files": sum(1 for path in repo.rglob("*") if path.is_file()),
        "manifest": manifest, "tasks": tasks, "sync_ms": sync_ms,
        ...
        "no_source_or_argv_replay": sum(bool(row["no_source_or_argv_replay"]) for row in graded),
        ...
        "results": graded,
        "protocol": {"secret_join_after_scenario_freeze": True, "external_edit": "syntax-preserving comment", "solution_loop_owner": "external-agent"},
    }
...
def run_matrix(
    ...
    *,
    ...
    sizes = tuple(int(size) for size in sizes); kinds = tuple(kinds)
    scenarios: list[dict[str, Any]] = []
    largest = max(sizes)
    for size in sizes:
        for kind in kinds:
            scenarios.append(run_scenario(root, kind=kind, noise_files=size, tasks=largest_tasks if size == largest and size >= 10000 else regular_tasks))
    total_tasks = sum(row["tasks"] for row in scenarios)
    payload = {
        "schema": SCHEMA,
        "summary": {
            "scenarios": len(scenarios), "tasks": total_tasks,
            ...
            "no_source_or_argv_replay": sum(row["no_source_or_argv_replay"] for row in scenarios),
            "max_repository_files": max(row["repository_files"] for row in scenarios),
            "mean_impact_bytes": statistics.fmean(row["mean_impact_bytes"] for row in scenarios),
        },
        "scenarios": scenarios,
        "scientific_boundary": [
...
def main() -> int:
    ...
    payload = run_matrix(args.root, sizes=sizes, regular_tasks=args.regular_tasks, largest_tasks=args.largest_tasks)
