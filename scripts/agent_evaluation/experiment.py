...
def experiment_manifest(*, corpus_identity: str, lanes: Iterable[ExperimentLane], secret_identity: str | None = None) -> dict[str, Any]:
    ...
    public={"schema":MANIFEST_SCHEMA,"corpus_identity":corpus_identity,"lanes":rows}
...
def load_jsonl(path: Path) -> list[dict[str, Any]]:
    ...
    for number,line in enumerate(path.read_text(encoding="utf-8").splitlines(),1):
        ...
        except json.JSONDecodeError as exc: raise ValueError(f"invalid JSONL at line {number}: {exc.msg}") from exc
...
def import_native_run(*, lane: ExperimentLane, task_id: str, session_id: str, repository_identity: str | None, event_path: Path) -> dict[str, Any]:
    native=load_jsonl(event_path)
    normalized=adapter(lane.harness).normalize_events(native,session_id=session_id,task_id=task_id,repository_identity=repository_identity)
    trace={
    ...
    return {
        ...
        "lane":asdict(lane),
        "native_event_sha256":"sha256:"+hashlib.sha256(event_path.read_bytes()).hexdigest(),
...
def coverage_ledger(*, manifest: dict[str, Any], task_ids: Iterable[str], bundles: Iterable[dict[str, Any]]) -> dict[str, Any]:
    ...
    lane_names=[str(row.get("name")) for row in manifest.get("lanes",[]) if isinstance(row,dict)]
    expected={(lane,task) for lane in lane_names for task in tasks}
    observed:set[tuple[str,str]]=set(); duplicates:set[tuple[str,str]]=set(); invalid=[]
    for bundle in bundles:
        if not isinstance(bundle,dict) or bundle.get("schema") != BUNDLE_SCHEMA:
            invalid.append("unsupported-bundle-schema"); continue
        lane=(bundle.get("lane") or {}).get("name") if isinstance(bundle.get("lane"),dict) else None
        trace=bundle.get("trace") if isinstance(bundle.get("trace"),dict) else {}
        task=trace.get("task_id")
        key=(str(lane),str(task))
        if key not in expected:
            invalid.append(f"unexpected:{key[0]}:{key[1]}"); continue
        if key in observed: duplicates.add(key)
        observed.add(key)
    ...
    return {
        ...
        "duplicates":[{"lane":lane,"task_id":task} for lane,task in sorted(duplicates)],
        "invalid":invalid,
    }
...
def assemble_experiment_report(*, manifest: dict[str, Any], task_ids: Iterable[str], bundles: Iterable[dict[str, Any]], grades: dict[str, bool]) -> dict[str, Any]:
    ...
    if set(grades) != set(task_list):
        missing=sorted(set(task_list)-set(grades)); extra=sorted(set(grades)-set(task_list))
        raise ValueError(f"SECRET grade task set mismatch: missing={missing} extra={extra}")
    lane_meta={str(row['name']):row for row in manifest.get('lanes',[]) if isinstance(row,dict) and 'name' in row}
    by_lane:dict[str,list[dict[str,Any]]]={name:[] for name in lane_meta}
    for bundle in bundle_list:
        lane=str(bundle['lane']['name']); trace=bundle['trace']; task=str(trace['task_id'])
        by_lane[lane].append(trace_economics(trace,GradedRun(task,bool(grades[task]))))
    metrics=[]
    rows={}
    for lane in sorted(by_lane):
        meta=lane_meta[lane]; lane_rows=sorted(by_lane[lane],key=lambda row:str(row['task_id']))
        rows[lane]=lane_rows
        metrics.append(summarize_lane(lane,lane_rows,model=str(meta.get('model') or ''),strategy=str(meta.get('strategy') or '')))
    return {
...
def experiment_certificate(*, manifest: dict[str, Any], bundles: Iterable[dict[str, Any]], report: dict[str, Any], secret_identity: str) -> dict[str, Any]:
    if manifest.get("schema") != MANIFEST_SCHEMA:
        raise ValueError("unsupported experiment manifest schema")
    if report.get("schema") != "hashmarks.agent-experiment-report.v1" or not report.get("complete"):
    ...
    if not isinstance(secret_identity,str) or not secret_identity.startswith("sha256:"):
    ...
    for bundle in bundles:
        ...
        trace_ids.append(trace_id);native_ids.append(native_id)
    payload={
...
def verify_experiment_certificate(*, certificate: dict[str, Any], manifest: dict[str, Any], bundles: Iterable[dict[str, Any]], report: dict[str, Any], secret_identity: str) -> bool:
    expected=experiment_certificate(manifest=manifest,bundles=bundles,report=report,secret_identity=secret_identity)
