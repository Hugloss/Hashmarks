...
ALLOWED_KINDS = {
    "grep", "glob", "repository_search", "file_read",
    "hashmarks_map", "hashmarks_find", "hashmarks_grep", "hashmarks_context", "hashmarks_source",
}
PASSTHROUGH_FIELDS = (
    "query", "path", "paths", "returned_paths", "evidence_paths",
    "bytes", "estimated_tokens", "model_input_tokens", "started_at_ns", "finished_at_ns", "fallback",
)
...
def _validate_event_value(field: str, value: Any, path: Path) -> Any:
    if field in {"bytes", "estimated_tokens", "model_input_tokens", "started_at_ns", "finished_at_ns"}:
...
def load_raw(path: Path) -> dict[str, Any]:
    ...
    for field in (
        "runner_identity", "task_id", "task_revision", "repository_identity",
        "mode", "run_id", "model_identity", "model_config_identity",
    ...
    for event in events:
        ...
        if not isinstance(sequence, int) or isinstance(sequence, bool) or sequence < 0 or sequence <= last_sequence:
            raise ValueError(f"runner event sequence must be strictly increasing non-negative integers: {path}")
        last_sequence = sequence
        tool = _nonempty(event.get("tool"), "event tool", path)
        if tool not in tool_map:
            raise ValueError(f"event tool {tool!r} is not explicitly declared in tool_map: {path}")
...
def normalize(path: Path) -> dict[str, Any]:
    ...
    for event in raw["events"]:
        ...
        if kind is None:
            ignored += 1
            continue
        item: dict[str, Any] = {"kind": kind, "runner_sequence": event["sequence"], "runner_tool": event["tool"]}
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Normalize an explicit runner tool log into hashmarks.agent-trace.v2")
