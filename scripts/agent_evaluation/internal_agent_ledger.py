...
def main():
    ...
    if a.cmd == "search":
        ...
        _event(
            ...
            pattern=a.pattern,
            wall_ms=ms,
            result_bytes=len(out.encode()),
            matches=sum(1 for x in out.splitlines() if x),
        )
        _save(a.state, st)
        print(out, end="")  # noqa: T201 - intentional command output
        return
    if a.cmd == "read":
        path = repo / a.path
        t = time.perf_counter()
        data = path.read_text()
        ms = (time.perf_counter() - t) * 1000
        _event(st, "read", path=a.path, wall_ms=ms, bytes=len(data.encode()))
        _save(a.state, st)
    ...
    if a.cmd in {"packet", "brief"}:
        ...
        t = time.perf_counter()
        p = subprocess.run(
            [sys.executable, "-c", code, str(repo), st["query"], str(a.budget), method],
            env=env,
            text=True,
            capture_output=True,
            check=True,
        ...
        packet = obj["packet"]
        packet_path = a.state.with_suffix("." + a.cmd + ".json")
        ...
        bts = len(json.dumps(shown, sort_keys=True).encode())
        _event(
            st,
    ...
    if a.cmd == "edit":
        ...
        _event(st, "edit", path=a.path, changed=changed)
        _save(a.state, st)
    ...
    if a.cmd == "verify":
        ...
        _event(
            ...
            stderr_bytes=len(p.stderr.encode()),
        )
        _save(a.state, st)
        print(p.stdout, end="")  # noqa: T201 - intentional command output
        print(p.stderr, end="", file=sys.stderr)  # noqa: T201 - intentional command output
    ...
    if a.cmd == "finalize":
        ...
        st["metrics"] = {
            ...
            "search_calls": sum(x["kind"] == "search" for x in ev),
            "read_calls": sum(x["kind"] == "read" for x in ev),
            "repository_read_bytes": sum(
                x.get("bytes", 0) + x.get("result_bytes", 0)
                for x in ev
                if x["kind"] in {"search", "read"}
            ),
            "hashmarks_visible_bytes": sum(
                x.get("visible_bytes", 0)
                for x in ev
                if x["kind"] in {"hashmarks_packet", "hashmarks_brief"}
            ),
            "verification_attempts": sum(x["kind"] == "verification" for x in ev),
            "failed_verifications": sum(
                x["kind"] == "verification" and not x.get("passed") for x in ev
            ),
            "wall_ms": (st["ended_ns"] - st["started_ns"]) / 1e6,
        }
        frozen = dict(st)
        frozen.pop("identity", None)
        st["identity"] = _sha(frozen)
        _save(a.state, st)
        a.output.parent.mkdir(parents=True, exist_ok=True)
        a.output.write_text(json.dumps(st, indent=2, sort_keys=True) + "\n")
        print(st["identity"])  # noqa: T201 - intentional command output
        return
