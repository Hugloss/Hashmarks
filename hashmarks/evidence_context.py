...
def _digest(payload: object) -> str:
    ...
    return "sha256:" + hashlib.sha256(EVIDENCE_CONTEXT_SCHEMA.encode("utf-8") + b"\0" + encoded).hexdigest()
...
def evidence_context_identity(
    ...
    *,
    ...
    producer = producer_implementation_identity or native_producer_implementation_identity()
    ...
    generation = require_generation(evidence_receipt.get("codemap_generation"), field="codemap_generation")
    revision = provenance.get("revision")
    try:
        json.dumps(revision, sort_keys=True, separators=(",", ":"), ensure_ascii=False, allow_nan=False)
