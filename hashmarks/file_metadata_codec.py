...
_METADATA_OVERFLOW_MAX_BYTES = (_METADATA_INTEGER_MAX_CHARS * _METADATA_FIELD_COUNT) + (_METADATA_FIELD_COUNT - 1)
...
def encode_metadata_fields(values: tuple[int, int, int, int, int]) -> tuple[tuple[int, int, int, int, int], bytes | None]:
...
def _overflow_parts(value: object) -> list[str]:
    if not isinstance(value, bytes) or not value or len(value) > _METADATA_OVERFLOW_MAX_BYTES:
        raise MetadataCodecError("persisted filesystem metadata overflow payload is invalid")
    try:
        parts = value.decode("ascii").split(",")
    except UnicodeDecodeError as exc:
        raise MetadataCodecError("persisted filesystem metadata overflow payload is not ASCII") from exc
    if len(parts) != _METADATA_FIELD_COUNT:
        raise MetadataCodecError("persisted filesystem metadata overflow payload has invalid field count")
    return parts


def _decode_integer(text: str) -> int:
    if not text or len(text) > _METADATA_INTEGER_MAX_CHARS:
        raise MetadataCodecError("persisted filesystem metadata integer has invalid length")
    try:
        number = int(text)
    except ValueError as exc:
        raise MetadataCodecError("persisted filesystem metadata is not an integer") from exc
    if str(number) != text:
...
def decode_overflow_metadata(value: object) -> tuple[int, int, int, int, int]:
    ...
    if all(_is_sqlite_integer(number) for number in decoded):
        raise MetadataCodecError("overflow payload must contain an out-of-range integer")
