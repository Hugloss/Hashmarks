...
class RelationshipsMixin:
    ...
    def _validate_adjacency_bounds(
        seed_limit: int, limit: int, per_seed_edge_limit: int, per_seed_result_limit: int
    ) -> None:
        if seed_limit < 1:
    ...
    def _adjacency_relation_weights() -> dict[str, float]:
        return {
            "imports": 42.0, "native-import": 44.0, "call": 38.0, "return-type": 36.0,
            "parameter-type": 34.0, "inherits": 40.0, "attribute-type": 32.0,
        }
    ...
    def _adjacency_admit(
        state: _TaskAdjacencyState, path: str, seed_rank: int, seed_path: str, relation: str, confidence: str
    ) -> None:
        if path and path not in state.primary_paths:
            state.pending_admissions.append((path, seed_rank, seed_path, relation, confidence))

    def _adjacency_import_edge(
        self, state: _TaskAdjacencyState, target: str, seed_rank: int, seed_path: str, confidence: str
    ) -> None:
        for path in self._resolve_import_paths(seed_path, target)[:4]:
            self._adjacency_admit(state, path, seed_rank, seed_path, "imports", confidence)
    ...
    def _adjacency_static_edge(
        ...
        if short is not None:
            state.symbolic_targets.setdefault(short.lower(), []).append((seed_rank, seed_path, kind, confidence))

    def _collect_adjacency_seeds(self, state: _TaskAdjacencyState, primary, per_seed_edge_limit: int) -> None:
        ...
        for seed_rank, seed in enumerate(primary, 1):
            ...
            for edge in self._fresh_native_file_edges_from(seed.path)[:per_seed_edge_limit]:
                target = str(edge.get("target") or "")
                if target:
                    confidence = str(edge.get("confidence") or "native")
                    self._adjacency_admit(state, target, seed_rank, seed.path, "native-import", confidence)
    ...
    def _admit_resolved_adjacency_alias(
        ...
        for row in target_rows:
            ...
            for seed_rank, seed_path, relation, confidence in matched[:8]:
                self._adjacency_admit(state, path, seed_rank, seed_path, relation, confidence)
    ...
    def _resolve_adjacency_symbols(self, state: _TaskAdjacencyState) -> None:
        ...
        resolved = self._session_exact_symbol_candidates(
            sorted(state.symbolic_targets), limit=max(160, min(5000, len(state.symbolic_targets) * 32)),
        )
    ...
    def _score_adjacency_row(
        ...
        candidate: tuple[str, int, str, str, str], file_row: Mapping[str, object],
    ) -> None:
        ...
        base_score = self._score(
            task, {"row_type": "file", **row}, tokens=state.tokens, raw_query=state.raw_query,
        )
        score = base_score + state.relation_weight.get(relation, 20.0) + (28.0 / (1.0 + seed_rank))
        provenance = {
            "seed_path": seed_path, "seed_rank": seed_rank,
        ...
        if isinstance(existing, list) and provenance not in existing and len(existing) < 4:
            existing.append(provenance)

    def _score_adjacency_admissions(self, task: str, state: _TaskAdjacencyState) -> None:
    ...
    @classmethod
    def _seed_adjacency_rows(cls, primary, ordered, per_seed_result_limit: int) -> list[dict[str, object]]:
        result: list[dict[str, object]] = []
        for seed_rank, seed in enumerate(primary, 1):
            local = [candidate for candidate in ordered if cls._candidate_matches_seed(candidate, seed_rank)]
            if local:
                result.append({
                    "seed_path": seed.path, "seed_rank": seed_rank,
                    "adjacent": local[:per_seed_result_limit],
                })
        return result

    def _path_graph_adjacency(
        self, task: str, paths: set[str], *, limit: int = 12,
        per_seed_edge_limit: int = 96, per_seed_result_limit: int = 3,
    ) -> dict[str, object]:
        ...
        for path in sorted(paths):
            ...
            if visibility is EvidenceVisibility.DENY:
                continue
            primary.append(SearchHit(path=path, score=100.0, kind="file", evidence_visibility=visibility))
        state = _TaskAdjacencyState(
            {hit.path for hit in primary}, _query_terms(task), task.strip().lower(),
        ...
        ordered = sorted(state.candidates.values(), key=lambda row: (-float(row["score"]), str(row["path"])))
        return {
            "schema": "hashmarks.task-graph-adjacency.v1", "task": task,
            "generation": self.store.generation(),
            "bounds": {
                "seed_limit": len(primary), "limit": limit,
                "per_seed_edge_limit": per_seed_edge_limit,
                "per_seed_result_limit": per_seed_result_limit, "max_hops": 1,
            },
            "primary": [hit.as_dict() for hit in primary], "adjacent": ordered[:limit],
            "seed_adjacency": self._seed_adjacency_rows(primary, ordered, per_seed_result_limit),
        }

    def task_graph_adjacency(
        self, task: str, *, seed_limit: int = 20, limit: int = 12,
        per_seed_edge_limit: int = 96, per_seed_result_limit: int = 3,
    ) -> dict[str, object]:
        """Return bounded one-hop graph evidence adjacent to canonical task hits."""
        self._validate_adjacency_bounds(seed_limit, limit, per_seed_edge_limit, per_seed_result_limit)
        ...
        return {
            ...
            "generation": self.store.generation(),
            "bounds": {
                "seed_limit": seed_limit, "limit": limit, "per_seed_edge_limit": per_seed_edge_limit,
                "per_seed_result_limit": per_seed_result_limit, "max_hops": 1,
            },
            "primary": [hit.as_dict() for hit in primary], "adjacent": ordered[:limit],
            "seed_adjacency": self._seed_adjacency_rows(primary, ordered, per_seed_result_limit),
    ...
    @staticmethod
    def _validate_relationship_bounds(seed_limit: int, adjacency_limit: int, impact_limit_per_surface: int) -> None:
        if seed_limit < 1:
            raise ValueError("seed_limit must be >= 1")
    ...
    def _relationship_impact_surfaces(
        ...
        try:
            impact = self.change_impact(task, max_depth=2, limit_per_surface=impact_limit_per_surface)
        except KeyError:
            return {}, "no-proven-impact-root"
        return impact["surfaces"], None  # type: ignore[return-value]

    @staticmethod
    def _append_relationship(relationships, seen, source: str, target: str, relation: str, metadata: dict[str, object]) -> None:
        ...
        relationships.append({
            ...
            "layer": metadata["layer"], "provenance": metadata["provenance"],
        })

    @classmethod
    def _append_adjacency_provenance(cls, relationships, seen, target: str, item: object) -> None:
        if not isinstance(item, dict):
            return
        cls._append_relationship(
            relationships, seen, str(item.get("seed_path") or ""), target,
            str(item.get("relation") or "related"), {"layer": "code-graph", "provenance": item},
        )
    ...
    @classmethod
    def _relationship_impact_edges(cls, surfaces, primary_paths, relationships, seen) -> None:
        for surface, rows in surfaces.items():
            ...
            for row in rows:
                ...
                target = str(row.get("path") or "")
                provenance = {
                    "surface": surface, "depth": row.get("depth"),
                    "source": row.get("provenance"), "relation": row.get("relation"),
                }
                for source in primary_paths[:4]:
                    cls._append_relationship(
                        relationships, seen, source, target, f"affects-{surface}",
                        {"layer": "change-impact", "provenance": provenance},
                    )

    def _relationship_authority_edges(self, primary_paths, relationships, seen) -> list[dict[str, object]]:
        ...
        for path in primary_paths[:4]:
            ...
            for row in value["authority"]:
                ...
                authority_rows.append({
                    "target_path": path, "authority_path": authority_path,
                    "kind": row.get("kind"), "specificity": row.get("specificity"),
                })
                self._append_relationship(
                    relationships, seen, authority_path, path, "governs",
                    {
                        "layer": "authority",
                        "provenance": {
                            "kind": row.get("kind"), "specificity": row.get("specificity"),
    ...
    def _relationship_layer_counts(relationships) -> dict[str, int]:
        ...
        for row in relationships:
            layer = str(row["layer"]); counts[layer] = counts.get(layer, 0) + 1
        return counts

    def task_relationships(
        self, task: str, *, seed_limit: int = 12, adjacency_limit: int = 18, impact_limit_per_surface: int = 12
    ) -> dict[str, object]:
        """Return bounded cross-layer repository relationships for one task."""
        self._validate_relationship_bounds(seed_limit, adjacency_limit, impact_limit_per_surface)
        primary = self.find_task(task, limit=seed_limit)
        primary_paths = [hit.path for hit in primary]
        adjacency = self.task_graph_adjacency(task, seed_limit=seed_limit, limit=adjacency_limit)
        impact_surfaces, impact_error = self._relationship_impact_surfaces(task, impact_limit_per_surface)
        relationships: list[dict[str, object]] = []
        seen: set[tuple[str, str, str]] = set()
        self._relationship_adjacency_edges(adjacency, relationships, seen)
        self._relationship_impact_edges(impact_surfaces, primary_paths, relationships, seen)
        authority_rows = self._relationship_authority_edges(primary_paths, relationships, seen)
        return {
            "schema": "hashmarks.task-relationships.v1", "task": task, "generation": self.store.generation(),
            "primary": [hit.as_dict() for hit in primary], "relationships": relationships,
            "authority": authority_rows, "layer_counts": self._relationship_layer_counts(relationships),
            "impact_status": "available" if impact_error is None else impact_error,
            "bounds": {
                "seed_limit": seed_limit, "adjacency_limit": adjacency_limit,
                "impact_limit_per_surface": impact_limit_per_surface, "impact_max_depth": 2,
                "adjacency_max_hops": 1, "authority_seed_limit": 4,
            },
            "ranking_effect": "none", "authority_effect": "advisory-navigation-only",
        }
