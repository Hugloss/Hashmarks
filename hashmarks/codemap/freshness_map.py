...
class EvidenceFreshnessMapMixin:
    ...
    def _freshness_identity(self, kind: str, payload: object) -> str:
        return "sha256:" + self._packet_digest(
            f"hashmarks.evidence-freshness.{kind}.v1", payload,
        )
    ...
    def _freshness_changed_revisions(
        self, changed_rows: Sequence[object],
    ...
    @staticmethod
    def _prior_entries(previous_map: Mapping[str, object] | None) -> list[Mapping[str, object]]:
    ...
    def evidence_freshness_map(
        ...
        *,
        ...
        selected_explanation = self.explain_verification_selection(
            task, selected_member or None, limit=limit, candidate_limit=16,
        )
        ...
        entries.append({
            ...
            "evidence_identity": self._freshness_identity("ownership", ownership_fact),
            "facts": ownership,
        })
        ...
        entries.append({
            ...
            "state": continuity_state,
            "evidence_identity": self._freshness_identity("verification-membership", verification_fact),
            "member": selected_member or None,
            "reason": selected_explanation.get("reason"),
        })
        ...
        for raw_member in negative_members:
            ...
            explanation = self.explain_verification_selection(
                task, member, limit=limit, candidate_limit=16,
            )
            ...
            if status == "selected":
                ...
                state = "unknown" if continuity_state != "invalidated" else "invalidated"
            ...
            entries.append({
                ...
                "member": member,
                "state": state,
                "evidence_identity": self._freshness_identity("negative-verification", negative_fact),
                "reason": reason,
            })
        ...
        if isinstance(project_impact, Mapping):
            ...
            entries.append({
                ...
                "evidence_identity": self._freshness_identity("cross-repository", project_fact),
                "dependencies": dependency_rows,
            })
        ...
        for old in self._prior_entries(previous_map):
            ...
            current_identity = str(current.get("evidence_identity") or "") if current else ""
            if current is None:
                if key[0] in {"ownership", "impact", "verification-membership", "cross-repository"}:
                    state = "invalidated"
        ...
        if prior:
            payload["prior"] = prior
        payload["freshness_map_identity"] = self._freshness_identity("map", {
            "repository_identity": repository_identity,
            "task_identity": task_identity,
            "entries": entries,
        })
