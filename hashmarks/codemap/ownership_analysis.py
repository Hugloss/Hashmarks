...
class OwnershipAnalysisMixin:
    ...
    def _import_ownership_findings(
        ...
        python_paths = {str(row["path"]) for row in all_rows if str(row.get("language") or "") == "python"}
        if paths is not None:
            candidates = []
            for rel in paths:
            ...
            candidates = [str(row["path"]) for row in rows if str(row.get("language") or "") == "python"]
        ...
        for rel in sorted(set(candidates)):
            ...
            findings.extend(
                ...
                for finding in analyze_python_import_ownership(
                    path=rel, source=snapshot.source, repository_paths=repository_paths, tree=snapshot.tree
                )
            )
    ...
    def import_ownership_findings(self, paths: Sequence[str] | None = None) -> dict[str, object]:
    ...
    @staticmethod
    def _repository_finding_from_import(finding: dict[str, object]) -> dict[str, object]:
        return {
    ...
    @staticmethod
    def _repository_finding_from_concurrency(finding: dict[str, object]) -> dict[str, object]:
        return {
    ...
    @staticmethod
    def _repository_finding_from_cache_owner(owner: dict[str, object]) -> dict[str, object]:
        return {
    ...
    def repository_findings(self, paths: Sequence[str] | None = None) -> dict[str, object]:
        ...
        repository_import_result = (
            import_result if normalized is None else self._import_ownership_findings(None, all_rows)
        )
        cache_result = self._cache_ownership_findings(normalized, all_rows, repository_import_result)
        concurrency_result = self._concurrency_risk_findings(normalized, all_rows)

        findings = [
            *(self._repository_finding_from_import(item) for item in import_result["findings"]),
            *(
    ...
    def _concurrency_risk_findings(
        ...
        if paths is None:
            rows=self.store.lexical_file_candidates(
                ["read","get","generation","write","set","update","commit","execute"],
                limit=max(256,min(10_000,len(python_paths) or 256)))
            candidates=[
                str(row["path"]) for row in rows
                if str(row.get("language") or "")=="python"
                and RepositoryDomain.TEST not in set(classify_repository_path(str(row["path"])))
            ]
        else:
            candidates=[]
            for rel in paths:
                if rel in python_paths:candidates.append(rel)
        findings=[]
        for rel in sorted(set(candidates)):
            try:snapshot=read_python_ast(self.workspace / rel, errors="replace")
            except (OSError,UnicodeError,SyntaxError,ValueError):continue
            findings.extend(x.as_dict() for x in analyze_python_concurrency_risk(
                path=rel, source=snapshot.source, tree=snapshot.tree
            ))
        generation,identity_generation,stale=self._generation_status()
        return {"schema":"hashmarks.concurrency-risk.v1","generation":generation,
                "identity_generation":identity_generation,"stale":stale,
                "summary":{"files_considered":len(set(candidates)),"sequences":len(findings),
    ...
    def concurrency_risk_findings(self, paths: Sequence[str] | None = None) -> dict[str, object]:
        self._ensure_map_ready()
        normalized, all_rows = self._ownership_analysis_inputs(paths)
    ...
    def _python_module_for_path(self, path: str) -> str | None:
        ...
        if len(parts)>1:
            ...
            for package in parts[:-1]:
                ...
                if not ((self.workspace/current/"__init__.py").is_file() or (self.workspace/current/"__init__.pyi").is_file()):
                    return None
        return ".".join(parts)


    def _reader_imports_cache_owner(self, reader: str, owner_path: str, owner_name: str) -> bool:
    ...
    def _ownership_add_imports(nodes, edges, findings) -> None:
        for finding in findings:
            ...
            nodes[("file", source, source)] = {"id": f"file:{source}", "kind": "file", "path": source, "role": "loader"}
            if not target:
                continue
            target = str(target)
            nodes[("file", target, target)] = {"id": f"file:{target}", "kind": "file", "path": target, "role": "module-owner"}
            edges.append({
                "source": f"file:{source}", "target": f"file:{target}", "relation": "loads-module",
                "confidence": finding["confidence"], "risk": finding["code"],
            })
    ...
    def _ownership_add_cache_owners(nodes, edges, owners) -> None:
        for owner in owners:
            path = str(owner["path"]); name = str(owner["owner"])
            nodes.setdefault(("file", path, path), {"id": f"file:{path}", "kind": "file", "path": path, "role": "source"})
            nodes[("cache", path, name)] = {
                "id": f"cache:{path}:{name}", "kind": "cache", "path": path, "owner": name,
                "scope": owner["scope"], "invalidation": owner["invalidation"],
            }
            edges.append({
                "source": f"file:{path}", "target": f"cache:{path}:{name}", "relation": "owns-cache",
                "confidence": owner["confidence"],
                "risk": "duplicate-module-identity" if owner["import_identity_risk"] else None,
            })
    ...
    def _cache_owner_name_counts(owners) -> dict[str, int]:
        counts: dict[str, int] = {}
        for owner in owners:
            name = str(owner["owner"]); counts[name] = counts.get(name, 0) + 1
        return counts


    def _ownership_add_cache_readers(self, nodes, edges, owners, selected: set[str]) -> None:
        counts = self._cache_owner_name_counts(owners)
        for owner in owners:
            path = str(owner["path"]); name = str(owner["owner"]); duplicate = counts.get(name, 0) > 1
            for ref in self.store.refs(name, limit=32):
                reader = str(ref["path"])
                if reader == path:
                    continue
                outside_selection = bool(selected) and {reader, path}.isdisjoint(selected)
                if outside_selection:
                    continue
                if duplicate and not self._reader_imports_cache_owner(reader, path, name):
                    continue
                nodes.setdefault(("file", reader, reader), {"id": f"file:{reader}", "kind": "file", "path": reader, "role": "reader"})
                edges.append({
                    "source": f"file:{reader}", "target": f"cache:{path}:{name}",
                    "relation": "references-cache-owner",
                    "confidence": "qualified-import" if duplicate else "lexical-symbol", "risk": None,
                })


    @staticmethod
    def _ownership_add_invalidations(nodes, edges, invalidation_result) -> tuple[set[str], set[str]]:
        cache_ids = {str(node["id"]) for node in nodes.values() if node.get("kind") == "cache"}
        resolved: set[str] = set()
        invalidation_nodes = {str(node["id"]): node for node in invalidation_result["nodes"]}
        for edge in invalidation_result["edges"]:
            target = str(edge["target"])
            if target not in cache_ids:
                continue
            source = str(edge["source"]); invalidator = invalidation_nodes.get(source)
            if invalidator is not None:
                nodes.setdefault(("invalidator", source, source), dict(invalidator))
            edges.append(dict(edge)); resolved.add(target)
    ...
    def _ownership_add_concurrency(nodes, edges, findings) -> int:
        unguarded = 0
        for finding in findings:
            path = str(finding["path"]); function = str(finding["function"]); line = int(finding["line"])
            nodes.setdefault(("file", path, path), {"id": f"file:{path}", "kind": "file", "path": path, "role": "source"})
            rid = f"risk:{path}:{function}:{line}"
            nodes[("concurrency-risk", rid, rid)] = {
                "id": rid, "kind": "concurrency-risk", "path": path, "function": function, "line": line,
                "code": finding["code"], "guarded": bool(finding["guarded"]),
                "read_call": finding["read_call"], "write_call": finding["write_call"],
                "confidence": finding["confidence"],
            }
            edges.append({
                "source": f"file:{path}", "target": rid, "relation": "contains-concurrency-risk",
                "confidence": finding["confidence"], "risk": finding["code"],
                "guarded": bool(finding["guarded"]), "line": line,
            })
            unguarded += int(not bool(finding["guarded"]))
        return unguarded


    def repository_ownership_graph(self, paths: Sequence[str] | None = None) -> dict[str, object]:
        ...
        repository_cache_result = (
            ...
            else self._cache_ownership_findings(None, all_rows, repository_import_result)
        )
        ...
        self._ownership_add_cache_readers(nodes, edges, cache_result["owners"], {str(x) for x in (paths or [])})
        cache_ids, resolved = self._ownership_add_invalidations(nodes, edges, invalidation_result)
        unguarded = self._ownership_add_concurrency(nodes, edges, concurrency_result["findings"])
        ambiguous = sorted(cache_ids - resolved)
        generation, identity_generation, stale = self._generation_status()
        return {
            "schema": "hashmarks.authority-ownership-graph.v3",
            "generation": generation, "identity_generation": identity_generation, "stale": stale,
            "summary": {
                "nodes": len(nodes), "edges": len(edges), "resolved_invalidation_owners": len(resolved),
            ...
            "edges": sorted(edges, key=lambda x: (str(x["source"]), str(x["target"]), str(x["relation"]), int(x.get("line", 0) or 0))),
    ...
    def _cache_owner_modules(
        ...
        for owner in cache_result["owners"]:
            path = str(owner["path"]); module = self._python_module_for_path(path)
            if not module:
                continue
            enriched = dict(owner); enriched["module"] = module; owners.append(enriched)
    ...
    def _cache_invalidator_findings(self, candidates: list[str], owners: list[dict[str, object]]) -> list[dict[str, object]]:
        findings: list[dict[str, object]] = []
        for rel in candidates:
            ...
            current_package = module if path_obj.stem == "__init__" else (module.rsplit(".", 1)[0] if "." in module else None)
            try:
                snapshot = read_python_ast(self.workspace / rel, errors="replace")
            except (OSError, UnicodeError, SyntaxError, ValueError):
                continue
            findings.extend(item.as_dict() for item in analyze_python_cache_invalidators(
                path=rel, source=snapshot.source, current_module=module, current_package=current_package,
                cache_owners=owners, tree=snapshot.tree
            ))
        return findings
    ...
    def _cache_owner_nodes(owners, owner_ids) -> dict[str, dict[str, object]]:
        ...
        for owner in owners:
            ...
            nodes[cid] = {
                "id": cid, "kind": "cache", "path": owner["path"], "module": owner["module"],
                "owner": owner["owner"], "scope": owner["scope"], "invalidation": owner["invalidation"],
            }
        return nodes


    @staticmethod
    def _cache_invalidation_edges(findings, owner_ids, nodes) -> tuple[list[dict[str, object]], set[str]]:
        edges: list[dict[str, object]] = []; invalidated: set[str] = set()
        for finding in findings:
            source_id = f"invalidator:{finding['path']}:{finding['invalidator']}"
            target_id = owner_ids.get((str(finding["target_module"]), str(finding["target_owner"])))
            if target_id is None:
                continue
            nodes.setdefault(source_id, {
                "id": source_id, "kind": "invalidator", "path": finding["path"], "owner": finding["invalidator"],
            })
            edges.append({
                "source": source_id, "target": target_id, "relation": "invalidates-cache",
                "method": finding["method"], "confidence": finding["confidence"], "line": finding["line"],
            })
    ...
    def _cache_invalidation_ownership_graph(
        ...
        return {
            ...
            "generation": generation, "identity_generation": identity_generation, "stale": stale,
            "summary": {
                "files_considered": len(candidates), "cache_owners": len(owners),
                "invalidators": sum(1 for node in nodes.values() if node.get("kind") == "invalidator"),
                "invalidation_edges": len(edges), "owners_without_resolved_invalidator": len(unresolved),
            },
            "nodes": sorted(nodes.values(), key=lambda node: str(node["id"])),
            "edges": sorted(edges, key=lambda edge: (str(edge["source"]), str(edge["target"]), int(edge["line"]))),
            "unresolved_cache_owners": unresolved,
            "boundary": "repository-evidence-only; cache mutation/execution authority remains external",
        }

    def cache_invalidation_ownership_graph(self, paths: Sequence[str] | None = None) -> dict[str, object]:
    ...
    def _cache_ownership_findings(
        ...
        if paths is None:
            rows = self.store.lexical_file_candidates(
                ["cache", "memo", "registry", "singleton", "pool", "lru_cache", "cached_property"],
                limit=max(256, min(10_000, len(python_paths) or 256)),
            )
            candidates = [str(row["path"]) for row in rows if str(row.get("language") or "") == "python"]
        else:
            ...
            for rel in paths:
                if rel in python_paths or (self.workspace / rel).suffix in {".py", ".pyi"}:
                    candidates.append(rel)
        risky_targets = {
            str(f["target_path"]) for f in import_result["findings"]
            if f.get("target_path") and f.get("code") in {"python-dynamic-module-identity-bypass", "python-duplicate-module-identity"}
        }
        findings: list[dict[str, object]] = []
        for rel in sorted(set(candidates)):
            try:
                ...
                continue
            findings.extend(item.as_dict() for item in analyze_python_cache_ownership(
                path=rel, source=snapshot.source, import_risk_targets=risky_targets, tree=snapshot.tree
            ))
        generation, identity_generation, stale = self._generation_status()
        ...
        return {
            ...
            "generation":generation,"identity_generation":identity_generation,"stale":stale,
            "summary":{"files_considered":len(set(candidates)),"owners":len(findings),
                       "import_identity_risks":risky,"invalidation_not_proven":unresolved},
            "owners":findings,
        }

    def cache_ownership_findings(self, paths: Sequence[str] | None = None) -> dict[str, object]:
        self._ensure_map_ready()
        normalized, all_rows = self._ownership_analysis_inputs(paths)
