...
class OwnershipGraphMixin:
    ...
    def _entry_role_domains() -> dict[str, tuple[RepositoryDomain, ...]]:
        return {
            ...
            "config_build": (RepositoryDomain.CONFIG, RepositoryDomain.BUILD, RepositoryDomain.PLAN),
    ...
    def _entry_role_symbol_specificity(self, role: str, path: str, task_terms: set[str]) -> int:
    ...
    def _entry_role_quality(role: str, domains: tuple[RepositoryDomain, ...]) -> int:
        ...
        if role == "contract":
            if RepositoryDomain.DOC not in values:
                return 0
            stronger = {RepositoryDomain.CONFIG, RepositoryDomain.SOURCE, RepositoryDomain.SCRIPT, RepositoryDomain.OWNERSHIP}
    ...
    @staticmethod
    def _entry_role_match(role: str, accepted, domains: tuple[RepositoryDomain, ...], path: str) -> bool:
    ...
    def _entry_role_rows(
        self, hits, role_domains: dict[str, tuple[RepositoryDomain, ...]], task_terms: set[str], per_role: int
    ) -> dict[str, list[dict[str, object]]]:
        candidates: dict[str, list[dict[str, object]]] = {role: [] for role in role_domains}
        for rank, hit in enumerate(hits, 1):
            ...
            for role, accepted in role_domains.items():
                ...
                candidates[role].append({
                    "path": hit.path, "canonical_rank": rank, "domains": domain_values,
                    "role_quality": self._entry_role_quality(role, domains),
                    "role_symbol_specificity": self._entry_role_symbol_specificity(role, hit.path, task_terms),
                })
        role_rows: dict[str, list[dict[str, object]]] = {}
        for role, rows in candidates.items():
            rows.sort(key=lambda row: (
                int(row["role_quality"]), -int(row["role_symbol_specificity"]),
                int(row["canonical_rank"]), str(row["path"]),
            ))
    ...
    def _entry_intent_roles() -> dict[str, tuple[str, ...]]:
        return {
            "config": ("config_build", "contract", "verification", "implementation", "orientation"),
            "test": ("verification", "config_build", "implementation", "contract", "orientation"),
            "conceptual": ("authority", "contract", "orientation", "implementation", "verification", "config_build"),
            "relationship": ("implementation", "contract", "verification", "config_build", "orientation"),
            "structural": ("implementation", "contract", "verification", "config_build", "orientation"),
            "identifier": ("implementation", "contract", "verification", "config_build", "orientation"),
            "path": ("implementation", "config_build", "contract", "verification", "orientation"),
            "hybrid": ("implementation", "contract", "config_build", "verification", "authority", "orientation"),
        }
    ...
    def _entry_domain_roles() -> dict[RepositoryDomain, str]:
        return {
            RepositoryDomain.OWNERSHIP: "authority", RepositoryDomain.CONTRACT: "contract",
            RepositoryDomain.CONFIG: "config_build", RepositoryDomain.BUILD: "config_build",
            RepositoryDomain.PLAN: "config_build", RepositoryDomain.TEST: "verification",
            RepositoryDomain.SOURCE: "implementation", RepositoryDomain.SCRIPT: "implementation",
            RepositoryDomain.ARCHITECTURE: "orientation", RepositoryDomain.DOC: "orientation",
        }
    ...
    @staticmethod
    def _entry_low_confidence_roles(preferred: list[str], defaults: list[str]) -> list[str]:
    ...
    @staticmethod
    def _entry_recommended(role_rows: dict[str, list[dict[str, object]]], ordered_roles: list[str]) -> list[dict[str, object]]:
        ...
        for role in ordered_roles:
            for row in role_rows[role]:
                ...
                if path in seen_paths:
                    continue
                seen_paths.add(path); recommended.append({"role": role, **row}); break
        return recommended
    ...
    def _entry_role_cues() -> dict[str, frozenset[str]]:
        return {
            "authority": frozenset({"agent", "agents", "authority", "authoritative", "owner", "owners", "ownership", "govern", "governs"}),
            "contract": frozenset({"contract", "contracts", "schema", "schemas", "policy", "policies", "invariant", "invariants", "requirement", "requirements"}),
            "config_build": frozenset({"build", "config", "configuration", "environment", "manifest", "makefile", "package", "packaging", "release", "setting", "settings", "toml", "vite", "yaml", "yml"}),
            "verification": frozenset({"coverage", "jest", "pytest", "spec", "specs", "test", "tests", "testing", "vitest"}),
            "implementation": frozenset({"class", "code", "function", "implementation", "method", "source", "symbol"}),
            "orientation": frozenset({"architecture", "design", "doc", "docs", "documentation", "overview", "readme"}),
        }


    @classmethod
    def _entry_explicit_roles(cls, task: str, role_rows: dict[str, list[dict[str, object]]]) -> list[str]:
        cue_words = {value.lower() for value in _WORD_RE.findall(task)}
        return [
            role for role, cues in cls._entry_role_cues().items()
    ...
    def _entry_distinctive_tokens(cls, task: str) -> list[str]:
        return [token.lower() for token in _WORD_RE.findall(task) if cls._is_distinctive_entry_token(token)]


    def _entry_row_text(self, row: dict[str, object]) -> tuple[str, str]:
        path = str(row.get("path") or "")
        symbol_text = " ".join(
            " ".join(str(symbol.get(key) or "").lower() for key in ("name", "qualname", "signature"))
            for symbol in self._session_symbols_for_path(path)
        )
        row_text = " ".join([
            *(str(row.get(key) or "").lower() for key in ("path", "name", "qualname", "signature")),
            symbol_text,
        ])
    ...
    def _verification_anchor_tokens(matches: list[str]) -> list[str]:
        return [token for token in matches if token.startswith("test_") or token.endswith("_test")]


    @staticmethod
    def _implementation_anchor_tokens(matches: list[str]) -> list[str]:
        return [token for token in matches if not token.startswith("test_") and not token.endswith("_test")]


    @classmethod
    def _entry_role_anchor_filter(cls, role: str, path: str, matches: list[str]) -> list[str]:
    ...
    @staticmethod
    def _entry_ambiguity_reason(explicit_roles: list[str], resolved_role: str | None, ambiguous: bool) -> str:
    ...
    def _entry_resolution(
        ...
        if resolved_role is not None:
            return {
                "status": "resolved", "role": resolved_role,
    ...
    def _entry_ambiguity(
        ...
        alternatives.sort(key=lambda row: (int(row["canonical_rank"]), str(row["role"]), str(row["path"])))
        ...
        return {
            "schema": "hashmarks.entry-point-ambiguity.v2", "ambiguous": ambiguous,
            "reason": self._entry_ambiguity_reason(explicit_roles, resolved_role, ambiguous),
            "explicit_roles": explicit_roles, "alternatives": alternatives,
            "resolution": self._entry_resolution(ambiguous, resolved_role, role_rows, role_anchor_tokens),
    ...
    def task_entry_points(
        ...
        role_rows = self._entry_role_rows(hits, self._entry_role_domains(), set(_query_terms(task)), per_role)
        recommended = self._entry_recommended(role_rows, self._entry_ordered_roles(route))
        return {
            "schema": "hashmarks.task-entry-points.v2", "task": task, "intent": route.intent.value,
            "confidence": route.confidence,
            "preferred_domains": [domain.value for domain in route.preferred_domains],
            "canonical": [hit.as_dict() for hit in hits], "roles": role_rows, "recommended": recommended,
            "ambiguity": self._entry_ambiguity(task, role_rows),
            "bounds": {"limit": limit, "per_role": per_role},
            "ranking_effect": "none", "discovery_effect": "none",
        }
    ...
    def _ownership_evidence_visible(self, path: str) -> bool:
        ...
        if file_row is None:
            return False
        try:
            return EvidenceVisibility(str(file_row["evidence_visibility"])) is not EvidenceVisibility.DENY
        except ValueError:
            return False


    def _ownership_node(self, state: _OwnershipGraphState, path: str) -> dict[str, object]:
        ...
        for symbol in symbols[:12]:
            symbol_terms.update(_query_terms(str(symbol.get("qualname") or symbol.get("name") or "")))
        path_terms = set(_query_terms(path.replace("/", " ")))
        current = {
            "path": path, "domains": domains,
            "task_locality_terms": sorted(state.task_terms.intersection(path_terms | symbol_terms)),
        }
        state.nodes[path] = current
        return current


    def _python_ownership_import_path(self, source_path: str, target: str) -> str | None:
    ...
    def _visible_resolved_import_paths(self, source_path: str, target: str) -> list[str]:
        return [candidate for candidate in self._resolve_import_paths(source_path, target) if self._ownership_evidence_visible(candidate)]
    ...
    def _exact_ownership_import_paths(self, source_path: str, target: str) -> list[str]:
        ...
        if self._decision_session_depth > 0 and self._decision_session_generation is not None:
        ...
        result = [] if unresolved else [
            path for path in resolved if self._ownership_evidence_visible(path)
        ]
        if cache_key is not None:
            self._decision_ownership_import_paths_cache[cache_key] = tuple(result)
        return result
    ...
    def _ownership_call_shorts(expandable, edge_map) -> list[str]:
        return sorted({
            ...
            if str(edge.get("kind") or "") == "call" and str(edge.get("target_short") or "")
        })


    @staticmethod
    def _index_ownership_symbols(call_symbols, wanted: set[str]) -> dict[str, list[dict]]:
    ...
    def _ownership_symbol_map(self, expandable, edge_map) -> dict[str, list[dict]]:
        ...
        return self._index_ownership_symbols(call_symbols, {value.lower() for value in call_shorts})


    @staticmethod
    def _ownership_symbol_target(path: str, symbol: dict, imported_paths: set[str]) -> str | None:
        candidate = str(symbol.get("path") or "")
        visibility = str(symbol.get("evidence_visibility") or EvidenceVisibility.DENY.value)
        visible = EvidenceVisibility(visibility) is not EvidenceVisibility.DENY
        if not candidate or candidate == path or candidate not in imported_paths or not visible:
            return None
        return candidate


    @classmethod
    def _ownership_call_targets(cls, path: str, edge: dict, imported_paths: set[str], symbols_by_short) -> list[tuple[str, str]]:
        short = str(edge.get("target_short") or "")
        if not short:
            return []
        targets = [
            target for symbol in symbols_by_short.get(short.lower(), ())
            if (target := cls._ownership_symbol_target(path, symbol, imported_paths)) is not None
        ]
        return [(target, "import-constrained-symbol") for target in targets]


    def _ownership_targets_for_edge(
        self, path: str, edge: dict, resolved_imports: dict[str, list[str]],
        imported_paths: set[str], symbols_by_short: dict[str, list[dict]],
    ) -> list[tuple[str, str]]:
        kind = str(edge.get("kind") or "")
        if kind == "import":
            target = str(edge.get("target") or "")
            return [(exact, "exact-module") for exact in resolved_imports.get(target, ())]
        if kind == "call":
            return self._ownership_call_targets(path, edge, imported_paths, symbols_by_short)
        return []


    @staticmethod
    def _ownership_candidate_row(
        state: _OwnershipGraphState, target_path: str, target_node: dict[str, object],
        depth: int, relation: str, resolution: str,
    ) -> None:
        domains = set(target_node["domains"])
        source_domain = RepositoryDomain.SOURCE.value in domains or RepositoryDomain.SCRIPT.value in domains
        if RepositoryDomain.TEST.value in domains or not source_domain:
            return
        row = state.candidates.setdefault(target_path, {
            "path": target_path, "depth": depth, "relations": [], "exact_edges": 0,
    ...
    def _record_ownership_target(
        self, state: _OwnershipGraphState, expansion: _OwnershipExpansion,
        target_path: str, resolution: str, kind: str,
    ) -> None:
        ...
        state.edges.append({
            "from": expansion.path, "to": target_path, "relation": relation, "depth": next_depth,
            "resolution": resolution, "cycle": cycle, "revisited": revisited,
        })
        self._ownership_candidate_row(state, target_path, target_node, next_depth, relation, resolution)
        if revisited:
            return
        state.seen.add(target_path)
        state.parents.setdefault(target_path, (expansion.path, relation))
        expansion.frontier.append((target_path, next_depth, expansion.ancestry + (target_path,)))


    @staticmethod
    def _ownership_import_targets(path_edges) -> list[str]:
        return list(dict.fromkeys(
            str(edge.get("target") or "") for edge in path_edges
            if str(edge.get("kind") or "") == "import" and str(edge.get("target") or "")
        ))


    def _ownership_import_resolution(self, path: str, path_edges) -> tuple[dict[str, list[str]], set[str]]:
        targets = self._ownership_import_targets(path_edges)
        resolved = {target: self._exact_ownership_import_paths(path, target) for target in targets}
        imported = {exact for rows in resolved.values() for exact in rows}
        return resolved, imported


    def _expand_ownership_path(
        self, state: _OwnershipGraphState, item: tuple[str, int, tuple[str, ...]],
        edge_map, symbols_by_short, frontier,
    ) -> None:
        ...
        for edge in path_edges:
            kind = str(edge.get("kind") or "")
            targets = self._ownership_targets_for_edge(path, edge, resolved, imported_paths, symbols_by_short)
            for target_path, resolution in dict.fromkeys(targets):
                self._record_ownership_target(state, expansion, target_path, resolution, kind)
    ...
    def _expand_ownership_level(
        self, state: _OwnershipGraphState, current_frontier: list[tuple[str, int, tuple[str, ...]]], max_depth: int
        ...
        for item in expandable:
            self._expand_ownership_path(state, item, edge_map, symbols_by_short, frontier)
        return frontier


    def _rank_ownership_candidates(self, state: _OwnershipGraphState) -> list[dict[str, object]]:
        ...
        for row in state.candidates.values():
            ...
            row["corroborated"] = bool({"verification-origin", "task-locality", "call-edge"}.intersection(corroboration))
            ranked.append(row)
        ranked.sort(key=lambda row: (
            not bool(row.get("corroborated")), -int(row.get("depth") or 0),
            -len(row.get("task_locality_terms") or []), -int(row.get("exact_edges") or 0),
            str(row.get("path") or ""),
        ))
        return ranked
    ...
    @staticmethod
    def _ownership_edge_refines(edge, target_locality: set[str], base_locality: set[str]) -> bool:
        if edge.get("relation") == "calls" and edge.get("resolution") == "import-constrained-symbol":
            return True
        return (
    ...
    def _delegated_target_for_edge(
        cls, edge: dict, base_path: str, base_depth: int, base_locality: set[str], ranked_by_path
    ) -> str | None:
        ...
        return target if cls._ownership_edge_refines(edge, locality, base_locality) else None


    @classmethod
    def _delegated_ownership_targets(
        cls, state: _OwnershipGraphState, ranked: list[dict[str, object]], selected: dict[str, object]
    ) -> set[str]:
        ...
        return {
            target for edge in state.edges
            if (target := cls._delegated_target_for_edge(edge, base_path, base_depth, base_locality, ranked_by_path)) is not None
        }
    ...
    def _select_ownership_candidate(
        cls, state: _OwnershipGraphState, ranked: list[dict[str, object]], max_depth: int
    ...
    def _ownership_path(
        ...
        if selected is None:
            return []
        cursor = str(selected["path"]); reverse: list[dict[str, object]] = []
        while cursor != state.start and cursor in state.parents:
            parent, relation = state.parents[cursor]
            reverse.append({"from": parent, "to": cursor, "relation": relation}); cursor = parent
    ...
    def ownership_relation_graph(
        ...
        if start_row is not None and EvidenceVisibility(str(start_row["evidence_visibility"])) is EvidenceVisibility.DENY:
            raise PermissionError(f"path is not agent-visible: {start}")
        state = _OwnershipGraphState(start, set(_query_terms(task)), {}, [], {}, {}, {start})
        ...
        selected, selection_reason = self._select_ownership_candidate(state, ranked, max_depth)
        owner_path = self._ownership_path(state, selected)
        return {
            "schema": "hashmarks.ownership-relation-graph.v1", "task": task, "start_path": start,
            "max_depth": max_depth, "nodes": sorted(state.nodes.values(), key=lambda row: str(row["path"])),
            "edges": state.edges, "candidates": ranked,
            "selected": self._selected_ownership_path(selected),
            "selection_reason": selection_reason, "owner_path": owner_path,
    ...
    @staticmethod
    def _selected_owner_row(graph: dict[str, object], selected_path: object) -> dict[str, object] | None:
        candidates = graph.get("candidates")
        if not isinstance(candidates, list):
            return None
        return next(
            (row for row in candidates if isinstance(row, dict) and row.get("path") == selected_path),
            None,
        )
    ...
    @staticmethod
    def _owner_projection_value(row: dict[str, object], key: str, default: object) -> object:
        value = row.get(key)
    ...
    def _structural_owner_candidate(
        ...
        return {
            "path": str(selected_path), "depth": int(depth),
            "via": str(via).removesuffix("s"), "source_path": str(source_path),
            "owner_path": owner_path, "corroboration": list(corroboration),
