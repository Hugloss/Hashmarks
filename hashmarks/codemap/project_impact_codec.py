...
def _impact_dictionary(
    roots: list[str], affected: list[dict[str, object]], edges: list[dict[str, object]]
) -> _ImpactDictionary:
    projects = sorted(
        {
            *roots,
            *(str(row.get("project") or "") for row in affected),
            *(str(row.get("from") or "") for row in edges),
            *(str(row.get("to") or "") for row in edges),
        }
        - {""}
    )
    return _ImpactDictionary(
        ...
        confidences=sorted(
            {str(row.get("confidence") or "") for row in edges if row.get("confidence")}
        ),
    )
...
def _compact_row(
    ...
    row = [
        ...
        depth,
        project_index.get(str(edge.get("to") or ""), -1),
    ...
    if dictionary.confidences:
        row.append(
            dictionary.confidence_index.get(str(edge.get("confidence") or ""), -1)
        )
    return row
...
def compact_project_impact(fragment: Mapping[str, object]) -> dict[str, object]:
    ...
    result: dict[str, object] = {
        ...
        "producers": dictionary.producers,
        "roots": [project_index[value] for value in roots if value in project_index],
        "rows": rows,
        "reported_affected": int(fragment.get("reported_affected") or len(affected)),
        "total_affected": int(fragment.get("total_affected") or len(affected)),
        "complete": bool(fragment.get("complete")),
    }
    if dictionary.confidences:
...
def _expanded_row(
    raw: object, dictionary: _ImpactDictionary
) -> tuple[dict[str, object] | None, dict[str, object] | None]:
    if not isinstance(raw, (list, tuple)) or len(raw) < 5:
        return None, None
    project_i, depth, target_i, kind_i, producer_i = (
        int(raw[index]) for index in range(5)
    )
    ...
    edge: dict[str, object] = {
        "from": project,
        "to": target,
        "kind": _lookup(dictionary.kinds, kind_i) or "declared",
