...
_TASK_SCOPE_EXPANSIONS: tuple[tuple[frozenset[str], tuple[str, ...]], ...] = (
    ...
    (frozenset({"plan", "plans", "goon", "goons"}), ("plan", "goon", "agents", "readme")),
    (frozenset({"make", "certify", "certification", "setup", "bootstrap"}), ("make", "certification", "docs", "contract")),
    (frozenset({"generated", "openapi"}), ("generated", "contract", "agents", "readme")),
    (frozenset({"schedule", "scheduler"}), ("schedule", "scheduler", "ownership", "backend", "agents")),
)
...
class TaskRetrievalMixin:
    ...
    def _task_resurrection_paths(self, recent_paths: tuple[str, ...]) -> tuple[str, ...]:
        return tuple(sorted(
            path
    ...
    def _reconcile_task_resurrection_paths(self, recent_paths: tuple[str, ...]) -> tuple[str, ...]:
    ...
    @staticmethod
    def _repository_domain_boost(path: str, route: QueryRoute) -> tuple[float, tuple[str, ...]]:
        ...
        matches = tuple(domain for domain in route.preferred_domains if domain in path_domains)
        if not matches:
            return 0.0, ()
        weights = {domain: max(16.0, 42.0 - index * 7.0) for index, domain in enumerate(route.preferred_domains)}
    ...
    def _apply_repository_domain_hints(
        ...
        for row in rows:
            ...
            if boost <= 0:
                ...
                continue
            value = dict(row)
            value["_relation_boost"] = float(value.get("_relation_boost", 0.0) or 0.0) + boost
    ...
    def _score(
        ...
        *,
        ...
        if self._is_test_path(str(row.get("path", ""))):
            score += 12.0 if any(token in {"test", "tests", "spec"} for token in tokens) else -16.0
    ...
    def _bounded_rerank_rows(
        ...
        *,
        ...
        for _key, row in heapq.nsmallest(path_quota, best_by_path.values(), key=lambda item: item[0]):
            selected.append(row)
    ...
    def _formulate_task_query_base(self, task: str, *, max_terms: int = 10) -> str:
        ...
        def score(term: str) -> tuple[float, int, str]:
            ...
            if any(ch in raw for ch in "_-/"):
                bonus += 1.5
            if term in {"api", "goon", "plan", "fleet", "uv", "scout", "codemap", "hashmarks", "executor", "certification", "workspace", "runtime", "contract", "identity", "schedule", "repository"}:
                bonus += 1.0
            return rarity + bonus, len(term), term

        ranked = sorted((term for term in words if term in frequencies), key=score, reverse=True)
    ...
    def task_query_views(self, task: str) -> dict[str, str]:
        ...
        if visible_words & _TASK_GOVERNANCE_CUES:
            additions.extend(("ownership", "authority", "architecture", "contract", "agents", "readme"))
    ...
    def _rare_task_anchor_hits(self, token: str, *, limit: int) -> tuple[SearchHit, ...]:
        ...
        file_rows = self._session_lexical_file_candidates([normalized], limit=max(8, min(32, limit * 2)))
        path_rows = self.store.path_candidates([normalized], limit=max(8, min(32, limit * 2)))
        ...
        if not by_path:
            return ()
        symbols = self.store.symbols_for_paths(list(by_path), limit=max(64, min(512, len(by_path) * 16)))
        symbol_by_path: dict[str, dict[str, object]] = {}
        for row in symbols:
            text = " ".join(str(row.get(key) or "").lower() for key in ("path", "name", "qualname"))
        ...
        def locality_order(path: str) -> tuple[int, int, str]:
            ...
            return (0 if is_test else 2 if archive_like else 1, len(Path(path).parts), path)

        output: list[SearchHit] = []
        for path in sorted(by_path, key=locality_order):
            row = by_path[path]
            visibility_raw = str(row.get("evidence_visibility") or EvidenceVisibility.SOURCE.value)
            try:
            ...
            output.append(SearchHit(
                ...
                name=str(symbol.get("name") or "") or None if symbol is not None else None,
                qualname=str(symbol.get("qualname") or "") or None if symbol is not None else None,
                signature=str(symbol.get("signature") or "") or None if symbol is not None else None,
                start_line=int(symbol.get("start_line") or 0) or None if symbol is not None else None,
                end_line=int(symbol.get("end_line") or 0) or None if symbol is not None else None,
    ...
    def _task_local_island_terms(self, task: str) -> tuple[str, ...]:
        ...
        if len(_WORD_RE.findall(task)) < 2:
            return ()
        locality_generic = {
            "accepted", "actually", "active", "behavior", "change", "drives",
            "implementation", "owner", "request", "response", "that", "update",
            "verification", "verify",
        }
        terms = [
            term for term in dict.fromkeys(_query_terms(task))
            if len(term) >= 4
        ...
        rare_terms = [
            ...
            if 0 < int(frequencies.get(term, 0)) <= max_documents
        ]
        rare_terms.sort(key=lambda term: (int(frequencies.get(term, 0)), -len(term), term))
    ...
    def _task_local_island_rows(
        ...
        if not island or len(island) > 16:
            return None
        domains_by_path = {
            str(row.get("path") or ""): set(classify_repository_path(str(row.get("path") or "")))
            for row in island
        }
        test_rows = [
            row for row in island
            if RepositoryDomain.TEST in domains_by_path[str(row.get("path") or "")]
        ]
        if len(test_rows) != 1:
            return None
        action_domains = {
            RepositoryDomain.SOURCE, RepositoryDomain.SCRIPT, RepositoryDomain.CONFIG,
            RepositoryDomain.BUILD, RepositoryDomain.CONTRACT,
        }
        if not any(
            RepositoryDomain.TEST not in domains_by_path[str(row.get("path") or "")]
            and bool(domains_by_path[str(row.get("path") or "")].intersection(action_domains))
            for row in island
        ):
            return None
    ...
    def _task_local_island_symbols(
        ...
        symbols = self.store.symbols_for_paths(paths, limit=max(64, min(512, len(paths) * 16)))
        symbol_by_path: dict[str, dict[str, object]] = {}
        selected_set = set(selected_terms)
        for row in symbols:
            text_terms = set(_query_terms(" ".join(
                str(row.get(key) or "") for key in ("name", "qualname", "signature")
            )))
            if len(selected_set.intersection(text_terms)) >= 2:
                symbol_by_path.setdefault(str(row.get("path") or ""), row)
        return symbol_by_path

    def _task_local_lexical_island_hits(self, task: str, *, limit: int) -> tuple[SearchHit, ...]:
        ...
        def locality_order(row: dict[str, object]) -> tuple[int, int, int, str]:
            ...
            parts = {part.lower() for part in Path(path).parts}
            archive_like = bool(parts.intersection({"archive", "legacy", "deprecated", "vendor"}))
            is_test = RepositoryDomain.TEST in domains
            return (
    ...
    def _task_find_trim_cache(self, generation: int) -> None:
        if len(self._task_result_cache) < 256 and len(self._task_authority_paths_cache) < 256:
            return
        self._task_result_cache = {
            key: value for key, value in self._task_result_cache.items() if key[0] == generation
        }
        self._task_authority_paths_cache = {
            key: value for key, value in self._task_authority_paths_cache.items() if key[0] == generation
        }
    ...
    def _task_raw_identifier_tokens(cls, task: str) -> list[str]:
        return [token for token in _WORD_RE.findall(task) if cls._task_is_identifier_token(token)]
    ...
    def _task_broad_authority_cues() -> set[str]:
        return {
            "config", "configuration", "settings", "makefile", "pyproject",
            "toml", "yaml", "yml", "sql", "schema", "policy", "invariant",
            "authority", "agents",
        }
    ...
    def _task_fast_identifier_tokens(raw_tokens: Sequence[str]) -> list[str]:
        return [token for token in raw_tokens if any(ch.isdigit() for ch in token) or "_" in token]
    ...
    def _task_rare_identifier_result(self, token: str, limit: int) -> tuple[SearchHit, ...] | None:
        fast_hits = self._rare_task_anchor_hits(token, limit=max(20, min(40, limit * 2)))
        unique_paths = list(dict.fromkeys(hit.path for hit in fast_hits))
        domains = {domain for path in unique_paths for domain in classify_repository_path(path)}
    ...
    @staticmethod
    def _task_sequence_specs(views: Mapping[str, str], limit: int) -> list[tuple[str, float, int]]:
    ...
    def _task_find_sequences(
        ...
        session_scope = nullcontext(self) if self._decision_session_depth > 0 else self.decision_session()
        with session_scope:
            for view, weight, view_fetch in specs:
    ...
    def _task_fuse_sequences(
        ...
        for sequence, weight in sequences:
            ...
            for hit in sequence:
                ...
                seen.add(hit.path)
                path_rank += 1
                path_scores[hit.path] = path_scores.get(hit.path, 0.0) + weight / (60.0 + path_rank)
    ...
    def _task_anchor_hit(anchor: Mapping[str, object]) -> SearchHit | None:
        ...
        if not anchor_path:
            return None
        return SearchHit(
            path=anchor_path, score=1000.0, kind=str(anchor.get("kind") or "symbol"),
    ...
    def _task_preserve_exact_anchor(
        ...
        rows.sort(key=lambda row: (
            token_order.get(str(row.get("name") or row.get("qualname") or "").lower(), len(token_order)),
        ...
        fusion.path_scores[anchor_hit.path] = max(fusion.path_scores.get(anchor_hit.path, 0.0), 2.0 / 61.0)
    ...
    def _task_component_hits(self, token: str) -> list[SearchHit]:
        ...
        for hit in self.find(token, limit=10):
            haystack = " ".join(value.lower() for value in (hit.path, hit.name or "", hit.qualname or ""))
    ...
    def _task_preserve_component_hits(
        ...
        for preserved, hit in enumerate(hits[:limit]):
            ...
            fusion.best_hit[hit.path] = hit if current is None or hit.score > current.score else current
    ...
    def _task_omitted_alnum_tokens(
        ...
        tokens = (
            token for token in _WORD_RE.findall(task)
    ...
    def _task_hits_for_token(
        ...
        return tuple(
            hit for hit in hits
            if lowered in " ".join(
                value.lower() for value in (hit.path, hit.name or "", hit.qualname or "")
            )
        )


    def _task_preserve_omitted_alnum_components(
        self, task: str, views: Mapping[str, str], cue_words: set[str], fusion: _TaskFindFusion
    ) -> None:
        tokens = self._task_omitted_alnum_tokens(task, views)
        if not tokens:
            return
        preserve_limit = 3 if cue_words.intersection({"config", "configuration", "policy", "settings"}) else 2
        proven = 0
        for offset in range(0, len(tokens), 8):
            ...
            for token in chunk:
                ...
                if not local_hits or len(local_hits) > 8:
                    continue
                self._task_preserve_component_hits(local_hits, fusion, limit=preserve_limit)
                proven += 1
                if proven >= 2:
                    return
    ...
    def _task_fill_ranked_paths(
        sequences: Sequence[tuple[Sequence[SearchHit], float]], ranked_paths: Sequence[str],
        limit: int, fusion: _TaskFindFusion
    ) -> None:
    ...
    def _task_has_authority_surface(selected: Sequence[str]) -> bool:
        return any(Path(candidate).name in {"README.md", "AGENTS.md"} for candidate in selected)


    @staticmethod
    def _task_selected_within(selected: Sequence[str], parent: Path) -> bool:
        parent_path = parent.as_posix()
        return any(_path_is_within(candidate, parent_path) for candidate in selected[:10])
    ...
    def _task_preserve_scoped_authority(
        self, views: Mapping[str, str], sequences: Sequence[tuple[Sequence[SearchHit], float]],
        limit: int, fusion: _TaskFindFusion
    ) -> None:
        route = route_query(views["base"])
        eligible = (
            len(sequences) >= 2 and limit >= 2
    ...
    def _task_scoped_readme_candidates(
        ...
        for hit in governance_hits:
            ...
            if rank > 20:
                break
            candidate = TaskRetrievalMixin._task_scoped_readme_candidate(hit, selected, rank)
    ...
    def _task_scoped_readme_candidate(
        ...
        if readme.name != "README.md" or len(parent.parts) < 3:
            return None
        local_evidence = sum(
            1 for candidate in selected[:10]
    ...
    def _find_task_impl(
        ...
        cache_key, cached = self._task_find_cache_key(task, limit, ensure_ready=ensure_ready)
