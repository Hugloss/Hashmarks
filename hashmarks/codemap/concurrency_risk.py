...
_READ_NAMES={"get","read","fetch","select","generation","load","read_text","fetchone"}
_WRITE_NAMES={"set","write","update","insert","save","commit","execute","bump","write_text"}
...
class Risk:
    path:str; function:str; line:int; read_call:str; write_call:str; guarded:bool; confidence:str
    def as_dict(self):
        return {
          "path":self.path,"function":self.function,"line":self.line,
          "read_call":self.read_call,"write_call":self.write_call,
          "guarded":self.guarded,"confidence":self.confidence,
          "code":"python-read-modify-write-without-visible-guard" if not self.guarded else "python-read-modify-write-visible-guard",
          "reason":("A function reads and later writes through the same lexical owner without a visible lock/transaction guard."
                    if not self.guarded else
                    "A read/write sequence exists, but a visible lock/transaction guard is present."),
          "recommendation":("Verify atomicity at the durable owner; use a transaction/CAS/owner lock when concurrent callers are possible."
                            if not self.guarded else
                            "Retain the visible guard and verify that it serializes every caller that can reach the durable owner."),
        }
...
class _CallCollector(ast.NodeVisitor):
    ...
    def visit_Call(self,node:ast.Call)->None:
        name=_name(node.func); leaf=name.lower().rsplit(".",1)[-1]
        self.calls.append((int(getattr(node,"lineno",1)),name,leaf,self._owner(node,name,leaf)))
...
def _compatible_branches(left:tuple[tuple[int,object],...], right:tuple[tuple[int,object],...])->bool:
    l=dict(left); r=dict(right)
...
def _fresh_local_owners(node: ast.FunctionDef | ast.AsyncFunctionDef) -> frozenset[str]:
    ...
    return frozenset(
        name for name, values in collector.assignments.items()
...
def _scoped_state_owners(tree: ast.Module) -> frozenset[str]:
    ...
    for node in tree.body:
        ...
        if target is None or not isinstance(value, ast.Call):
            continue
        constructor = value.func.value if isinstance(value.func, ast.Subscript) else value.func
        if _name(constructor).rsplit(".", 1)[-1] in _SCOPED_STATE_CONSTRUCTORS:
            owners.update(_target_names(target))
    return frozenset(owners)

class V(ast.NodeVisitor):
    def __init__(self,path,os_aliases:frozenset[str]=frozenset({"os"}),scoped_state_owners:frozenset[str]=frozenset()):
        self.path=path
        self.os_aliases=os_aliases
    ...
    def _events_from_node(self,node:ast.AST,guards:frozenset[str],branches:tuple[tuple[int,object],...])->list[_Event]:
        c=_CallCollector(self.os_aliases); c.visit(node)
        return [_Event(line,n,leaf,owner,guards,branches) for line,n,leaf,owner in c.calls]
    ...
    def _scan_match(self,stmt,guards,branches):
        ...
        for index,case in enumerate(stmt.cases):
            events.extend(self._scan_block(case.body,guards,branches+((match_id,index),)))
        return events
    ...
    def _risk_for_read(self,node,calls,index,fresh_local_owners:frozenset[str]):
        ...
        if read.leaf not in _READ_NAMES:
            return None
        owner=read.owner
        if not owner or owner in self.scoped_state_owners or owner in fresh_local_owners:
            return None
        write=self._first_compatible_write(calls,index,owner)
        if write is None:
            return None
        guarded=owner in read.guards and owner in write.guards
        return Risk(self.path,node.name,read.line,read.call,write.call,guarded,"high" if guarded else "medium")
    ...
    def visit_FunctionDef(self,node):
        self._fn(node)
        for stmt in node.body:
            if isinstance(stmt,(ast.FunctionDef,ast.AsyncFunctionDef,ast.ClassDef)): self.visit(stmt)
...
def analyze_python_concurrency_risk(*,path:str,source:str,tree:ast.Module|None=None):
    if tree is None:
        try:tree=ast.parse(source)
        except (SyntaxError,ValueError):return ()
    v=V(path,_os_aliases(tree),_scoped_state_owners(tree));v.visit(tree);return tuple(v.r)
