...
from .task_action_types import _TaskActionConfigState, _TaskActionCues, _TaskActionProjectionState, _TaskActionSurface
...
class TaskActionEvidenceMixin:
    ...
    def _task_action_cues(task: str) -> _TaskActionCues:
        ...
        explicit_test_edit = bool(
            ...
            or re.search(r"\b(?:add|write|create)\s+(?:a\s+)?(?:regression\s+)?test\b", task_lower)
        ...
        return _TaskActionCues(
            ...
            explicit_architecture_contract=bool(
                re.search(r"\barchitecture\b.{0,32}\b(?:contract|invariant|policy)\b", task_lower)
            ),
        )
    ...
    def _strong_config_cues() -> set[str]:
        return {
            "config", "configuration", "makefile", "pyproject", "package.json",
            "settings", "toml", "vite", "yaml", "yml",
        }
    ...
    def _task_action_surface(path: str) -> _TaskActionSurface:
        ...
        return _TaskActionSurface(
            ...
            is_contract=bool(domain_set & {RepositoryDomain.OWNERSHIP, RepositoryDomain.CONTRACT}),
            is_config=bool(domain_set & {RepositoryDomain.CONFIG, RepositoryDomain.BUILD, RepositoryDomain.PLAN}),
            is_source=bool(domain_set & {RepositoryDomain.SOURCE, RepositoryDomain.SCRIPT}),
        )
    ...
    def _task_action_test_roles(
        ...
        if not surface.is_test:
            return []
        return ["verify", "edit"] if explicit_test_edit and not disproven else ["verify"]
    ...
    def _task_action_roles(
        ...
        *,
        ...
        if "edit" not in roles and not surface.benchmark_external and not surface.is_test:
    ...
    def _task_action_row(
        ...
        return {
            "path": hit.path, "canonical_rank": rank, "canonical_score": hit.score,
            "domains": [domain.value for domain in surface.domains], "roles": roles,
            "name": hit.name, "qualname": hit.qualname, "signature": hit.signature,
    ...
    def _task_action_projection_state(
        ...
        return _TaskActionProjectionState(
            terms=terms, document_frequency=document_frequency, candidates=candidates,
            file_rows=file_rows, text_cache={}, rows=rows, limit=limit,
        )
    ...
    def _task_surface_file_row(
        ...
        if not path or domain not in path_domains:
            return None
        if domain is RepositoryDomain.TEST and RepositoryDomain.SOURCE not in path_domains:
            return None
        file_row = state.file_rows.get(path)
        if not isinstance(file_row, Mapping):
            return None
        if EvidenceVisibility(str(file_row["evidence_visibility"])) is EvidenceVisibility.DENY:
            return None
        return file_row
    ...
    def _task_surface_candidate(
        self, candidate: dict[str, object], domain: RepositoryDomain, state: _TaskActionProjectionState
    ...
    def _task_surface_candidates(
        ...
        candidates = [
            ...
            if (scored := self._task_surface_candidate(candidate, domain, state)) is not None
        ]
        if domain is RepositoryDomain.TEST:
            candidates.sort(key=lambda item: (-item[1], -item[0], str(item[2].get("path") or "")))
        else:
            candidates.sort(key=lambda item: (-item[0], -item[1], str(item[2].get("path") or "")))
        return candidates


    @staticmethod
    def _task_surface_is_unique(
        domain: RepositoryDomain, candidates: Sequence[tuple[float, int, dict[str, object]]]
    ) -> bool:
        if len(candidates) == 1:
            return True
    ...
    def _task_surface_projection_row(
        ...
        existing = {
            "path": path, "canonical_rank": state.limit + 1, "canonical_score": 0.0,
            "domains": [item.value for item in classify_repository_path(path)],
            "roles": ["verify"] if domain is RepositoryDomain.TEST else ["edit", "related"],
            "name": symbol.get("name"), "qualname": symbol.get("qualname"),
            "signature": symbol.get("signature"), "start_line": symbol.get("start_line"),
            "end_line": symbol.get("end_line"),
            "evidence_visibility": str(file_row["evidence_visibility"]),
            "lexical_surface_projection": True, "lexical_surface_score": best_score,
    ...
    def _task_identifier_verification_row(
        ...
        def match(row: dict[str, object]) -> tuple[int, int]:
            text = (
                " ".join(str(row.get(key) or "").lower() for key in ("path", "name", "qualname", "signature"))
                + " "
                + " ".join(
        ...
        return local_verify if local_verify is not None and match(local_verify)[0] > 0 else None
    ...
    def _literal_sql_references(self, verify_path: str) -> list[str]:
        try:
            verify_source = (self.workspace / verify_path).read_text(encoding="utf-8", errors="replace")
        except OSError:
            return []
        ...
        for match in re.finditer(r"[\"']([^\"']+\.sql)[\"']", verify_source):
            ...
            if (
                ...
                and EvidenceVisibility(str(referenced_row["evidence_visibility"])) is not EvidenceVisibility.DENY
            ):
    ...
    def _literal_sql_reference_projection(
        ...
        existing = next((row for row in rows if str(row.get("path")) == owner_path), None)
        if existing is None:
        ...
        owner = {
            ...
            "owner_path": [{"from": verify_path, "to": owner_path, "relation": "references"}],
            "corroboration": ["verification-origin", "exact-file-reference", "task-sql-cue"],
            "cycle_count": 0,
            "revisit_count": 0,
    ...
    def _active_verification_locality(
        ...
        if not verify_path:
            return None
        active_anchor = self._structural_owner_candidate(verify_path, max_depth=3, task=task)
    ...
    def _projected_verification_locality_row(
        ...
        symbol = symbols[0] if symbols else {}
        projected = {
            "path": active_path, "canonical_rank": limit + depth, "canonical_score": 0.0,
            "domains": [domain.value for domain in classify_repository_path(active_path)],
            "roles": ["related"], "name": symbol.get("name"),
            "qualname": symbol.get("qualname"), "signature": symbol.get("signature"),
            "start_line": symbol.get("start_line"), "end_line": symbol.get("end_line"),
            "evidence_visibility": (str(file_row["evidence_visibility"]) if file_row is not None else EvidenceVisibility.SOURCE.value),
    ...
    def _verification_locality_source_anchor(
        self, task: str, verify: dict[str, object] | None, rows: list[dict[str, object]], limit: int
    ) -> dict[str, object] | None:
        active = self._active_verification_locality(task, verify)
        if active is None:
            return None
        active_path, depth = active
        return self._projected_verification_locality_row(active_path, depth, rows, limit)
    ...
    def _local_config_projection_row(
        ...
        allowed = {RepositoryDomain.CONFIG, RepositoryDomain.BUILD, RepositoryDomain.PLAN, RepositoryDomain.CONTRACT}
        if not set(local_domains).intersection(allowed):
            return None
        symbols = self._session_symbols_for_path(local_path)
        symbol = symbols[0] if symbols else {}
        return {
            "path": local_path, "canonical_rank": limit + 1, "canonical_score": 0.0,
            "domains": [domain.value for domain in local_domains], "roles": ["edit", "related"],
            "name": symbol.get("name"), "qualname": symbol.get("qualname"),
            "signature": symbol.get("signature"), "start_line": symbol.get("start_line"),
            "end_line": symbol.get("end_line"), "evidence_visibility": str(file_row["evidence_visibility"]),
            "locality_projection": True,
        }


    def _admit_config_locality_siblings(
        self, source_anchor: dict[str, object], rows: list[dict[str, object]],
        config_candidates: list[dict[str, object]], failed: set[str], limit: int,
    ) -> None:
    ...
    def _config_locality_key(
        ...
        anchor_parts = {
            part.lower() for part in Path(str(anchor["path"])).parts
            if part.lower() not in generic_parts
        }
        row_parts = {
            part.lower() for part in Path(str(row["path"])).parts
            if part.lower() not in generic_parts
        }
        return (
    ...
    def _local_config_for_anchor(
        ...
        local_candidates = [
            row for row in config_candidates
    ...
    def _task_action_initial_config_candidates(
        ...
        domains = {
            RepositoryDomain.CONFIG.value, RepositoryDomain.BUILD.value,
            RepositoryDomain.PLAN.value, RepositoryDomain.CONTRACT.value,
        }
        return [
            row for row in rows
