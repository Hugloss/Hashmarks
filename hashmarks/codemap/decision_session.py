...
class DecisionSessionMixin:
    ...
    def _diagnostic_normalize(value: object) -> object:
        ...
        if isinstance(value, Mapping):
            return {
                str(k): DecisionSessionMixin._diagnostic_normalize(v)
    ...
    def _diagnostic_request_identity(
    ...
    def _decision_diagnostic_span(
        ...
        parent_id = (
            self._decision_diagnostics_stack[-1]["id"]
        ...
        try:
            ...
            producer = self._decision_diagnostics_producers.setdefault(
                name,
                {
                    "calls": 0,
                    "inclusive_ns": 0,
    ...
    def _decision_diagnostics_receipt(self) -> dict[str, object]:
        ...
        reuse = {
            ...
            if k.endswith("_hit") or k.endswith("_miss")
        }
        ...
        return {
            ...
            "wall_time_ns": max(
                0, time.perf_counter_ns() - self._decision_diagnostics_started_ns
            ),
            "producers": producers,
    ...
    def _session_preload_module_paths(self, modules: Iterable[str]) -> None:
        ...
        missing = [
            ...
            if (generation, module) not in self._decision_module_paths_cache
        ]
        if not missing:
            return
        grouped = self.store.module_paths_many(missing)
        for module in missing:
            self._decision_module_paths_cache[(generation, module)] = tuple(
                grouped.get(module, ())
            )
            self._decision_session_stats["module_paths_miss"] += 1
    ...
    def _session_exact_symbol_candidates(
        ...
        *,
        ...
        self._decision_exact_symbols_cache[key] = (
            requested,
            tuple(dict(row) for row in value),
        )
        return value
    ...
    def _session_refs_many(
        ...
        *,
        ...
        missing_shorts = list(
            dict.fromkeys(
                short
                for short in target_short.values()
        ...
        for target in unique:
            ...
            result[target] = (
                [] if cached is None else [dict(row) for row in cached[1][:requested]]
            )
        return result
    ...
    def _session_edges_from_many(
        ...
        *,
        ...
        missing = [
            ...
            or self._decision_edges_from_cache[(generation, seed[0], seed[1])][0]
            < requested
        ]
        if missing:
        ...
        missing = sorted(
            {
                path
                for path in paths
                if path and (generation, path) not in self._decision_symbols_cache
            }
        )
        if not missing:
            return
    ...
    def _session_symbols_for_paths(
    ...
    def _session_lexical_document_frequencies(
    ...
    def _session_lexical_file_candidates(
