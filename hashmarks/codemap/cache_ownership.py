...
class CacheOwner:
    ...
    def as_dict(self) -> dict[str, object]:
        ...
        recommendation = (
            ...
            "Keep cache ownership at one canonical module identity; document lifecycle/invalidation when callers depend on reuse."
        )
        return {
            "path": self.path, "line": self.line, "owner": self.owner, "kind": self.kind,
            "scope": self.scope, "mutable": self.mutable, "invalidation": self.invalidation,
            "confidence": self.confidence, "import_identity_risk": self.import_identity_risk,
...
class _Visitor(ast.NodeVisitor):
    ...
    def _decorated(self,node: ast.FunctionDef|ast.AsyncFunctionDef)->None:
        for dec in node.decorator_list:
            ...
            if name in _CACHE_FACTORIES:
                self.owners.append(CacheOwner(
                    self.path,int(node.lineno),node.name,f"decorator:{name}",
                    "function",False,"decorator-managed","high"
                ))

    def visit_FunctionDef(self,node:ast.FunctionDef)->None:
        self._decorated(node); self.stack.append(node.name); self.generic_visit(node); self.stack.pop()
    ...
    def _record_module_assignment(self,node:ast.Assign,target:ast.Name)->None:
        ...
        if not _looks_cache_name(target.id):
            return
        self.owners.append(CacheOwner(
            self.path,int(node.lineno),target.id,self._assignment_kind(node.value),
            "module",True,"not-proven","medium"
        ))
...
def analyze_python_cache_ownership(*,path:str,source:str,import_risk_targets:Iterable[str]=(),tree:ast.Module|None=None)->tuple[CacheOwner,...]:
    tree=_ownership_tree(source,tree)
    if tree is None:
        return ()
    visitor=_Visitor(path); visitor.visit(tree)
        ...
        if owner.owner in visitor.invalidators:
            ...
            for invalidated in visitor.invalidators:
                ...
                if current==owner.owner:
                    ...
                    break
        out.append(CacheOwner(
            path=owner.path,line=owner.line,owner=owner.owner,kind=owner.kind,scope=owner.scope,
            mutable=owner.mutable,invalidation=invalidation,confidence=owner.confidence,
