...
class _TypeServerClientProtocol(Protocol):
    ...
    def resolve_import(self, source_uri: str, leading_dots: int, name_parts: tuple[str, ...], snapshot: int) -> str | None: ...
    def close(self) -> None: ...
...
class _JsonRpcProcess:
    ...
    def __init__(self, command: list[str], *, cwd: Path, timeout: float) -> None:
        ...
        self._reader = threading.Thread(target=self._read_loop, name="hashmarks-pyright-tsp-reader", daemon=True)
        self._reader.start()
        self._stderr_reader = threading.Thread(target=self._read_stderr, name="hashmarks-pyright-tsp-stderr", daemon=True)
    ...
    def _send(self, message: dict) -> None:
        ...
        if stream is None or self._proc.poll() is not None:
            detail = self._stderr_lines[-1] if self._stderr_lines else "type server exited"
    ...
    def _server_reply_result(method: str, params: object) -> object:
        ...
        if method == "workspace/applyEdit":
            return {"applied": False, "failureReason": "Hashmarks analysis client is read-only"}
        return None
    ...
    def _reply_to_server_request(self, message: dict) -> None:
        ...
        supported = {
            "workspace/configuration", "client/registerCapability", "client/unregisterCapability",
    ...
    def _wait_response(self, request_id: int, method: str) -> object:
        while True:
            try:
                ...
                detail = self._stderr_lines[-1] if self._stderr_lines else f"timeout waiting for {method}"
...
class _PyrightTypeServerClient:
    def __init__(self, executable: Path | str, *, workspace: Path, timeout: float) -> None:
        self.workspace = workspace
        self._rpc = _JsonRpcProcess([str(executable), "--stdio"], cwd=workspace, timeout=timeout)
    ...
    def initialize(self) -> None:
        ...
        self._rpc.request(
            "initialize",
            {
                ...
                "rootUri": root_uri,
                "capabilities": {},
                "workspaceFolders": [{"uri": root_uri, "name": self.workspace.name or "workspace"}],
            },
        )
        self._rpc.notify("initialized", {})
        version = self._rpc.request("typeServer/getSupportedProtocolVersion")
        if not isinstance(version, str):
            raise RuntimeError("Pyright Type Server returned an invalid protocol version")
        ...
        try:
            value = self._rpc.request(
                "typeServer/resolveImport",
                {
                    ...
                    "moduleDescriptor": {"leadingDots": leading_dots, "nameParts": list(name_parts)},
                    "snapshot": snapshot,
                },
            )
...
def _relative_alias_specs(node: ast.ImportFrom, rel: str, leading: int) -> list[PythonImportSpec]:
    result: list[PythonImportSpec] = []
    for alias in node.names:
        if alias.name == "*":
            continue
...
def _file_uri_path(uri: str) -> str | None:
    ...
    windows_drive = os.name == "nt" and raw_path.startswith("/") and len(raw_path) >= 3 and raw_path[2] == ":"
...
def _uri_to_workspace_path(uri: str, workspace: Path) -> str | None:
    ...
    try:
        ...
        rel = normalize_relative_path(target.relative_to(workspace).as_posix(), allow_root=False)
...
class PyrightTypeServerProvider:
    ...
    def __init__(
        ...
        *,
        client_factory: Callable[[Path | str, Path, float], _TypeServerClientProtocol] | None = None,
    ) -> None:
    ...
    def _executable(self, workspace: Path) -> Path | None:
        ...
        suffixes = ("pyright-typeserver.cmd", "pyright-typeserver.exe") if os.name == "nt" else ("pyright-typeserver",)
    ...
    def _package_version(workspace: Path) -> str | None:
        package_json = workspace / "node_modules" / "pyright-typeserver" / "package.json"
        try:
    ...
    def _client(self, executable: Path | str, workspace: Path, timeout: float) -> _TypeServerClientProtocol:
        if self._client_factory is not None:
            return self._client_factory(executable, workspace, timeout)
        return _PyrightTypeServerClient(executable, workspace=workspace, timeout=timeout)

    @staticmethod
    def _source_specs(workspace: Path, sources: Iterable[str]) -> list[PythonImportSpec]:
        specs: list[PythonImportSpec] = []
        for raw in sources:
            try:
                rel = normalize_relative_path(str(raw).replace("\\", "/"), allow_root=False)
            except ValueError:
                continue
    ...
    def _producer(package_version: str | None, protocol_version: str | None) -> str:
        ...
        return f"pyright-typeserver:{package_version or 'unknown'}:tsp-{protocol_version}"

    @staticmethod
    def _resolve_spec(
        client: _TypeServerClientProtocol, spec: PythonImportSpec, source_uri: str, snapshot: int
    ) -> tuple[str | None, int, str | None]:
        try:
            return client.resolve_import(source_uri, spec.leading_dots, spec.name_parts, snapshot), snapshot, None
        except _JsonRpcError as exc:
            if exc.code != -32802:
                return None, snapshot, str(exc)
        refreshed = client.snapshot()
        try:
            result = client.resolve_import(source_uri, spec.leading_dots, spec.name_parts, refreshed)
            return result, refreshed, None
        except (OSError, RuntimeError) as exc:
            return None, refreshed, str(exc)

    def _collect_edges(
        self, client: _TypeServerClientProtocol, specs: list[PythonImportSpec], state: _EdgeCollectState
    ) -> list[NativeFileEdge]:
        edges: list[NativeFileEdge] = []
        seen: set[tuple[str, str, str]] = set()
        for spec in specs:
            source_uri = (state.workspace / spec.source).as_uri()
            result_uri, state.snapshot, error = self._resolve_spec(client, spec, source_uri, state.snapshot)
            if error is not None:
                state.warnings.append(f"{spec.source}: {spec.display}: {error}")
                continue
            target = None if result_uri is None else _uri_to_workspace_path(result_uri, state.workspace)
            ...
            edges.append(NativeFileEdge(
                source=spec.source, target=target, kind="python-import-resolution",
                confidence="native", producer=state.producer, specifier=spec.display,
            ))
        return edges
    ...
    def collect(
        ...
        *,
        ...
        if executable is None:
            return PyrightTypeServerResolution(self.name, (), ("pyright-typeserver executable unavailable",))
        ...
        try:
            ...
            state = _EdgeCollectState(canonical_workspace, client.snapshot(), producer, warnings)
        ...
        return PyrightTypeServerResolution(
            producer=self._producer(package_version, protocol_version), edges=tuple(edges),
            warnings=tuple(dict.fromkeys(warnings)), protocol_version=protocol_version, package_version=package_version,
        )
