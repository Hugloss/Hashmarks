...
Before handing an edit back, run `make agent-finish`. If Ruff is available, apply and verify canonical Ruff formatting plus the blocking correctness/import rules. Historical complexity/size debt is governed separately by `make lint-debt-gate`; do not use raw full-repository `ruff check .` as a substitute for that baseline-aware authority. If Ruff is unavailable, run portable repository hygiene, do not install or substitute another Python formatter merely to mimic Ruff, do not manually guess Ruff formatting, and report canonical formatting as `UNVERIFIED`.
...
After every meaningful repair batch and before expensive validation, run `make agent-finish`, then focused validation, then continue semantic dogfood. Before CI or handoff when the lint environment is available, run `make agent-preflight` to add the baseline-aware Ruff debt no-growth gate. Formatting normalization is routine candidate hygiene, not a campaign boundary, and successful formatting is not evidence that repaired behavior is correct.
