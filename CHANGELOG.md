...
- Add an explicit producer-side repository declaration discovery SPI and deterministic discovery wrapper: caller-selected providers may detect/normalize producer syntax, while Hashmarks keeps provider provenance/detection separate from declaration semantics, fails closed on provider errors, performs no ambient plugin loading, and keeps arbitrary provider execution outside MCP.
