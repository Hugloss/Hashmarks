...
- Replace the declaration-provider context's raw workspace path with bounded admitted repository-file enumeration that reuses the shared repository admission/pruning/symlink discovery owner without requiring parser support for the producer format. Provider enumeration queries/results are observed and revalidated, so dynamic completeness/discovery cannot quietly depend on an untracked filesystem snapshot.
